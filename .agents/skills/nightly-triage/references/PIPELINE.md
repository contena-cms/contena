...
```bash
gh run view <RUN_ID> --repo contena/contena \
  --json displayTitle,workflowName,headBranch,conclusion,event
gh run view <RUN_ID> --repo contena/contena --json jobs \
...
gh run view --repo contena/contena --job <JOB_ID> --log-failed > job-<slug>.log

# PHPUnit failure headers — errors and failures are SEPARATE numbered lists,
# so "1)" appears twice per log; dedupe with sort -u
grep -oE '[0-9]+\) Contena\\Tests\\[A-Za-z0-9\\]+::[A-Za-z_0-9]+' job-<slug>.log \
...
1. FQCN → path: `Contena\Tests\Integration\…` → `tests/integration/…` + `.php`.
2. Resolve the mirrored source file: `tests/integration/X` → `src/X`, stripping
   the trailing `Test` where needed.
3. Use the most-specific matching source-directory mapping from DOMAINS.md.
4. If no source file exists, use the test-path heuristic from DOMAINS.md.
5. Apply ROUTING.md overrides (they beat all of the above).
...
- **Reproduce** — the standard block below, so each team can rerun its
  ...
  **Run a failing test locally with major flags:**
  ...
  then rerun the test (this matches the CI job env in [`.github/workflows/integration-major.yml`](https://github.com/contena/contena/blob/trunk/.github/workflows/integration-major.yml)).
