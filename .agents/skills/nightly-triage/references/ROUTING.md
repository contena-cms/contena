# Routing: Source Path → Domain Label, Overrides, Known Causes
...
## Source path → domain label (condensed)

| Source path | Label |
|---|---|
| `src/Core/Framework/**`, `src/Core/DevOps/**`, `src/Core/Migration/**` | `domain/framework` + one `component/*` |
| `src/Core/System/User/**`, `src/Core/System/Acl/**`, `src/Core/System/SystemConfig/**` | `domain/framework` + one `component/*` |
| `src/Core/System/Country/**`, `src/Core/System/DataDictionary/**` | `domain/discovery` |
| `src/Core/Content/Media/**` | `domain/discovery` |
| `src/Administration/**` | match the backend domain when possible; otherwise `domain/framework` + `component/administration` |
...
| Path scope | Default route | Route to |
|---|---|---|
...
- Ownership of the root cause = the most-specific path mapping for the file
  where the breaking change lives, with routing overrides applied.
...
- Path surprises are real: shared framework failures can surface in a domain
  test. Follow the test path for initial ownership and the confirmed root
  cause path for filing.
