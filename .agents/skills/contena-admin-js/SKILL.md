...
- Administration Vue components must be native `.vue` SFCs using `<script setup>`. Base components declare extension-facing bindings with one top-level `ctDefinePublic({ ... })`; `.override.vue` components use `useCtPreviousState()` and one top-level `ctDefineOverride({ ... })`. Do not author `createExtendableSetup` wrappers or `ComponentPublicApiMapping` entries.
