...
These are illustrative final outputs in the **strict JSON shape** emitted by the gh aw CI workflow (`.github/workflows/ct-triage.md`). The interactive skill emits the equivalent information as Markdown — the field semantics are identical, only the wire format differs. Your actual reasoning, paths, SHAs, and issue numbers must come from your real investigation — **never invented**.
...
**Emission rules:** the unattended gh aw workflow emits the JSON object as its final message — no markdown code fence, no preamble, no trailing prose. The post-run processor (`.github/workflows/process-ct-triage-result.yml`) runs the validator (`.github/bin/js/validate-ct-triage-output.ts`) to enforce these constraints + scan for secret leakage before publishing deterministic issue updates.
...
```json
{
  ...
  "duplicate_of": null,
  "missing_template_fields": ["contena_version", "affected_area", "actual_behaviour", "expected_behaviour", "reproduction_steps"],
  "affected_paths": [],
  "related_issues": [],
  "related_prs": [],
...
```json
{
  ...
  "confidence": 0.88,
  "reasoning": "Same defect as #15800: 'ct-media-upload-v2 cannot be cleared'. gh issue view 15800 shows matching actual_behaviour + repro. #15800 still open, no fix on trunk.",
  "evidence_quotes": [
    "[issue] ct-media-upload-v2 ... can't be cleared anymore",
    "[shell] issue #15800: 'media upload cannot be cleared once set'"
  ],
  "duplicate_of": 15800,
  "missing_template_fields": [],
  "affected_paths": ["src/Administration/Resources/app/administration/src/component/form/ct-media-upload-v2"],
  "related_issues": [],
  "related_prs": [],
  "recent_commits_in_area": [],
  "change_size_estimate": "unknown"
...
```json
{
  ...
  "suggested_labels": ["domain/framework", "component/core"],
  "confidence": 0.82,
  "reasoning": "Reporter: 'plugin XYZ doesn't work after install'. rg confirms plugin XYZ is third-party (not in src/). Behaviour matches the plugin's documented `contena.yaml` config requirement. Not a core defect.",
  "evidence_quotes": [
...
```json
{
  ...
  "confidence": 0.79,
  "reasoning": "Reporter wants product list sortable by margin. rg shows ct-product list view exposes a fixed sortable column set (name/stock/price); margin is not a stored column. New capability, not a regression.",
  "evidence_quotes": [
