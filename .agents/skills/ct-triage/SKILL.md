...
name: ct-triage
description: >
  Triage a Contena 6 GitHub bug issue. Read the issue body, identify the affected
  ...
  label, or assess a Contena issue, when they reference an issue by number
...
# Contena Issue Triage

## Context (interactive)

You operate inside the `contena/contena` monorepo with full read access to the codebase and to GitHub via shell tools. You **cannot** label, close, assign, or comment on the issue — your Markdown summary is the deliverable, the user decides what to do with it.

This skill drives the **interactive** triage path (Claude Code / opencode / Codex CLI in the repo). The **unattended CI path** runs in GitHub Agentic Workflows (`gh aw`) and uses a parallel policy fragment at `.github/aw/ct-triage-policy.md` that emits JSON via `upload-artifact` instead of Markdown. Both modes load the same shared policy from **`.github/aw/shared/ct-triage-policy.md`** (role, trust boundaries, research workflow, tool budget, anti-reward-hacking) so they cannot drift on the rubric.

## Invocation

The user typed something like "triage issue #16599". Apply the shared policy in **`.github/aw/shared/ct-triage-policy.md`** — start with its Step 0 (fetch the issue) and continue through Step 6 (emit).
...
Apply the policy in `.github/aw/shared/ct-triage-policy.md`, then emit your Markdown summary using the structure above. The Markdown is your only output.
