...
- **`not-a-bug`** — Working-as-designed, config question, third-party plugin issue, support request misfiled as bug, or **product/user-centric request** that belongs in the Contena Feedback & Ideas portal rather than as a code change.
...
Severity reflects **impact × probability**: how broken is the system AND how many merchants are realistically affected? Contena uses separate `priority/*` labels for business urgency — do not invent those.

Exactly one of:

### `critical`

Immediate, direct impact on customers / partners / Contena itself. Includes data loss, security, legal, large revenue/cost impact, or a flagship feature losing its main functionality.

Concrete Contena patterns that are usually `critical`:
...
Confidence is your subjective probability that `disposition + severity + primary domain + duplicate_of` match what a senior Contena engineer would conclude after the same 5–10 minutes of investigation.
