...
name: ct-bugfixer
description: >
  Diagnose and fix a Contena 6 GitHub issue, or improve an existing Bugfixer pull
  request after maintainer feedback. Use when the user asks to fix an issue,
  create a bugfix PR, handle a qi/ct-bugfixer issue, improve a Bugfixer PR, or react to
  /ct-bugfixer feedback.
license: MIT
...
You operate inside the `contena/contena` monorepo with local edit access and
...
from `.github/workflows/ct-bugfixer.md` and uses safe outputs for branch creation,
commits, PR creation, PR branch updates, comments, and no-op results.

Both modes use the same shared policy from
**`.github/aw/shared/ct-bugfixer-policy.md`**. Apply that policy first; keep any
...
- "fix issue #12345"
- "this issue has `qi/ct-bugfixer`, should Bugfixer run?"
- "improve PR #12345 based on the comments"
- "/ct-bugfixer improve ..."

Read `.github/aw/shared/ct-bugfixer-policy.md`, then determine whether this is a
...
Apply the policy in `.github/aw/shared/ct-bugfixer-policy.md`, perform the focused
