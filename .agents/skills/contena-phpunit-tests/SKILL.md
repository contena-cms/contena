---
name: contena-phpunit-tests
description: Write or update Contena PHPUnit tests. Use when adding or changing tests under tests/ (unit, integration, migration, BC) — including data providers, feature flags, and coverage annotations.
license: MIT
---

# Contena PHPUnit Tests
...
| --- | --- | --- |
| `Context::createDefaultContext()` | Platform-owned data only | Platform-owned data only |
| Target tenant Context | That tenant's data only | That tenant's data only; ownership is injected and remains immutable |
| Another tenant Context | The other tenant's data only; never the target tenant or platform data | Its own data only; attempts to mutate the target tenant or platform data fail |
| `Context::createGlobalContext()` | Platform and all tenant data | Platform-owned data only; attempts to create, update, or delete tenant-owned data fail |

- Include positive writes for Default, the target tenant, the other tenant, and Global; assert the persisted ownership from a Global read.
- Include negative update or delete assertions proving that Default, another tenant, and Global cannot mutate the target tenant's row.
- For derived stores such as OpenSearch, caches, and projections, perform writes with the originating Context and repeat the four-context visibility assertions against the derived read path.
- Reuse `TenantTestBehaviour` for tenant fixtures and contexts. Use `TenantScopeTest` as the reference DAL boundary test instead of recreating tenant setup helpers.
...
- Do not add `#[CoversClass]`, `#[CoversFunction]`, or `#[CoversNothing]` to integration tests. Contena's PHPStan rule allows those attributes only on unit and migration tests.
