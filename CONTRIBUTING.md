...
Contena is available under the MIT license. If you want to contribute code (features or bug fixes), you have to create a pull request and include valid license information. Contribute your code under the MIT license.

If you want more details about available licensing or the contribution agreements we offer, you can contact us at <contact@contena.cn>.

## Contributing to the Contena code base

If you want to learn how to contribute code to Contena, please refer to [Contributing Code](https://developer.contena.cn/docs/resources/guidelines/code/contribution.html).
...
Also, make sure that if you change something in a manner that is relevant to external developers please describe your change in a meaningful way. For more information refer to [this document](https://github.com/contena/contena/blob/trunk/delivery-process/documenting-a-release.md).
...
```bash
git clone git@github.com:contena/contena.git
cd contena
docker compose up -d
```
...
2. Creates the database and installs Contena (`init:db`)
...
**Default login**: Username `admin`, Password `contena`.
...
| `composer init:db` | Drop existing database and reinstall Contena |
...
The IDE will restart with your environment set up inside the container. The container starts automatically each time you reopen the project. The terminal and other tools (including AI agent commands in Cursor) will use the container shell. PHP tooling and other extensions will be configured for optimal use with Contena.
...
Instead of using regular ports, you can use OrbStack's URL generation feature. OrbStack generates URLs like `https://web.orb.local` for each running container, allowing easier access without managing port mappings. This also lets you run multiple Contena instances simultaneously without port conflicts.
...
services:
    web:
        ports: !override []
        environment:
            APP_URL: https://web.ct-trunk.orb.local
...
The `APP_URL` follows the pattern `web.<project-name>.orb.local` — the project name is your folder name. So for a folder called `contena`, the URL becomes `https://web.contena.orb.local`. You can also visit `https://orb.local` in your browser to see all running containers and their URLs.

The `SYMFONY_TRUSTED_PROXIES` setting is required to access Contena via HTTPS using `.orb.local` domains.
...
| `composer install -o` | Install PHP dependencies with optimized autoloader |
| `composer init:db` | Drop existing database and reinstall Contena with demo data |
| `composer init:js` | Install all JavaScript dependencies (admin + storefront + extensions) |
| `composer init:testdb` | Initialize the test database |
| `composer build:js` | Build all frontend assets (admin + storefront) |
| `composer build:js:admin` | Build only the administration frontend |
...
Developer documentation for Contena is available [here](https://developer.contena.cn/docs/). You can also contribute to the documentation by submitting your pull requests to [this repository](https://github.com/contena/docs).

## Translations

Contena translations are done by the community and can be installed from the plugin store. If you wish to improve Contena's translations, you can do so in our [Crowdin project page](https://crowdin.com/project/contena6).
