...
Recently, the need arose to deprecate the `Meteor Admin SDK` public API built into the Contena Core.
We need to be able to deprecate the public API, which consists of `component sections` and `data sets`.

### Component sections
The `ct-extension-component-section` component represents component sections in Contena.
...
<template>
    <ct-extension-component-section
        position-identifier="ct-chart-card__before"
    />
</template>
```
...
createdComponent() {
    Contena.ExtensionAPI.publishData({
        id: 'ct-dashboard-detail__todayOrderData',
        path: 'todayOrderData',
        scope: this,
    });
},
