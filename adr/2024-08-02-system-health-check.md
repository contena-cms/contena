---
title: System Health Checks in Contena
date: 2024-08-02
area: core
...
We will implement system health checks in Contena to monitor certain parts of the system with the aim to detect failures and issues early.
...
1. **Contena\Core\Framework\SystemCheck\BaseCheck**:
    - Defines a base class for all system checks.

2. **Contena\Core\Framework\Health\Check\Category**:
...
3. **Contena\Core\Framework\SystemCheck\Check\Result**:
    - Represents the outcome state of a check.

4. **Contena\Core\Framework\SystemCheck\Check\Status**:
...
5. **Contena\Core\Framework\SystemCheck\Check\SystemCheckExecutionContext**:
...
This distinction is primarily reflected in the `Contena\Core\Framework\SystemCheck\BaseCheck` class method:
```php
...
```php
    protected function allowedSystemCheckExecutionContexts(): array
    {
        return \Contena\Core\Framework\SystemCheck\Check\SystemCheckExecutionContext::readiness();
    }
```
...
```php
    protected function allowedSystemCheckExecutionContexts(): array
    {
        return \Contena\Core\Framework\SystemCheck\Check\SystemCheckExecutionContext::cases();
    }
```
...
```php
    protected function allowedSystemCheckExecutionContexts(): array
    {
        return \Contena\Core\Framework\SystemCheck\Check\SystemCheckExecutionContext::longRunning();
    }
```
