...
* Definition
    ...
    * Each custom entity, is registered with the prefix `custom_entity_` or the `ce_` shorthand.
        * App developers can then define that they would like to have `custom_entity_ct_blog` as an entity
        * To prevent naming collisions, app developers should always add their developer prefix to the entity name 
        * We then create the `custom_entity_ct_blog` table
...
* A request `/api/custom-entity-ct-blog`, then runs into our controller, and we get for the parameter `entity` the value `ct-blog`. We then pass this value to the parent method and prefetch it
