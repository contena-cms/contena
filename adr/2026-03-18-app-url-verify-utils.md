...
Recent improvements in Contena, including the introduction of the fingerprinting mechanism for the Shop ID (see PR https://github.com/contena/contena/pull/11677), have made it possible to detect changes in the Contena environment that should trigger a Shop ID change. Examples include when the APP_URL changes or the environment otherwise looks different, such as when a production shop is copied to a staging system.
...
Some APP URL verification already exists in the Contena Administration, but we want to improve its robustness.

## Decision

When a Shop ID changes (see scenarios below): we verify that the APP_URL points back to the same instance of Contena (itself).

We achieve this by introducing a new public API end point `api/app-system/shop/verify`.

Verification flow:

1. Contena calls this endpoint on the instance defined by the configured APP_URL environment variable.
...
* The installation path of Contena changed (when no apps installed).
