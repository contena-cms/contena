...
Contena normalizes persisted date/time values to UTC, while different parts of the system choose their display timezone differently. The Storefront can set Twig's default timezone from the browser timezone cookie, and the Administration uses the logged-in user's profile timezone for date formatting. Server-side rendering for documents and mails does not have one consistent merchant-controlled timezone.

Depending on the entry point, templates can fall back to UTC, as reported for documents in [#15139](https://github.com/contena/contena/issues/15139).

## Decision

We add an optional `businessTimeZone` field to sales channels. When it is set, Contena treats it as the merchant-controlled timezone for server-side rendering of sales-channel output such as documents and mails. Code that needs to apply a timezone for one Twig render call uses `renderWithTimezoneOverride` on Contena's Twig environment, which temporarily changes Twig's default timezone and restores the previous value afterwards:
