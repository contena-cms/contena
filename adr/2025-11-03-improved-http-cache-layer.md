...
Contena currently supports an HTTP-cache layer; however, the current implementation has some limitations:
...
* **Actual cache-control configuration is hard-coded and splattered**: The values set for `cache-control` headers are hard-coded and splattered (e.g., hardcoded in the reverse proxy config and in contena), they cannot be configured based on projects needs, and also on route level only the max-age is configurable.
...
We will simplify the reverse proxy configuration by only relying on the `ct-cache-hash`, as the only application state that the reverse proxy needs to take into account.
The `ct-cache-hash` will be set as a cookie and as a header, additionally we add the `ct-cache-hash` header name as a `vary` header to the response. So the response headers will look like this:
```
ct-cache-hash: theHash
vary: ct-cache-hash
set-cookie: ct-cache-hash=theHash;
```
...
The only contena specific reverse proxy configuration will be to set the `ct-cache-hash` header based on the `ct-cache-hash` cookie on the reverse proxy.
...
So the configuration (on the contena side, as well as on the reverse proxy) will be the same as in the storefront.
