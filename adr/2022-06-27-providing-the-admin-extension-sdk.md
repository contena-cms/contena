...
The SDK is not hard-bound to Contena version like plugins. This dependency will be included directly in the apps and also supports newer Contena versions. This allows us to provide separate releases independent of the administration. Example: A small bug can be fixed and released in minutes. A similar deployment in the platform would take much more time.

### SDK is just a convenience layer for the PostMessage API
Similar as the previous reason the SDK is not hard-bound to the Contena releases and is a completely separate package. And if we would add it into the monorepo we would be bound to the Contena release cyclus. With a separate repository we can work independent and react faster.
