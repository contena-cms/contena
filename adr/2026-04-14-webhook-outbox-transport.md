...
A core motivation is **delivery consistency across deployment topologies**. Contena runs on MySQL-only stacks, managed SQS, RabbitMQ, and various hosting environments. Each transport has different retry semantics. The webhook contract with app developers should be the same regardless of which Messenger transport the shop has configured.
...
- **`RetryWebhookMessageFailedSubscriber`** proves the pattern of Contena-owned failure handling layered on top of Messenger.

The work tracked under [contena/contena#16560](https://github.com/contena/contena/issues/16560) and merged via [#16692](https://github.com/contena/contena/pull/16692) is the Phase 1 implementation of this ADR.

## Decision

We implement webhook delivery as a dedicated Symfony Messenger transport (`contena-webhook://`) backed by a MySQL application-level outbox with **best-effort FIFO** delivery per app. Phase 1 — the subject of this ADR — ships the transport foundation. Phase 2 and Phase 3 extend it with endpoint health and consumer tooling and are summarised at the end as roadmap.
...
- **Dedicated transport.** `WebhookEventMessage` routes to `contena-webhook://default`; Messenger's transport-level retry is disabled — the outbox owns the lifecycle.
...
- **Contena-owned retry.** Failures move to `PENDING_RETRY` with `next_retry_at`. Fixed schedule **5s → 30s → 5min → 30min → 4h**.
...
- **Consumer contract headers.** Rework envelopes carry `X-Contena-Event-Id`, `X-Contena-Sequence`, `X-Contena-Attempt`.
...
A single transient failure does **not** stall the partition. The failed message moves to `PENDING_RETRY` and the loop continues. Retries are picked up later when due — they may arrive after newer messages, breaking strict order. Consumers reconcile via `X-Contena-Sequence`.
...
|:---|:---|:---|
| `X-Contena-Event-Id` | Deduplication. Stable across retries. **Sole** idempotency key. | `018f3a2b-…` |
| `X-Contena-Sequence` | Monotonic counter for last-write-wins reordering of retries. | `48291` |
| `X-Contena-Attempt` | 0-indexed attempt counter. Distinguishes first delivery from retries. **Not** part of the dedupe key. | `0`, `1`, `2` |
...
| **Best-effort FIFO (this ADR)** | Insertion order on the happy path; retries break order | Always progresses | Throughput scales with partition count; consumers must reorder via `X-Contena-Sequence`. |
...
- **Application-level outbox (chosen)** — webhook layer owns the outbox; decoupled from DAL via the event dispatcher. Fits Contena's dispatch-after-commit model and the extension architecture. The accepted trade-off is that a process crash strictly between the business commit and the outbox write would lose the event.
...
- **At-least-once delivery.** The same event may be delivered multiple times (retries, crash recovery, future probing). Deduplicate by `X-Contena-Event-Id` only — it is unique per event and stable across retries.
- **Best-effort ordering.** Within a partition (one per app by default), first-attempt delivery is in insertion order. If A fails and retries, B and C still deliver. When A's retry arrives it may be after B and C — use `X-Contena-Sequence` for last-write-wins reordering.
...
- **Retry attempts.** `X-Contena-Attempt` is 0-indexed and changes on each delivery attempt. It is not stable across retries and must not be part of a dedupe key.
...
- **Retry behaviour is owned by Contena, not the queue backend.** The outbox drives retry timing (5s → 4h) so the contract for app developers is identical on MySQL, Redis, AMQP, or SQS deployments instead of varying with whichever Messenger transport the shop has configured.
