...
Contena has several consumers for repository guidance: human contributors, Codex, Claude, Gemini, and other agent tools.
...
Contena-specific skills live in this repository under `.agents/skills/`.
...
The accepted downside is that exact reuse across plugin or other repositories is harder. That trade-off is intentional: Contena-specific skills should stay close to the branch-local platform guidance they depend on instead of becoming a second source of truth.

## Initial Skills

- `contena-knowledge-capture` for saving durable knowledge and routing it to AGENTS, coding guidelines, README, ADR, skills, or local notes.
...
- `contena-change-scope` for root-cause analysis, boyscouting, and cleanup scope.
- `contena-release-docs` for release notes, upgrade notes, and developer-facing changelog decisions.
- `contena-pr-hygiene` for PR templates, conventional titles, and review follow-up commits.
- `contena-php-code` for PHP architecture, API schema, migrations, deprecations, and BC-sensitive code.
- `contena-admin-js` for Administration JavaScript, TypeScript, Vue, ACL, and Jest work.
- `contena-phpunit-tests` for PHPUnit test structure, fixtures, feature flags, coverage, and data providers.
