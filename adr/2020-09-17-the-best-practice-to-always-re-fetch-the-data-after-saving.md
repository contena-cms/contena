...
<ct-switch-field
    v-model="data.status"
    :label="$tc('ct-review.detail.labelStatus')">
</ct-switch-field>

<!-- we will save data with onSave method -->
<ct-button-process @click="onSave">
...
onSave() {
    this.repository.save(this.data, Contena.Context.api).then(() => {
...
loadEntityData() {
    ...
    const context = { ...Contena.Context.api, inheritance: true };
