...
## Consequences
### Removal of Vuex
Below you will find an overview of what will be removed on which Contena Version.

#### 6.7
For Contena 6.7 we want to transition all our modules but still leave the possibility for you to use Vuex for your own states.

- All `Contena.State` functions will cause warnings to appear in the DevTools. For example `Contena.State.registerModule is deprecated. Use Contena.Store.register instead!`
- All Vuex state definitions will be transitioned to Pinia:
    - src/module/ct-bulk-edit/state/ct-bulk-edit.state.js
    - src/module/ct-product/page/ct-product-detail/state.js
    - src/module/ct-category/page/ct-category-detail/state.js
    - src/module/ct-extension/store/extensions.store.ts
    - src/module/ct-settings-payment/state/overview-cards.store.ts
    - src/module/ct-settings-seo/component/ct-seo-url/state.js
    - src/module/ct-settings-shipping/page/ct-settings-shipping-detail/state.js
    ...
    - src/app/state/contena-apps.store.ts
    ...
    - src/module/ct-flow/state/flow.state.js
    - src/module/ct-order/state/order.store.ts
    - src/module/ct-order/state/order-detail.store.js
    - src/module/ct-profile/state/ct-profile.state.js
    - src/module/ct-promotion-v2/page/ct-promotion-v2-detail/state.js

#### 6.8
With Contena 6.8 we will entirely remove everything Vuex related including the dependency.

- `Contena.State` - Will be removed. Use `Contena.Store` instead.
...
## Transition to Pinia
Pinia calls its state-holding entities `stores`. Therefore, we decided to hold everything Pinia-related under `Contena.Store`.
The `Contena.Store` implementation follows the Singleton pattern. The private constructor controls the creation of the Pinia root state.
...
For now, we have decided to limit the public API of `Contena.Store` to the following:
...
The rest of the previous Vuex (`Contena.State`) public API is implemented into Pinia itself.

```typescript
// Setup
const piniaStore = Contena.Store.get('...');

// From Vuex subscribe
Contena.State.subscribe(...);
// To Pinia $subscribe
store.$subscribe(...);

// From Vuex commit
Contena.State.commit(...);
// To Pinia action call
store.someAction(...);

// From Vuex dispatch
Contena.State.dispatch(...);
// To Pinia action call
...
5. Use `Contena.Store.register` instead of `Contena.State.registerModule`.

Let's look at a simple Vuex module and how to transition it:
```typescript
// Old Vuex implementation
Contena.State.registerModule('example', {
    state: {
        id: '',
    },
    getters: {
...
// Notice that the mutation setId was removed! You can directly modify a Pinia store state after retrieving it with Contena.Store.get.
