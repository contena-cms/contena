...
An app wants to render a custom iFrame in a card on the dashboard. The "location" of the iFrame has then a specific "locationID" like `ct-dashboard-example-app-dashboard-card`. The app can also render another iFrames which also get "locationIDs". In our example it is a iFrame in a custom modal: `example-app-example-modal-content`.
...
```js
if (ct.location.is('ct-dashboard-example-app-dashboard-card')) {
    renderDashboardCard();
}

if (ct.location.is('example-app-example-modal-content')) {
    renderModalContent();
}
```
...
ct.ui.componentSection('ct-manufacturer-card-custom-fields__before').add({
