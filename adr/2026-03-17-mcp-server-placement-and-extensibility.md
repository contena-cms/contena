...
Contena now contains an experimental MCP server foundation that exposes a native `/api/_mcp` endpoint and integrates with Contena authentication, ACL, rate limiting, feature flags, and capability discovery.
...
- plugins use the `contena.mcp.tool` DI tag, remapped by `McpToolDiscoveryCompilerPass`
...
- the `contena-context` system prompt
...
- the capability should scale or deploy independently from Contena
...
- `contena-entity-upsert`
- `contena-entity-delete`
- `contena-order-state`
- `contena-system-config-read`
- `contena-system-config-write`

If Contena wants a conservative core MCP surface, some of these can start in a plugin and be promoted later.
...
- `contena-order-summary` (currently in Core)
- `contena-customer-lookup` (currently in Core)
- `contena-product-create` (currently in Core)
- `contena-revenue-report` (currently in Core)
- `contena-bestseller-report` (currently in Core)
- `contena-storefront-search` (currently in Core)
- `contena-cart-manage` (currently in Core)
- `contena-cart-checkout` (currently in Core)
- `contena-checkout-methods` (currently in Core)
- `contena-theme-config` (already in Storefront bundle)
- `contena-media-upload` (currently in Core)

`contena-theme-config` is already correctly placed in `src/Storefront/Mcp/Tool/` because it depends on Storefront-specific services. The remaining tools all live in `src/Core/Framework/Mcp/Tool/` today and need to be moved if this decision is adopted.

Note that several tools with "storefront" or "cart" in their name (`contena-storefront-search`, `contena-cart-manage`, `contena-cart-checkout`, `contena-checkout-methods`) use Core SalesChannel services, not Storefront bundle services. The move is about classification as assistant workflows, not about fixing dependency direction violations.
...
Plugins and bundles register custom tools, prompts, and resources through DI and use normal Contena services. This is the preferred model for domain-specific capabilities, project-specific workflows, vertical features, and capabilities depending on optional bundles.

The current implementation already supports this:

- plugins tag services with `contena.mcp.tool` (remapped by `McpToolDiscoveryCompilerPass`)
...
- naming rules (`contena-*` for core, `{plugin}-*` for plugins, `{app}-*` for apps)
...
|---|---|---|---|
| Needed for every Contena MCP installation? | Best fit | Maybe | Rarely |
...
foreach tagged('contena.mcp.tool') as $service:
...
```php
// Plugin tool class
#[McpTool(name: 'ct-erp-sync-orders', description: '...')]
...
// Plugin services.xml -- uses contena.mcp.tool tag (remapped by McpToolDiscoveryCompilerPass)
<service id="Ct\Erp\Mcp\Tool\SyncOrdersTool">
    <tag name="contena.mcp.tool"/>
    <tag name="contena.feature" flag="MCP_SERVER"/>
</service>

// Non-core bundle (e.g. Storefront) -- uses mcp.tool tag directly
<service id="Contena\Storefront\Mcp\Tool\ThemeConfigTool">
    <tag name="mcp.tool"/>
    <tag name="contena.feature" flag="MCP_SERVER"/>
</service>
```
...
client calls contena-mcp-endpoint -> capability registry resolves app tool
...
|---|---|---|
| `OrderSummaryTool` | `contena-order-summary` | Workflow |
| `CustomerLookupTool` | `contena-customer-lookup` | Workflow |
| `ProductCreateTool` | `contena-product-create` | Workflow |
| `RevenueReportTool` | `contena-revenue-report` | Reporting |
| `BestsellerReportTool` | `contena-bestseller-report` | Reporting |
| `StorefrontSearchTool` | `contena-storefront-search` | Storefront |
| `CartManageTool` | `contena-cart-manage` | Storefront |
| `CartCheckoutTool` | `contena-cart-checkout` | Storefront |
| `CheckoutMethodsTool` | `contena-checkout-methods` | Storefront |
| `MediaUploadTool` | `contena-media-upload` | Media |

### What stays in core (9 tools + 1 prompt + 7 resources)

| Tool class | MCP name |
|---|---|
| `EntitySchemaTool` | `contena-entity-schema` |
| `EntitySearchTool` | `contena-entity-search` |
| `EntityReadTool` | `contena-entity-read` |
| `EntityAggregateTool` | `contena-entity-aggregate` |
| `EntityUpsertTool` | `contena-entity-upsert` |
| `EntityDeleteTool` | `contena-entity-delete` |
| `OrderStateTool` | `contena-order-state` |
| `SystemConfigReadTool` | `contena-system-config-read` |
| `SystemConfigWriteTool` | `contena-system-config-write` |

Plus `ContenaContextPrompt` and all seven resources (entities, sales channels, currencies, languages, state machines, business events, flow actions).

### Already correctly placed

`ThemeConfigTool` (`contena-theme-config`) already lives in `src/Storefront/Mcp/Tool/` with its DI registration in `src/Storefront/DependencyInjection/mcp.xml`.
...
3. register it in the target plugin or bundle's DI config (using `contena.mcp.tool` tag for plugins, `mcp.tool` for bundles)
...
- Contena keeps a native MCP platform instead of outsourcing the whole feature
...
Treating both kinds as if they belong in core would overcommit the platform too early. Treating both kinds as if they belong outside core would throw away the value of Contena-native auth, ACL, runtime integration, and discovery.
