...
Right now only Contena Backend is able to communicate with the App Server in a secure way.
This is because the App Server is able to verify the origin of the request by checking request signature, 
and this signature is generated by the Contena 6 backend application with a shop to app server secret.
...
sequenceDiagram
    participant Client
    participant Contena Backend
    participant App Server
    Client->>Contena Backend: GET /store-api/app-system/TestApp/generate-token
    Contena Backend->>Client: Responds with Signed JWT Token
