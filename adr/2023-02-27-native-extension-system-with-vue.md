...
## Context
Our current plugin extension system for the administration is based on our Component Factory. This factory generates native Vue components at runtime based on a base Contena component which can be extended or overwritten by plugins. This approach offers flexibility to plugin developers to modify every component. For the template part, we use Twig.JS and compile the Vue template in the client at runtime.
...
Our main idea for the template part was to use native Vue components named ct-block that can replace the Twig.JS template part. Plugin developers can extend or overwrite the ct-block component as needed.

```html
<ct-block name="ct-hello-world">
<div>Hello World</div>
</ct-block>
```
...
<script lang="ts">
    import SwOrignal from './ct-original.vue';
    // use our extensibility helpers
    import { useComponentExtender, getPropsFromComponent } from 'ct-vue-extensbiles';
    
    const myCustomData = ref('test');
    
    export default {
        name: 'ct-extended-component',
        props: {
