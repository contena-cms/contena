...
- Due to the simple unpacking of a changeset of the Contena update, the user cannot use Composer to install additional packages
- Extensions have to package their own dependencies inside their extension zip and overwrite dependencies of Contena or other extensions which can cause problems
...
This allows us to react faster on bugs and implement new features, without having to wait for a new Contena release.

The new web updater will use the same update process as the CLI update using composer.

So the process will be:

- The Contena Admin will do a basic update check that all extensions are compatible with the next Contena version
...
- Run `composer update` to update Contena
- Run `bin/console` to update the database
- Disable the maintenance mode
- Delete the Phar and Redirect the user to the Contena Admin

The new way of managing a Contena project will also allow us to setup a new project with the new tool.
...
To merge the upcoming changes of the `.htaccess` file we will use our already existing [UpdateHtaccess](https://github.com/contena/platform/blob/6.4.17.0/src/Core/Framework/Update/Services/UpdateHtaccess.php) which is using the Markers to update only our own changes.
