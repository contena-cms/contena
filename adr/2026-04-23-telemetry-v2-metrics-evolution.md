...
Level 1 — Global off: When `contena.telemetry.metrics.enabled` is `false`:
  - `Meter::emit()` checks the `enabled` flag and returns immediately. This is already near-no-op.
  - A compiler pass removes services tagged with `contena.telemetry.subscriber` (event subscribers,
    flush listener) and `contena.telemetry.periodic_metric_collector` from the container, so they
...
**Subscriber identification**: Telemetry subscribers are identified by the `contena.telemetry.subscriber`
DI tag, added to their service definitions. Periodic-metric collectors are identified by the
`contena.telemetry.periodic_metric_collector` tag. When telemetry is globally disabled, the compiler
...
- Replacement value: Configured globally via `contena.telemetry.metrics.replace_unknown_label_values_with`
...
by a Contena scheduled task iterating tagged `PeriodicMetricCollectorInterface` services:
...
The task uses Contena's standard scheduled-task scheduling — `CollectPeriodicMetricsTask::getDefaultInterval()`
...
Plugins needing a different frequency register their own Contena scheduled task.
...
final class Span
{
    public function __construct(
        public readonly string $name,
        public readonly string $category = 'contena',
...
- **Transport packages** (contena/opentelemetry, contena/prometheus-exporter):
...
- **Developers**:
  - Telemetry subscribers should be tagged with `contena.telemetry.subscriber` for zero-overhead global disable.
...
- **Operators**:
  - Use `contena.telemetry.metrics.enabled: true/false` to enable/disable globally
