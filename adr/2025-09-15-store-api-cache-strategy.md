...
At the same time, Storefront (classical Contena frontend based on twig templates, mentioned here and further in
this doc only for context and strategy alignment) already has a caching mechanism in place and Contena provides
...
3. Use cache headers (not cookies) to differentiate contexts on Store API
   - Use `ct-currency-id` and `ct-language-id` to differentiate currency and language. Contena must update currency and
     language ids in the context of the current request based on these headers.
   - Use `ct-context-hash` to differentiate other context aspects (e.g. logged in customer, active rules, etc). Use the
...
- SDK updates
   ...
   - Implement automatic request method selection (fallback to POST when the compressed _criteria would exceed practical
     URL limits).
   - Track and resend `ct-currency-id`, `ct-language-id`, and `ct-context-hash` headers on subsequent requests.
- Clients
   ...
   - Client should beware that `ct-currency-id`, `ct-language-id` can change the response language and currency.
- Extensions 
...
3. Two-step flow: POST returns a request hash; GET retrieves cached data by hash:
   ...
   - More complex implementation for Contena (need to store request hashes and map them to actual requests).
