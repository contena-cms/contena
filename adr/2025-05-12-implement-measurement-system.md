...
The `MeasurementUnits` will be initialized in `\Contena\Core\System\SalesChannel\Context\SalesChannelContextFactory::create` based on current sales channel domain
...
```php
namespace Contena\Core\Content\Product\Subscriber;
...
- `ct-measurement-length-unit`: overrides the default length unit
- `ct-measurement-weight-unit`: overrides the default weight unit
...
class ProductSubscriber implements EventSubscriberInterface 
{
    ...
    {        
        $lengthUnit = $request->headers->get('ct-measurement-length-unit', 'mm');
        $weightUnit = $request->headers->get('ct-measurement-weight-unit', 'kg');
