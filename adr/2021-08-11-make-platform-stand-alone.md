---
title: Make contena/platform stand-alone for development and testing
date: 2021-08-11
area: product-operations
tags: [contena, platform, development, testing]
---
...
running the application. In practice this is provided by one of the templates: `contena/development` or `contena/production`.
This creates a cyclic dependency, which brings some problems:
- `contena/development` and `contena/platform` need to be updated in lockstep, which makes updating them individually sometimes impossible
...
- use contena/platform directly in the pipeline
- allow development without a template by moving the development tooling into platform
- only advertise this as `contena/platform` development setup. Projects should still start with `contena/production` as a template
- `contena/development` should continue to work
...
- projects may try to use contena/platform directly
- yet another contena setup to choose from
