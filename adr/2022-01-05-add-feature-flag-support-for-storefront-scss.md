...
* The feature configuration from `Feature::getAll()` is converted to a SCSS map inside `\Contena\Storefront\Theme\ThemeCompiler::getFeatureConfigScssMap`.
  * This SCSS map is always added to the SCSS string which gets processed by `\Contena\Storefront\Theme\ThemeCompiler::compileTheme`.
  * For webpack hot-proxy the `var/config_js_features.json` is used instead.
* The SCSS map looks like this: `$ct-features: ("FEATURE_NEXT_1234": false, "FEATURE_NEXT_1235": true);`
