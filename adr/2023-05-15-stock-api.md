...
The stock handling in Contena 6 is currently not very flexible and does not support many common use cases.
...
We have a way to disable the stock handling behavior of Contena.
...
namespace Contena\Core\Content\Product\Stock;
...
abstract class AbstractStockStorage
{
     ...
     * * \Contena\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader
     * * \Contena\Core\Content\Product\Stock\LoadProductStockSubscriber
     *
...
### Update `\Contena\Core\Content\Product\DataAbstractionLayer\ProductIndexer`

We update `\Contena\Core\Content\Product\DataAbstractionLayer\ProductIndexer` to depend on `\Contena\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` as well as `Contena\Core\Content\Product\DataAbstractionLayer\StockUpdater`.

If `STOCK_HANDLING` is enabled then we call the `\Contena\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage::index` method with the IDs of the product which have changed.

Otherwise, we call `Contena\Core\Content\Product\DataAbstractionLayer\StockUpdater::update`

### Deprecate `Contena\Core\Content\Product\DataAbstractionLayer\StockUpdater`

It will be removed with 6.6.

### Introduce `Contena\Core\Content\Product\Stock\OrderStockSubscriber`

We introduce a new subscriber which listens to the various required events and interacts with `\Contena\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` via it's new API (`alter`).

All of Contena's internal business rules for handling stock are located in this subscriber.

The subscriber listens to various events and calls `\Contena\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage::alter` with the appropriate changesets for the following scenarios:
...
It is possible to disable Contena's internal stock handling by setting the configuration `contena.stock.enable_stock_management` to false.

### Introduce new core stock storage implementation.

We introduce a new implementation of `\Contena\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` for managing the stock levels. It is responsible for incrementing/decrementing stock values based on the provided changesets.
...
* `available_stock` is calculated from the `stock` value minus open order quantities. This calculation is preformed in `Contena\Core\Content\Product\DataAbstractionLayer\StockUpdater::updateAvailableStockAndSales`. It is slow because the `SUM` may run over millions of rows.
...
* \Contena\Core\Content\Product\DataAbstractionLayer\StockUpdate\AbstractStockUpdateFilter
* \Contena\Tests\Unit\Core\Content\Product\DataAbstractionLayer\StockUpdate\TestStockUpdateFilter
* \Contena\Commercial\MultiWarehouse\Domain\Order\ExcludeMultiWarehouseStockUpdateFilter
* \Contena\Core\Content\Product\DataAbstractionLayer\StockUpdate\StockUpdateFilterProvider

### `ProductDefinition` updates

In Contena version 6.6 or if the `STOCK_HANDLING` feature flag is enabled:
...
We update the various locations in Contena where stock is loaded and augment the product with any stock information that is loaded from the stock storage.

This includes:

* \Contena\Core\Content\Product\Subscriber\ProductSubscriber::salesChannelLoaded
* \Contena\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader::load
...
However, in order to support this API, we must update `\Contena\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader::load` because it currently does not pass along the `SalesChannelContext` which is necessary for `AbstractStockStorage::load`.
...
It is the role of `Contena\Core\Content\Product\Stock\OrderStockSubscriber` to listen to the required contena events for these scenarios and then interact with the stock storage implementation.
