...
*  `issue` (Required): Add the corresponding Jira issue key. Can be the key of a single ticket or the key of an epic. For external contributions this is added by a contena employee, when there is no corresponding issue in Jira yet.
...
*  `author`(Required): This field is optional for contena employees, but required for all external developers. It can be used to identify the author of code changes, or to display the name in a changelog.

*  `author_email`(Required): This field is optional for contena employees, but required for all external developers.
...
Good changelog:
*  Changed method `registerCmsElement()` in `module/ct-cms/service/cms.service.js` to fix the mapping of the element data.
```
...
If you just want to get the changes of a specific version or even just a special part of Contena, you can use a console command to generate the corresponding changelog.
