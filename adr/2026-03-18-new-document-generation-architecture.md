...
Documents can be generated by using the `Contena\Core\Checkout\DocumentV2\DocumentGenerator` service.
...
- Passing in a Contena context with the necessary permissions to generate the document.
...
- A Symfony service.
- Tagged with `contena.documentV2.provider`.
...
- A Symfony service.
- Tagged with `contena.documentV2.renderer`.
...
abstract class AbstractDocumentRenderer
{
    ...
    /**
     * Persist the rendered document to a file, returning its Contena media ID.
     */
...
- They declare whether they support a certain document type by returning `true` from their `supports` implementation.
  - The reason they do not just return an array of supported types is easier extensibility.
    ...
    by giving the tagged service a higher priority than the one provided by Contena.
...
- There was also a decision in the past not to introduce further interfaces and to use abstract classes instead.
  See [2020-11-25-decoration-pattern.md](https://github.com/contena/contena/blob/trunk/adr/2020-11-25-decoration-pattern.md).
