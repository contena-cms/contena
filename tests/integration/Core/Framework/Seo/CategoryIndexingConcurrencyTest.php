...
class CategoryIndexingConcurrencyTest extends TestCase
{
    ...
    {
        // a real channel id is required: the seo_url unique indexes only enforce uniqueness
        // (and therefore only produce the colliding locks) when channel_id is not NULL
        $channelId = $this->connection->fetchOne('SELECT LOWER(HEX(id)) FROM channel LIMIT 1');
        if (!\is_string($channelId)) {
            static::markTestSkipped('At least one channel is required for the reproduction.');
        }
    ...
    private function generateSeoUrls(int $worker): void
    {
        ...
        for ($round = 0; $round < self::ROUNDS; ++$round) {
            ...
            $this->seoUrlPersister->updateSeoUrls($context, self::ROUTE_NAME, $foreignKeys, $seoUrls, $channel);
        }
    }
