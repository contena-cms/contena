...
class PluginServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        require_once $this->fixturePath . 'plugins/CtTestPlugin/src/CtTestPlugin.php';
        require_once $this->fixturePath . 'plugins/CtTestNoDefaultLang/src/CtTestNoDefaultLang.php';
    ...
    public function testRefreshPlugins(): void
    {
        ...
        $plugin = $this->fetchCtTestPluginEntity();
    ...
    public function testRefreshPluginsWithRootComposerJsonContainingPlugin(): void
    {
        ...
        $plugin = $this->fetchCtTestPluginEntity(baseClass: 'Ct\RootTest\RootPlugin');
    ...
    public function testRefreshPluginWithoutExtraLabelProperty(): void
    {
        ...
        $composerJsonException = $errors->filter(static fn (ContenaHttpException $error) => $error instanceof PluginComposerJsonInvalidException);
        ...
        $errorFound = false;
        $errorString = 'Plugin composer.json has invalid "type" (must be "contena-platform-plugin"), or invalid "extra/contena-plugin-class" value, or missing extra.label property';

        foreach ($composerJsonException as $exception) {
            $parameters = $exception->getParameters();
            if (!\array_key_exists('composerJsonPath', $parameters) || !str_contains($parameters['composerJsonPath'], '/plugins/CtTestNoExtraLabelProperty/composer.json')) {
                continue;
            }
    ...
    public function testRefreshPluginsWithNonStandardLanguage(): void
    {
        ...
        $plugin = $this->fetchCtTestPluginEntity($nonStandardLanguageContext);
    ...
    public function testRefreshPluginsWithDifferentDefaultLanguage(): void
    {
        ...
        $plugin = $this->fetchCtTestNoDefaultLangPluginEntity();
    ...
    {
        $context = new Context(new SystemSource(), [$this->getDeDeLanguageId()]);

        $this->pluginService->refreshPlugins($context, new NullIO());

        $plugin = $this->fetchCtTestPluginEntity($context);
    ...
    public function testRefreshPluginsExistingWithPluginUpdate(): void
    {
        ...
        $this->createPlugin($this->pluginRepo, $this->context, CtTestPlugin::PLUGIN_OLD_VERSION, $installedAt);

        $this->pluginService->refreshPlugins($this->context, new NullIO());

        $plugin = $this->fetchCtTestPluginEntity();

        static::assertSame(CtTestPlugin::class, $plugin->getBaseClass());
        static::assertSame(CtTestPlugin::PLUGIN_LABEL, $plugin->getLabel());
        static::assertSame(CtTestPlugin::PLUGIN_VERSION, $plugin->getUpgradeVersion());
    }
    ...
    public function testRefreshPluginsExistingNotInstalledWithPluginUpdate(): void
    {
        ...
        static::assertSame(CtTestPlugin::class, $plugin->getBaseClass());
        static::assertSame(CtTestPlugin::PLUGIN_LABEL, $plugin->getLabel());
        static::assertSame(CtTestPlugin::PLUGIN_VERSION, $plugin->getVersion());
    }
    ...
    {
        $nonExistentPluginBaseClass = 'CtFoo\\CtFoo';
        ...
        $plugin = $pluginCollection->filterByProperty('baseClass', CtTestPlugin::class)->first();
    ...
    public function testGetPluginByName(): void
    {
        ...
        $plugin = $this->pluginService->getPluginByName('CtTestPlugin', $this->context);
    ...
    public function testGetPluginByNameThrowsException(): void
    {
        ...
        $this->expectExceptionObject(PluginException::notFound('CtFoo'));
        $this->pluginService->getPluginByName('CtFoo', $this->context);
    }

    private function assertDefaultPlugin(PluginEntity $plugin): void
    {
        static::assertSame(CtTestPlugin::class, $plugin->getBaseClass());
        static::assertSame(CtTestPlugin::PLUGIN_LABEL, $plugin->getTranslated()['label']);
        static::assertSame(CtTestPlugin::PLUGIN_VERSION, $plugin->getVersion());
    }

    private function assertNoDefaultPlugin(PluginEntity $plugin): void
    {
        static::assertSame(CtTestNoDefaultLang::class, $plugin->getBaseClass());
        static::assertSame(CtTestNoDefaultLang::PLUGIN_LABEL, $plugin->getTranslated()['label']);
        static::assertSame(CtTestNoDefaultLang::PLUGIN_VERSION, $plugin->getVersion());
    }

    private function assertGermanPlugin(PluginEntity $plugin): void
    {
        static::assertSame(CtTestPlugin::class, $plugin->getBaseClass());
        static::assertSame(CtTestPlugin::PLUGIN_GERMAN_LABEL, $plugin->getLabel());
        static::assertSame(CtTestPlugin::PLUGIN_VERSION, $plugin->getVersion());
    ...
    private function assertPluginMetaInformation(PluginEntity $plugin): void
    {
        ...
        static::assertSame('contena AG', $plugin->getAuthor());
        static::assertSame('(c) by contena AG', $plugin->getCopyright());
        static::assertSame('MIT', $plugin->getLicense());
    }

    private function fetchCtTestPluginEntity(?Context $context = null, string $baseClass = CtTestPlugin::class): PluginEntity
    {
        if ($context === null) {
    ...
    private function fetchCtTestNoDefaultLangPluginEntity(?Context $context = null): PluginEntity
    {
        if ($context === null) {
            $context = $this->context;
        }

        $criteria = new Criteria()->addFilter(new EqualsFilter('baseClass', CtTestNoDefaultLang::class));
