...
EOF;
    ...
    public function testWriteUtf8(): void
    {
        ...
        $written = $this->getWriter()->insert($this->registerDefinition(ListDefinition::class), [$data], $this->createWriteContext());

        static::assertArrayHasKey(ListDefinition::ENTITY_NAME, $written);
        static::assertCount(1, $written[ListDefinition::ENTITY_NAME]);
        $payload = $written[ListDefinition::ENTITY_NAME][0]->getPayload();
        static::assertCount(1, $payload['data']);
        static::assertSame('😄', $payload['data'][0]);
    }
