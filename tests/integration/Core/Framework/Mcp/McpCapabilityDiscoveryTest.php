...
class McpCapabilityDiscoveryTest extends TestCase
{
    ...
    public function testInitializeResponseInstructionsGuideToolDiscovery(): void
    {
        ...
        static::assertStringContainsString(
            'contena-tool-search',
            $instructions,
            'Server instructions must point clients at contena-tool-search when no advertised tool matches the requested action.',
        );
    }
    ...
    public function testFreshSessionAdvertisesOnlyDiscoveryTools(): void
    {
        ...
        static::assertContains('contena-tool-search', $tools);
        static::assertContains('contena-toolset-enable', $tools);
        static::assertContains('contena-toolsets-list', $tools);

        // ...but no domain tool is, until its toolset is enabled.
        static::assertNotContains('contena-entity-schema', $tools);
        static::assertNotContains('contena-entity-search', $tools);
        static::assertNotContains('contena-entity-read', $tools);
        static::assertNotContains('contena-entity-delete', $tools);
        static::assertNotContains('contena-system-config-read', $tools);
    }
    ...
    public function testEnablingToolsetDeliversToolsListChangedNotification(): void
    {
        ...
        $browser->request(
            ...
            json_encode([
                'jsonrpc' => '2.0',
                'method' => 'tools/call',
                'params' => ['name' => 'contena-toolset-enable', 'arguments' => ['toolset' => $toolsetName]],
                'id' => 2,
            ], \JSON_THROW_ON_ERROR),
        );
    ...
    {
        yield 'contena-toolsets-list' => ['contena-toolsets-list'];
        yield 'contena-toolset-enable' => ['contena-toolset-enable'];
    ...
    {
        yield 'contena-context' => ['contena-context'];
    }
    ...
    {
        yield 'contena-entity-list' => ['contena-entity-list'];
        yield 'contena-channels' => ['contena-channels'];
        yield 'contena-languages' => ['contena-languages'];
        yield 'contena-state-machines' => ['contena-state-machines'];
        yield 'contena-business-events' => ['contena-business-events'];
        yield 'contena-flow-actions' => ['contena-flow-actions'];
        yield 'contena-extensions' => ['contena-extensions'];
    }
