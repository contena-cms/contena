 ...
 *
 * Contena's `mcp.yaml` intentionally omits `session.store` so the bundle's default (`file` →
...
class McpSessionPersistenceTest extends TestCase
{
    ...
    public static function serverProvider(): iterable
    {
        ...
        yield 'store api' => ['mcp.server.channel_api.session.store'];
    }
     ...
     * Admin API endpoint valid on the Channel API one — across a firewall boundary.
     */
    public function testEachServerHasItsOwnSessionStore(): void
    {
        ...
        static::assertNotSame(
            ...
            $container->get('mcp.server.channel_api.session.store'),
        );
    }
