...
namespace Contena\Tests\Integration\Core\Framework\ContentSystem\Channel;
 ...
 *
 * The schema is taken from the assembled Channel API document rather than from a fragment file on disk: the
 ...
 * footer carry no path at all and resolve by channel through `DomainAwareLayoutResolver`, so their
 * layouts are bound by a `header_content_layout` / `footer_content_layout` assignment scoped to the browser's
 * own channel — the resolver's second tier, domain null and channel set. Nothing about that resolution is
...
class ContentRouteResponseSchemaConformanceTest extends TestCase
{
    ...
    /**
     * The id the assembled Channel API document's `components` node is registered under, so a `#/components/...`
     * pointer taken off a route entry resolves against it verbatim.
     */
    private const SCHEMA_DOCUMENT_URI = 'https://schema.contena.test/channel-api.json';
    ...
    protected function setUp(): void
    {
        ...
        $this->browser = $this->createChannelBrowser();
    }
    ...
    private function declaredResponseSchemaRef(string $schemaPath): string
    {
        ...
        static::assertIsString($ref, \sprintf('The Channel API document declares no JSON 200 response schema for "%s".', $schemaPath));
    ...
    private function storeApiDocument(): array
    {
        ...
        $generator = $this->getContainer()->get(ChannelApiGenerator::class);

        return $this->storeApiDocument = $generator->generate(
            $this->getContainer()->get(ChannelDefinitionInstanceRegistry::class)->getDefinitions(),
    ...
    private function requestUri(string $schemaPath): string
    {
        if (!str_contains($schemaPath, '{path}')) {
            return '/channel-api' . $schemaPath;
        }

        return '/channel-api' . str_replace('{path}', 'category/' . $this->ids->get('category'), $schemaPath);
    }
     ...
     * assignment is scoped to the browser's own channel with a null domain — the middle tier of the
    ...
    private function assignmentPayload(string $section): array
    {
        ...
        return [
            'id' => $this->ids->get('assignment'),
            'domainId' => null,
            'channelId' => $this->getChannelApiChannelId(),
            'contentLayoutId' => $this->ids->get('layout'),
        ];
    }
