...
class ContentRouteRenderingTest extends TestCase
{
    ...
    public function testFullFormatBodyAndInProcessPageDescribeTheSameForest(): void
    {
        ...
        // The public alias resolves to the main section's full-format route — the same service the Frontend
        // consumes in process.
    ...
    public function testSeoUrlEnrichmentReachesAnEntityOnARenderedProperty(): void
    {
        ...
        // Fixture guard: a canonical seo url for this category really exists, so `ChannelApiSeoResolver::enrich()`
        ...
        // `RenderedElement` is not a `Struct`, so `getVars()` cannot walk it; `ChannelApiSeoResolver` reaches
    ...
    public function testFullFormatFiltersProtectedEntityFields(): void
    {
        ...
        static::assertArrayNotHasKey('blogMedia', $media);
    ...
    {
        return '/channel-api/' . $format . '/category/' . $this->ids->get('category');
    }
    ...
    /**
     * The canonical seo-url ids `ChannelApiSeoResolver::enrich()` would find for the fixture category, read on
     ...
     * reproduced here: the fixture's seo-url row and `createChannelBrowser()` both use the system language.
     *
     * @return list<string>
     */
