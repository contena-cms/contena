...
/**
 * Pins the write-to-serve round trip of a translatable property: a `Ct:Content:Text.text` language map is
 * persisted through the DAL and served through the real `ContentRoute`, once per requested language.
 *
 * Three languages back the fallback cases, because the language chain a `ChannelContext` carries is
 ...
 *
 * All three are members of the browser's channel. A language outside `channel_language` is refused
...
class ContentRouteTranslationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // The languages must exist before the channel that lists them, so the browser is built here
        // rather than by the trait's lazy accessor.
        $this->browser = $this->createChannelBrowser(channelOverrides: [
            'languages' => [
    ...
    /**
     * One `Ct:Content:Text` root holding the given language map, bound to a category the request addresses by
     ...
     * language id sends no header, which is the channel's own language — the system language here.
    ...
    private function servedProperties(?string $languageId): array
    {
        ...
        $this->browser->request('GET', '/channel-api/content/category/' . $this->ids->get('category'));
