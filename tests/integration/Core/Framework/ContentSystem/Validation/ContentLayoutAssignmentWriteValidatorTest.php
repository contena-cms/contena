...
class ContentLayoutAssignmentWriteValidatorTest extends TestCase
{
    ...
    public function testRejectsAssignmentMismatchingRootSource(): void
    {
        $context = Context::createDefaultContext();
        $categoryId = $this->createCategory($context);
        $layoutId = $this->createLayout('blog', $context);

        $assignmentId = $this->ids->get('assignment');

        try {
            $this->assignmentRepository()->create([$this->assignment($assignmentId, $categoryId, $layoutId)], $context);
            static::fail('Expected the type-match to reject the assignment of a blog-rooted layout to a category.');
        } catch (WriteException $exception) {
            static::assertStringContainsString('root source is "blog"', $exception->getMessage());
    ...
    public function testRejectsAtomicCreateAndBindMismatchingRootSource(): void
    {
        ...
        try {
            $this->layoutRepository()->create([$this->layoutWithCategoryBinding($layoutId, $assignmentId, $categoryId, 'blog')], $context);
    ...
    public function testRejectsAtomicCreateAndBindMismatchViaSyncApi(): void
    {
        ...
        $operations = [
            new SyncOperation(
                ...
                [$this->layoutWithCategoryBinding($layoutId, $assignmentId, $categoryId, 'blog')],
            ),
        ];

        try {
            $this->syncService()->sync($operations, $context, new SyncBehavior());
            static::fail('Expected the type-match to reject the atomic Sync create-and-bind of a blog-rooted layout to a category.');
        } catch (WriteException $exception) {
            static::assertStringContainsString('root source is "blog"', $exception->getMessage());
        }
