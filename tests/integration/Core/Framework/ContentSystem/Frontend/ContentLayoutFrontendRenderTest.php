...
/**
 * Pins that a persisted `content_layout` renders to Frontend markup through the real
 * `frontend.content.layout` route, and that a media-backed element's media URL survives the
 * `|ct_encode_media_url` filter chain into the emitted `img` tag.
 *
...
class ContentLayoutFrontendRenderTest extends TestCase
{
    ...
    private const MEDIA_PATH = 'media/frontend-render-probe.png';
    ...
    #[TestDox('renders every layout element with its data-element-id into the frontend markup')]
    ...
    public function testDanglingMediaIdRendersThePlaceholderMarkup(): void
    {
        ...
        $this->createTestCategory($categoryId, 'Frontend render dangling media category');
        ...
        $images = $xpath->query('//img[contains(concat(" ", normalize-space(@class), " "), " ct-media-image ")]');
        ...
        $placeholders = $xpath->query('//div[contains(concat(" ", normalize-space(@class), " "), " ct-media-image__placeholder ")]');
    ...
    {
        $this->persistContentLayout($this->ids->get('layout'), 'frontend-render', '1.0.0', 'category', [[
    ...
    private function persistDanglingMediaLayout(string $categoryId): void
    {
        ...
        $this->persistContentLayout($layoutId, 'frontend-render-dangling-media', '1.0.0', 'category', [[
    ...
    {
        $this->createTestCategory($this->ids->create('category'), 'Frontend render category');
    }
