...
{
    /**
     * Number of platform snippet files downloaded per locale (Administration, Core, Frontend).
     */
    private const PLATFORM_FILE_COUNT = 3;

    /**
     * Number of snippet files downloaded per configured plugin per locale (Administration, Frontend).
     */
    private const PLUGIN_FILE_COUNT = 2;

    public function getTranslationRequestHandler(): MockHandler
    {
        $handler = static::getContainer()->get('contena.translation.mock_handler');
