...
class McpServerRegistryConfigTest extends TestCase
{
     ...
     * Channel API tools, and vice versa. The bundle assigns a capability to every server whose patterns
    ...
    public function testServerRegistriesDoNotOverlap(): void
    {
        ...
        static::assertArrayHasKey('channel_api', $registries);

        foreach ($this->patterns($registries['admin']) as $adminPattern) {
            foreach ($this->patterns($registries['channel_api']) as $channelApiPattern) {
                static::assertFalse(
                    str_starts_with($adminPattern, $channelApiPattern) || str_starts_with($channelApiPattern, $adminPattern),
                    \sprintf('The MCP server patterns "%s" and "%s" overlap, so a capability would land on both endpoints.', $adminPattern, $channelApiPattern),
                );
            }
        }
    }
    ...
    {
        yield 'core tools' => ['Contena\\Core\\Framework\\Mcp\\Tool\\EntitySearchTool'];
        yield 'core prompts' => ['Contena\\Core\\Framework\\Mcp\\Prompt\\ContenaContextPrompt'];
        yield 'core resources' => ['Contena\\Core\\Framework\\Mcp\\Resource\\EntityListResource'];
        yield 'frontend tools' => ['Contena\\Frontend\\Mcp\\Tool\\ThemeConfigTool'];
        yield 'store api tools' => ['Contena\\Core\\System\\Channel\\Mcp\\Tool\\ChannelApiContextTool'];
    }
    ...
    private function loadRegistries(): array
    {
        ...
        $container->setParameter('kernel.bundles', ['Framework' => Framework::class, 'Frontend' => Frontend::class]);
