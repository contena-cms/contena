...
class ThemeConfigToolTest extends TestCase
{
    public function testDeclaresExplicitThemeGroup(): void
    {
        ...
        // prefix (McpToolAnalysisCompilerPass), which for "contena-theme-config" would
        // produce the accidental "contena" toolset. Guard the intended "theme" group.
    ...
    public function testGetReturnsThemeConfig(): void
    {
        ...
        $expectedConfig = ['ct-color-brand-primary' => ['value' => '#ff0000']];
    ...
    public function testUpdateDryRunReturnsPreview(): void
    {
        ...
        $config = json_encode(['ct-color-brand-primary' => ['value' => '#0000ff']], \JSON_THROW_ON_ERROR);
        $output = $tool($channelId, 'update', $config, true);
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);
    ...
    public function testUpdateCommitCallsThemeService(): void
    {
        $themeId = Uuid::randomHex();
        $channelId = Uuid::randomHex();
        $configValues = ['ct-color-brand-primary' => ['value' => '#0000ff']];
        ...
        $output = $tool($channelId, 'update', $config, false);
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);

        static::assertTrue($data['success']);
        static::assertFalse($data['_meta']['dryRun']);
        static::assertSame(['ct-color-brand-primary'], $data['data']['updatedKeys']);
    }

    public function testResolvesChannelByName(): void
    {
        $themeId = Uuid::randomHex();
        $channelId = Uuid::randomHex();
        $expectedConfig = ['ct-color-brand-primary' => ['value' => '#ff0000']];
    ...
    public function testUnknownChannelNameListsAvailableNames(): void
    {
        $contextProvider = static::createStub(McpContextProvider::class);
        $contextProvider->method('getContext')->willReturn($this->createAdminContext(['theme:read']));
    ...
    public function testUppercaseUuidIsAccepted(): void
    {
        ...
        $output = $tool(strtoupper($channelId), 'get');
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);
        ...
        static::assertSame(Uuid::fromHexToBytes($channelId), $lookupParams['id']);
    }

    public function testUnknownChannelUuidReturnsError(): void
    {
        $contextProvider = static::createStub(McpContextProvider::class);
        $contextProvider->method('getContext')->willReturn($this->createAdminContext(['theme:read']));
    ...
    public function testErrorReportsNoneWhenSystemHasNoChannels(): void
    {
        $contextProvider = static::createStub(McpContextProvider::class);
        $contextProvider->method('getContext')->willReturn($this->createAdminContext(['theme:read']));
        ...
        static::assertFalse($data['success']);
        static::assertStringContainsString('Available channels: none.', $data['error']);
    }
    ...
    public function testNoThemeReturnsError(): void
    {
        ...
        $tool = new ThemeConfigTool(
            static::createStub(ThemeService::class),
            $contextProvider,
            $this->createConnection(false, [$channelId]),
        );
    ...
    public function testUpdateWithEmptyConfigReturnsError(): void
    {
        ...
        $output = $tool($channelId, 'update', '{}', false);
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);
    ...
    public function testUpdateExceptionReturnsError(): void
    {
        ...
        $config = json_encode(['ct-color-brand-primary' => ['value' => '#ff0000']], \JSON_THROW_ON_ERROR);
        $output = $tool($channelId, 'update', $config, false);
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);
    ...
    public function testMalformedConfigJsonReturnsError(): void
    {
        ...
        $output = $tool($channelId, 'update', 'not-json', false);
        $data = json_decode($output, true, 512, \JSON_THROW_ON_ERROR);
    ...
    public function testEmptyChannelIdReturnsError(): void
    {
        $tool = new ThemeConfigTool(
            static::createStub(ThemeService::class),
            static::createStub(McpContextProvider::class),
        ...
        static::assertFalse($data['success']);
        static::assertStringContainsString('channelId is required', $data['error']);
    }
    ...
    /**
     * The tool issues three queries: the channel lookup, the name hint list and the theme
     ...
     *
     * @param list<string> $resolvedIds channel IDs the lookup resolves to
