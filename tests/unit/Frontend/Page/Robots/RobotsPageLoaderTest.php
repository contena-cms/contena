...
use Contena\Frontend\Page\Robots\RobotsPage;
use Contena\Frontend\Page\Robots\RobotsPageLoadedEvent;
...
class RobotsPageLoaderTest extends TestCase
{
    ...
    public function testLoadWithValidHostname(): void
    {
        ...
        $context = Context::createDefaultContext();
        $channelId = 'test-sales-channel-id';

        $domain = $this->createExampleComDomain($channelId);
        $domains = [$domain];
    ...
    public function testLoadWithMultipleDomains(): void
    {
        $request = new Request(server: ['HTTP_HOST' => 'example.com']);
        $context = Context::createDefaultContext();
        $channelId1 = 'test-sales-channel-id-1';
        $channelId2 = 'test-sales-channel-id-2';

        $domains = $this->createStandardDomains($channelId1, $channelId2);
    ...
    public function testLoadWithHttpAndHttpsDomains(): void
    {
        ...
        $httpDomain = $this->createExampleComHttpDomain($channelId);
        $httpsDomain = $this->createExampleComDomain($channelId);
    ...
    public function testLoadWithDifferentHostnames(): void
    {
        ...
        $domain1 = $this->createExampleComDomain($channelId1);

        // Domain for different.org (different hostname)
        $domain2 = $this->createDifferentOrgDomain($channelId2);
     ...
     */
    private function createExampleComDomain(string $channelId = 'test-sales-channel-id'): ChannelDomainEntity
    {
     ...
     */
    private function createExampleComEnDomain(string $channelId = 'test-sales-channel-id'): ChannelDomainEntity
    {
        $domain = new ChannelDomainEntity();
     ...
     */
    private function createExampleComHttpDomain(string $channelId = 'test-sales-channel-id'): ChannelDomainEntity
    {
        $domain = new ChannelDomainEntity();
     ...
     */
    private function createDifferentOrgDomain(string $channelId = 'test-sales-channel-id'): ChannelDomainEntity
    {
        $domain = new ChannelDomainEntity();
     ...
     */
    private function createStandardDomains(string $channelId1 = 'test-sales-channel-id-1', string $channelId2 = 'test-sales-channel-id-2'): array
    {
        return [
            $this->createExampleComDomain($channelId1),
            $this->createExampleComEnDomain($channelId2),
        ];
    }
     ...
     *
     * @param ChannelDomainEntity[] $domains
    ...
    {
        $this->channelDomainRepository = new StaticEntityRepository([
            new ChannelDomainCollection($domains),
        ]);
        ...
        foreach ($config as $key => $value) {
            if (\is_array($value)) {
                ...
                foreach ($value as $index => $configValue) {
                    if (isset($domains[$index]) && $domains[$index] instanceof ChannelDomainEntity) {
                        $this->systemConfigService->set($key, $configValue, $domains[$index]->getChannelId());
                    }
                }
            } else {
