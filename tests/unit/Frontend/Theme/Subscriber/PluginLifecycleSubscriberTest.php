...
class PluginLifecycleSubscriberTest extends TestCase
{
    ...
    public function testPluginPostUpdateRefreshesImportMaps(): void
    {
        ...
        $configurations = new FrontendPluginConfigurationCollection([new FrontendPluginConfiguration('TestPlugin')]);
    ...
    public function testPluginPreDeactivateDeactivatesThemeWithAdditionalBundles(): void
    {
        ...
        $config = new FrontendPluginConfiguration('ComponentTestTheme');
        $config->setAdditionalBundles(true);

        $configurations = new FrontendPluginConfigurationCollection([$config]);
        $registry = static::createStub(FrontendPluginRegistry::class);
        $registry->method('getConfigurations')->willReturn($configurations);
    ...
    public function testPluginPreDeactivateHandlesThemeUninstallWithoutAdditionalBundles(): void
    {
        ...
        $config = new FrontendPluginConfiguration('SimpleThemePlugin');
    ...
    public function testPluginPreUninstallRefreshesImportMaps(): void
    {
        ...
        $configurations = new FrontendPluginConfigurationCollection();
        $registry = static::createStub(FrontendPluginRegistry::class);
        $registry->method('getConfigurations')->willReturn($configurations);
    ...
    public function testPluginPreUninstallDeactivatesThemeForAdditionalBundles(): void
    {
        ...
        $themeLifecycleHandler->expects($this->once())
            ...
            ->with(
                ...
                static::callback(static function (FrontendPluginConfigurationCollection $collection): bool {
    ...
    public function testPluginPostUninstallRemovesThemeWhenUserDataNotKept(): void
    {
        ...
        $themeRepository->expects($this->once())
            ->method('search')
            ->with(static::isInstanceOf(Criteria::class), $context)
            ->willReturn(new EntitySearchResult(0, new ThemeCollection(), null, new Criteria(), $context));

        $themeLifecycleService = $this->createThemeLifecycleService($themeRepository);

        $subscriber = new PluginLifecycleSubscriber(
            static::createStub(FrontendPluginRegistry::class),
            '/var/www/html',
            static::createStub(AbstractFrontendPluginConfigurationFactory::class),
    ...
    {
        $themeRepository = $this->createMock(EntityRepository::class);
        $themeRepository->expects($this->never())->method('search');
        $themeLifecycleService = $this->createThemeLifecycleService($themeRepository);
    ...
    public function testPluginPostActivateInstallsThemeAndRefreshesImportMaps(): void
    {
        ...
        $existingConfig = new FrontendPluginConfiguration('ExistingPlugin');
        $configurations = new FrontendPluginConfigurationCollection([$existingConfig]);

        $registry = static::createStub(FrontendPluginRegistry::class);
        $registry->method('getConfigurations')->willReturn($configurations);

        $newConfig = new FrontendPluginConfiguration('AsyncPlugin');
        $factory = $this->createMock(AbstractFrontendPluginConfigurationFactory::class);
        ...
        $themeLifecycleHandler->expects($this->once())
            ...
            ->with(
                $context,
                static::callback(static fn (FrontendPluginConfigurationCollection $collection): bool => $collection->getByTechnicalName('AsyncPlugin') !== null)
            );
    ...
    public function testPluginPostActivateThrowsForInvalidPluginClass(): void
    {
        ...
        $this->expectExceptionObject(ThemeException::invalidPluginClass(InvalidPluginForLifecycleSubscriber::class));
        $subscriber->pluginPostActivate($event);
    }

    private function createSubscriber(
        FrontendPluginRegistry $registry,
        ThemeLifecycleHandler $themeLifecycleHandler,
        ?AbstractFrontendPluginConfigurationFactory $factory = null,
        ?ThemeLifecycleService $themeLifecycleService = null,
    ): PluginLifecycleSubscriber {
        return new PluginLifecycleSubscriber(
            $registry,
            '/var/www/html',
            $factory ?? static::createStub(AbstractFrontendPluginConfigurationFactory::class),
            $themeLifecycleHandler,
            $themeLifecycleService ?? $this->createThemeLifecycleService(),
        );
    }
     ...
     */
    private function createThemeLifecycleService(?EntityRepository $themeRepository = null): ThemeLifecycleService
    {
        $reflection = new \ReflectionClass(ThemeLifecycleService::class);
        $service = $reflection->newInstanceWithoutConstructor();

        if ($themeRepository !== null) {
            $reflection->getProperty('themeRepository')->setValue($service, $themeRepository);
        }
