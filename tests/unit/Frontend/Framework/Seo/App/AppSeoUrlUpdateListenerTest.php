...
{
    private const BLOG_ROUTE = 'frontend.app.SwagSeoUrlApp.blog-detail';
    ...
    {
        $listener = $this->createListener([$this->route('blog'), $this->route('ce_blog')]);

        $listener->updateAppSeoUrls($this->writtenEvent([
            'blog' => [$this->writeResult('blog', 'blog-1'), $this->writeResult('blog', 'blog-2')],
            'ce_blog' => [$this->writeResult('ce_blog', 'blog-1')],
        ]));

        static::assertSame([
            ['routeName' => self::BLOG_ROUTE, 'ids' => ['blog-1', 'blog-2']],
    ...
    {
        $listener = $this->createListener([$this->route('blog')]);

        $listener->updateAppSeoUrls($this->writtenEvent([
            'blog' => [$this->writeResult('blog', 'blog-1')],
            'blog_translation' => [
                $this->writeResult('blog_translation', ['blogId' => 'blog-1', 'languageId' => 'language-1']),
                $this->writeResult('blog_translation', ['blogId' => 'blog-2', 'languageId' => 'language-1']),
            ],
        ]));

        static::assertSame([['routeName' => self::BLOG_ROUTE, 'ids' => ['blog-1', 'blog-2']]], $this->updates);
    }
    ...
    public function testWrittenChannelDomainsResynchroniseTheStaticUrls(): void
    {
