...
{
    public function testSubscribesToTheFrontendScopedResponseEvent(): void
    {
        static::assertSame(
            ['frontend.scope.response' => 'onResponse'],
            ClearSiteDataListener::getSubscribedEvents()
        );
    }

    public function testHeaderIsSentForAnOptedInFrontendResponse(): void
    {
        $response = $this->dispatch(['cookies', 'storage'], $this->createFrontendLogoutRequest());
    ...
    {
        $response = $this->dispatch([], $this->createFrontendLogoutRequest());
    ...
    {
        $request = $this->createFrontendLogoutRequest();
    ...
    {
        $response = $this->dispatch(['storage'], $this->createFrontendLogoutRequest(), HttpKernelInterface::SUB_REQUEST);
    ...
    private function createFrontendLogoutRequest(): Request
    {
        $request = new Request();
