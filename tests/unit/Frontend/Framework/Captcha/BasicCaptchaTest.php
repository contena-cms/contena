...
class BasicCaptchaTest extends TestCase
{
    ...
    #[TestDox('passes the channel ID from the request context to the config lookup')]
    public function testSupportsUsesContextChannelId(): void
    {
        ...
        $systemConfigService->expects($this->once())
            ->method('get')
            ->with('core.basicInformation.activeCaptchasV2', $channelId)
            ->willReturn($active);

        $context = static::createStub(ChannelContext::class);
        $context->method('getChannelId')->willReturn($channelId);
