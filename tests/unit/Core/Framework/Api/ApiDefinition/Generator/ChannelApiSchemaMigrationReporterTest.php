...
use Contena\Core\Framework\Test\DataAbstractionLayer\Search\Definition\GroupByTestDefinition;
 ...
 * @internal
 */
#[CoversClass(ChannelApiSchemaMigrationReporter::class)]
class ChannelApiSchemaMigrationReporterTest extends TestCase
{
    ...
    public function testReportGroupsChannelApiSchemaMigrationState(): void
    {
    ...
    public function testReportTracksLegacyJsonApiSchemaForPhpOwnedDefinition(): void
    {
        $report = $this->createReporter(
            ...
            $this->createDefinitions([ChannelSimpleDefinition::class]),
    ...
    public function testCoreScopeIgnoresExtensionDefinitionsAndSchemaFiles(): void
    {
        ...
        $coreReport = $reporter->report($definitions, CoreChannelApiSchemaMigrationScopeProvider::SCOPE);
        ...
        $allReport = $reporter->report($definitions, AllChannelApiSchemaMigrationScopeProvider::SCOPE);
    ...
    {
        $this->expectExceptionObject(ApiException::unsupportedChannelApiSchemaMigrationScope('extension', [
            CoreChannelApiSchemaMigrationScopeProvider::SCOPE,
            AllChannelApiSchemaMigrationScopeProvider::SCOPE,
        ]));
    ...
    public static function invalidAllowlistProvider(): iterable
    {
        ...
        yield 'missing list' => [
            '{}',
            'The "phpGeneratedChannelApiSchemas" list is missing.',
        ];

        yield 'non-string schema name' => [
            '{"phpGeneratedChannelApiSchemas":[1]}',
            'The "phpGeneratedChannelApiSchemas" list must contain only schema names.',
        ];
    }
    ...
    ): ChannelApiSchemaMigrationReporter {
        return new ChannelApiSchemaMigrationReporter(
            $definitionSchemaBuilder ?? new OpenApiDefinitionSchemaBuilder(),
            [
                new CoreChannelApiSchemaMigrationScopeProvider($schemaPath, $allowlistPath),
                new AllChannelApiSchemaMigrationScopeProvider($bundleSchemaPathCollection ?? new BundleSchemaPathCollection([]), $schemaPath, $allowlistPath),
            ],
    ...
    /**
     * @param list<string> $phpGeneratedChannelApiSchemas
     */
    private function createAllowlistPath(array $phpGeneratedChannelApiSchemas = []): string
    {
        ...
        $this->filesystem->dumpFile($allowlistPath, json_encode([
            'phpGeneratedChannelApiSchemas' => $phpGeneratedChannelApiSchemas,
        ], \JSON_THROW_ON_ERROR));
