...
use Contena\Tests\Unit\Core\Framework\Api\ApiDefinition\Generator\_fixtures\DefinitionWithJsonOverride;
use Contena\Tests\Unit\Core\Framework\Api\ApiDefinition\Generator\_fixtures\PluginBundleWithSchema\PluginBundleWithSchema;
 ...
 * @internal
 */
#[CoversClass(ChannelApiGenerator::class)]
class ChannelApiGeneratorTest extends TestCase
{
    private const BASE_DESCRIPTION = 'Base description.';

    /**
     * Associations of {@see DefinitionWithAssociations} that are documented for the Channel API:
    ...
    private ChannelApiGenerator $generator;

    private ChannelApiGenerator $customApiGenerator;

    private ChannelApiGenerator $pluginApiGenerator;

    private ChannelApiGenerator $associationEnrichmentGenerator;
    ...
    {
        $this->generator = new ChannelApiGenerator(
        ...
        $this->customApiGenerator = new ChannelApiGenerator(
            new OpenApiSchemaBuilder('0.1.0'),
            new OpenApiDefinitionSchemaBuilder(),
            [
                'Framework' => ['path' => __DIR__ . '/_fixtures'],
        ...
        $this->pluginApiGenerator = new ChannelApiGenerator(
            new OpenApiSchemaBuilder('0.1.0'),
            new OpenApiDefinitionSchemaBuilder(),
            [
                'Framework' => ['path' => __DIR__ . '/_fixtures'],
        ...
        $this->associationEnrichmentGenerator = new ChannelApiGenerator(
            new OpenApiSchemaBuilder('0.1.0'),
            new OpenApiDefinitionSchemaBuilder(),
            [
                'Framework' => ['path' => __DIR__ . '/_fixtures'],
    ...
    public function testChannelApiLoadsJsonApiBaseSchemasFromJson(): void
    {
        $schema = $this->generator->generate(
            $this->definitionRegistry->getDefinitions(),
            DefinitionService::CHANNEL_API,
    ...
    public function testUnreferencedPhpGeneratedChannelApiSchemaIsNotEmitted(): void
    {
        $definitionRegistry = new StaticDefinitionInstanceRegistry(
            [ChannelSimpleDefinition::class],
            static::createStub(ValidatorInterface::class),
            static::createStub(EntityWriteGatewayInterface::class)
        );
        $schema = $this->generator->generate(
    ...
    public function testTransitivelyReferencedPhpGeneratedChannelApiSchemaIsEmitted(): void
    {
    ...
    public function testSchemaContainsCustomPathsOnly(): void
    {
        ...
        static::assertArrayHasKey('post', $paths['/search/content-presentation']);
        static::assertArrayNotHasKey('/category', $paths);
    }
        ...
        foreach ($schema['paths'] as $path => $pathDefinition) {
            foreach (['get', 'post', 'put', 'patch', 'delete'] as $method) {
                ...
                if ($shouldBeInjected) {
                    ...
                    static::assertTrue(
                        $hasHeader,
                        \sprintf('%s %s (%s) should advertise ct-language-id', strtoupper($method), $path, $operationId)
                    );
                } else {
                    $assertedSkippedOperation = true;
                    static::assertFalse(
                        $hasHeader,
                        \sprintf('%s %s (%s) must not advertise ct-language-id', strtoupper($method), $path, $operationId)
                    );
                }
            }
        }
        ...
        foreach ($schema['paths'] as $path => $pathDefinition) {
            foreach (['get', 'post', 'put', 'patch', 'delete'] as $method) {
                ...
                if ($shouldBeInjected) {
                    ...
                    static::assertTrue(
                        $hasHeader,
                        \sprintf('%s %s (%s) should advertise ct-domain', strtoupper($method), $path, $operationId)
                    );
                } else {
                    $assertedSkippedOperation = true;
                    static::assertFalse(
                        $hasHeader,
                        \sprintf('%s %s (%s) must not advertise ct-domain', strtoupper($method), $path, $operationId)
                    );
                }
            }
        }
    ...
    {
        $this->expectExceptionObject(ApiException::unsupportedChannelApiSchemaEndpoint());
    ...
    {
        $schema = ['paths' => $this->generateAssociationEnrichmentSchema()];
    ...
    public static function supportsDataProvider(): \Generator
    {
        yield 'correct format and API' => [
            'format' => ChannelApiGenerator::FORMAT,
            'api' => DefinitionService::CHANNEL_API,
            'expected' => true,
        ];
    ...
    public function testSwLanguageIdHeaderIsInjectedOrKeptWithoutDuplicationPerOperation(): void
    {
        ...
        $inlineNames = array_filter(
            $inline['parameters'],
            static fn (array $param): bool => ($param['name'] ?? null) === 'ct-language-id'
        );
        ...
        $mixedCaseNames = array_filter(
            ...
            static fn (array $param): bool => isset($param['name']) && strtolower((string) $param['name']) === 'ct-language-id'
        );
    ...
    /**
     * Generates the Channel API schema for the bundle that contributes one path per response shape
     ...
     */
    private function generateSchema(ChannelApiGenerator $generator, ?string $bundleName): array
    {
