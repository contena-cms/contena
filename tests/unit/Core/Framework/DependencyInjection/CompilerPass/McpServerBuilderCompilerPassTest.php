...
class McpServerBuilderCompilerPassTest extends TestCase
{
    public function testRequestHandlersAreScopedPerServer(): void
    {
        ...
        static::assertSame('mcp.channel_api.request_handler', $this->taggedIteratorTag($container, 'channel_api', 'addRequestHandlers'));
    }
    ...
    public function testNotificationHandlersAreScopedPerServer(): void
    {
        ...
        static::assertSame('mcp.channel_api.notification_handler', $this->taggedIteratorTag($container, 'channel_api', 'addNotificationHandlers'));
    }
    ...
    public function testCapabilityLoadersStayOnTheAdminServer(): void
    {
        ...
        static::assertSame([], $this->calls($container, 'channel_api', 'addLoaders'));
    }

    public function testBothServersPageWithTheContenaParameter(): void
    {
        $container = $this->createContainer();

        new McpServerBuilderCompilerPass()->process($container);

        foreach (['admin', 'channel_api'] as $server) {
            static::assertSame(
                [['%contena.mcp.pagination_limit%']],
    ...
    public function testUnrelatedCallsKeepTheirArgumentsAndOrder(): void
    {
        ...
        static::assertSame([['Contena', '1.0.0']], $this->calls($container, 'admin', 'setServerInfo'));
    }
    ...
    public function testSkipsAServerThatIsNotRegistered(): void
    {
        ...
        static::assertFalse($container->hasDefinition('mcp.server.channel_api.builder'));
    }
    ...
    private function createContainer(): ContainerBuilder
    {
        ...
        foreach (['admin' => 'Contena', 'channel_api' => 'Contena Channel API'] as $server => $name) {
