...
class OperationResolverTest extends TestCase
{
    ...
    public static function routeProvider(): \Generator
    {
        ...
        yield 'list suffix is a read' => ['api.blog.list', 'read'];
        yield 'detail suffix is a read' => ['api.blog.detail', 'read'];
        yield 'search suffix is a read' => ['api.blog.search', 'read'];
        yield 'search-ids suffix is a read' => ['api.blog.search-ids', 'read'];
        yield 'aggregate suffix is a read' => ['api.blog.aggregate', 'read'];

        yield 'create suffix is a write' => ['api.blog.create', 'write'];
        yield 'update suffix is a write' => ['api.blog.update', 'write'];
        yield 'delete suffix is a delete' => ['api.blog.delete', 'delete'];

        yield 'unknown CRUD suffix is none' => ['api.blog.unknown', 'none'];

        yield 'action API is none' => ['api.action.cache.index', 'none'];
        yield 'non-api route is none' => ['frontend.detail.page', 'none'];
        yield 'channel-api route is none' => ['channel-api.blog.search', 'none'];
    }
}
