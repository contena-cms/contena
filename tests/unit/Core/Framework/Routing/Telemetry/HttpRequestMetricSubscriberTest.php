...
class HttpRequestMetricSubscriberTest extends TestCase
{
    ...
    public function testResolvesLabelsFromRoutedRequestCapturedOnResponse(): void
    {
        ...
        $routed->attributes->set('_route', 'frontend.blog.page');
        $routed->attributes->set(PlatformRequest::ATTRIBUTE_ROUTE_SCOPE, ['frontend']);
        ...
        static::assertSame('frontend', $this->getMetric('http.server.request.duration')->labels['area']);
    }
    ...
    public function testIgnoresEsiFragmentsAndSubRequests(): void
    {
        ...
        $page = Request::create('/');
        $page->attributes->set('_route', 'frontend.blog.page');
        $page->attributes->set(PlatformRequest::ATTRIBUTE_ROUTE_SCOPE, ['frontend']);
        ...
        $fragment->attributes->set('_route', 'frontend.content.layout');
        $fragment->attributes->set(PlatformRequest::ATTRIBUTE_ROUTE_SCOPE, ['frontend']);
        ...
        $sub = Request::create('/');
        $sub->attributes->set(PlatformRequest::ATTRIBUTE_ROUTE_SCOPE, ['channel-api']);
    ...
    public function testEmitsMetricsWithSharedLabels(): void
    {
        ...
        $this->createSubscriber($counter)
            ->onKernelTerminate($this->createTerminateEvent('channel-api.blog.search', ['channel-api'], 200, microtime(true)));

        static::assertCount(3, $this->emitted);

        $sharedLabels = ['area' => 'channel-api', 'domain' => 'blog', 'operation' => 'none'];
    ...
    public function testQueryCountIsNotEmittedWhenMiddlewareIsNotWired(): void
    {
        ...
        $this->createSubscriber()
            ->onKernelTerminate($this->createTerminateEvent('frontend.blog.page', ['frontend'], 200, microtime(true)));
    ...
    {
        $this->createSubscriber()
            ->onKernelTerminate($this->createTerminateEvent('frontend.blog.page', ['frontend'], 404, microtime(true)));
    ...
    {
        $this->createSubscriber()
            ->onKernelTerminate($this->createTerminateEvent('frontend.blog.page', ['frontend'], 200, null));
