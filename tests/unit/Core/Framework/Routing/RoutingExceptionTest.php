...
class RoutingExceptionTest extends TestCase
{
    ...
    {
        $e = RoutingException::sessionContextNotResolvable('the request carries no frontend session cookie');

        static::assertSame(RoutingException::class, $e::class);
        static::assertSame(Response::HTTP_BAD_REQUEST, $e->getStatusCode());
        static::assertSame(RoutingException::SESSION_CONTEXT_NOT_RESOLVABLE, $e->getErrorCode());
        static::assertStringContainsString('the request carries no frontend session cookie', $e->getMessage());
    }
}
