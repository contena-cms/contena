...
class BindingSpecificationCanonicalizerTest extends TestCase
{
    ...
    {
        // "channel_blog" produces a subtype of the declared BlogEntity; only the exact "blog"
        // definition matches, so derivation stays unambiguous.
        $canonicalizer = $this->canonicalizer(
            ['image' => $this->typeSpec('image', ['item' => $this->reference(BlogEntity::class)])],
            $this->map(['entity' => [$this->capability(BlogEntity::class, ['entity' => 'blog'])]], ['entity' => $this->entitySpec()]),
            $this->definitions(['blog' => BlogEntity::class, 'channel_blog' => ChannelBlogEntity::class]),
        );

        $result = $canonicalizer->canonicalize($this->dto('image', ['item' => ['entity' => ['property' => 'blogId']]]), 'binding');

        static::assertIsArray($result->resolves);
        static::assertSame(
            ['loader' => 'entity', 'config' => ['property' => 'blogId', 'entity' => 'blog']],
    ...
    public function testDerivationUsesChannelProducedClass(): void
    {
        ...
        $channelDefinition->method('getEntityClass')->willReturn(ChannelBlogEntity::class);

        $channelRegistry = static::createStub(ChannelDefinitionInstanceRegistry::class);
        $channelRegistry->method('getChannelDefinitions')->willReturn(['blog' => $channelDefinition]);

        $canonicalizer = $this->canonicalizer(
            ['image' => $this->typeSpec('image', ['item' => $this->reference(ChannelBlogEntity::class)])],
            $this->map(['entity' => [$this->capability(ChannelBlogEntity::class, ['entity' => 'blog'])]], ['entity' => $this->entitySpec()]),
            $this->definitions(['blog' => BlogEntity::class]),
            $channelRegistry,
        );
    ...
    public function testTierACollectionDerivationUsesChannelCollectionClass(): void
    {
        ...
        $canonicalizer = $this->canonicalizer(
            ...
            $this->map([], []),
            ['blog' => $this->collectionEntityDefinition('blog', BlogCollection::class)],
            $channelRegistry,
        );
        ...
        static::assertIsArray($result->resolves);
        static::assertSame(
            ['loader' => 'entity_collection', 'config' => ['entity' => 'blog', 'property' => 'mediaIds']],
            $result->resolves['media'],
        );
    }
    ...
    public function testTierACollectionDerivationSkipsBareEntityCollection(): void
    {
        ...
        $channelRegistry = static::createStub(ChannelDefinitionInstanceRegistry::class);
        $channelRegistry->method('getChannelDefinitions')->willReturn(['media' => $channelDefinition]);
    ...
    private function canonicalizer(
        ...
        array $definitions = [],
        ?ChannelDefinitionInstanceRegistry $channelRegistry = null,
