...
use Contena\Core\Framework\ContentSystem\Layout\Element\Context\Distribution\SlicedDistributionConfig;
...
use Contena\Core\Framework\ContentSystem\Output\Index\LoaderValueIdentity;
...
use Contena\Core\Framework\ContentSystem\Output\Index\ValueOrigin;
...
use Contena\Core\Test\Stub\ContentSystem\StubExtractorEntity;
...
class ResolvedValueIndexFactoryTest extends TestCase
{
    ...
    /**
     * The blogion case for the loader-identity map: two elements loading the same entity get two distinct
    ...
    {
        $first = new StubExtractorEntity('blog-1');
        $second = new StubExtractorEntity('blog-1');
        ...
        $index = $this->factory()->create(
            ...
            [
                ...
                'element-2' => ['blog' => $this->loaderProvenance($second)],
            ]
        );

        $assignments = $index->assignments();
        static::assertSame($assignments['element-1']['blog'], $assignments['element-2']['blog']);
    ...
    {
        $first = new EntityCollection([new StubExtractorEntity('blog-1')]);
        $second = new EntityCollection([new StubExtractorEntity('blog-1')]);
        static::assertNotSame($first, $second);
        ...
        $index = $this->factory()->create(
            ...
            [
                ...
                'element-2' => ['blogs' => $this->loaderProvenance($second)],
            ]
        );

        $assignments = $index->assignments();
        static::assertSame($assignments['element-1']['blogs'], $assignments['element-2']['blogs']);
        static::assertSame([$first], array_values($index->data()));
    }
    ...
    public function testSameEntityUnderDifferentSourcesGetsDistinctRefs(): void
    {
        ...
        $index = $this->factory()->create(
            ...
            [
                ...
                'element-2' => ['blog' => $this->loaderProvenance($second, source: 'second_loader')],
            ]
        );

        $assignments = $index->assignments();
        static::assertNotSame($assignments['element-1']['blog'], $assignments['element-2']['blog']);
        static::assertSame($first, $index->value($assignments['element-1']['blog']));
        static::assertSame($second, $index->value($assignments['element-2']['blog']));
    }
    ...
    public function testSameEntityUnderDifferentConfigsGetsDistinctRefs(): void
    {
        ...
        $index = $this->factory()->create(
            ...
            [
                ...
                'element-2' => ['blog' => $this->loaderProvenance($second, configHash: 'config-b')],
            ]
        );
    ...
    public function testSameConfigWithDifferentInputsGetsDistinctRefs(): void
    {
        ...
        $index = $this->factory()->create(
            ...
            [
                ...
                'element-2' => ['blogs' => $this->loaderProvenance($second, inputsHash: 'inputs-b')],
            ]
        );

        $assignments = $index->assignments();
        static::assertNotSame($assignments['element-1']['blogs'], $assignments['element-2']['blogs']);
    ...
    {
        $id = 'blog-and-category-id';
        $blog = new BlogEntity();
        $blog->setUniqueIdentifier($id);
        $category = new CategoryEntity();
        $category->setUniqueIdentifier($id);
        static::assertNotSame($blog->getApiAlias(), $category->getApiAlias());
        ...
        $assignments = $index->assignments();
        $blogRef = $assignments['element-1']['subject'];
        $categoryRef = $assignments['element-2']['subject'];

        static::assertNotSame($blogRef, $categoryRef);
        static::assertSame($blog, $index->value($blogRef));
    ...
    public function testBroadcastDeliverySharesTheProviderRef(): void
    {
        ...
        $index = $this->factory()->create([$parent], [
            'parent' => ['blog' => $this->loaderProvenance($provided)],
            'child' => ['blog' => new ValueProvenance(ValueOrigin::DeliveredContext)],
        ]);

        $assignments = $index->assignments();
        static::assertSame($assignments['parent']['blog'], $assignments['child']['blog']);
    ...
    public function testLoaderNullsShareOneRef(): void
    {
        ...
        $assignments = $index->assignments();
        static::assertSame($assignments['element-1']['blog'], $assignments['element-2']['category']);
    ...
    public function testReplacedLoaderValueGetsItsOwnRef(): void
    {
        $index = $this->factory()->create(
            ...
            [
                'element-1' => ['blog' => $this->loaderProvenance('original')],
                'element-2' => [
                    'blog' => $this->loaderProvenance('original'),
                    'zulu' => new ValueProvenance(ValueOrigin::DeclaredAuthored),
                ],
            ]
        );
        ...
        $assignments = $index->assignments();
        static::assertNotSame($assignments['element-1']['blog'], $assignments['element-2']['blog']);
        static::assertSame('original', $index->value($assignments['element-1']['blog']));
        static::assertSame('rewritten', $index->value($assignments['element-2']['blog']));
        static::assertSame(['zulu', 'blog', 'note'], array_keys($assignments['element-2']));
    }
    ...
    public function testReplacedLoaderObjectValueGetsItsOwnRef(): void
    {
        ...
        $index = $this->factory()->create(
            ...
            [
                'element-1' => ['blog' => $this->loaderProvenance($produced)],
                'element-2' => ['blog' => $this->loaderProvenance($produced)],
            ]
        );

        $assignments = $index->assignments();
        static::assertNotSame($assignments['element-1']['blog'], $assignments['element-2']['blog']);
        static::assertSame($produced, $index->value($assignments['element-1']['blog']));
        static::assertSame($replacement, $index->value($assignments['element-2']['blog']));
    }
