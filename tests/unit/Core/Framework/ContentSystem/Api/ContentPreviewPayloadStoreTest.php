...
class ContentPreviewPayloadStoreTest extends TestCase
{
    ...
    public static function malformedEnvelopeProvider(): iterable
    {
        ...
        yield 'channelId empty' => [
            self::envelope(['channelId' => '']),
            self::constraintViolation('channelId'),
        ];

        yield 'channelId not a string' => [
            self::envelope(['channelId' => 42]),
            ContentSystemException::previewPayloadInvalid('channelId', 'string', 'int'),
        ];
        ...
        yield 'declared field omitted' => [
            array_diff_key(self::envelope([]), ['memberId' => null]),
            ContentSystemException::previewPayloadInvalid('memberId', 'present', 'absent'),
        ];
    }
