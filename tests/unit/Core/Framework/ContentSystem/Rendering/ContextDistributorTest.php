...
class ContextDistributorTest extends TestCase
{
    ...
    {
        $children = [$this->consumerOf('child-1', 'blog'), $this->consumerOf('child-2', 'blog')];
        ...
        static::assertSame(['blog' => 'blog-data'], $deliveries[0]->context);
        static::assertSame(['blog' => 'blog-data'], $deliveries[1]->context);
    }
    ...
    public function testPropertyAliasIsTheDeliveredKey(): void
    {
        ...
        $deliveries = $this->distributor()->distribute($parent, ['blog' => 'blog-data'], [$child]);

        static::assertSame(['myBlog' => 'blog-data'], $deliveries[0]->context);
    }
    ...
    public function testConsumerAliasSelectsChildrenIndependentlyOfTheProviderKey(): void
    {
        ...
        $parent = $this->providerOf('featuredBlog', BroadcastDistributionConfig::aliased('blog'));

        $deliveries = $this->distributor()->distribute($parent, ['featuredBlog' => 'blog-data'], $children);
    ...
    {
        $children = [$this->consumerOf('child-1', 'blog.cover')];
        $parent = $this->providerOf('blog', BroadcastDistributionConfig::simple());

        $deliveries = $this->distributor()->distribute(
            $parent,
            ['blog' => new StubContextStruct('cover-url')],
            $children
        );

        static::assertSame(['blog.cover' => 'cover-url'], $deliveries[0]->context);
    }
    ...
    public function testLastProviderWinsOnACollision(): void
    {
        ...
        static::assertSame(['blog' => 'second-data'], $deliveries[0]->context);
    }
    ...
    public function testOneValueFillsEveryMatchingKeyOnTheSameChild(): void
    {
        ...
        $child = StoredElementBuilder::create('Ct:Box', 'child-1')
            ...
            ->build();
        $parent = $this->providerOf('blog', IndexedDistributionConfig::simple());

        $deliveries = $this->distributor()->distribute($parent, ['blog' => [$first, $second]], [$child]);

        static::assertSame(['blog' => $first, 'blog.cover' => 'first-cover'], $deliveries[0]->context);
    }
    ...
    public function testDottedConsumerOccupiesItsOwnIndexedPosition(): void
    {
        $first = new StubContextStruct('first-cover');
        $second = new StubContextStruct('second-cover');
        $children = [$this->consumerOf('child-1', 'blog'), $this->consumerOf('child-2', 'blog.cover')];
        $parent = $this->providerOf('blog', IndexedDistributionConfig::simple());

        $deliveries = $this->distributor()->distribute($parent, ['blog' => [$first, $second]], $children);

        static::assertSame(['blog' => $first], $deliveries[0]->context);
        static::assertSame(['blog.cover' => 'second-cover'], $deliveries[1]->context);
    }
    ...
    public function testRootScopedConsumerKeyIsSkippedOnAChildThatMatchedThroughParentScope(): void
    {
        ...
        $deliveries = $this->distributor()->distribute(
            $parent,
            ['blog' => new StubContextStruct('parent-cover')],
            [$child]
        );

        static::assertSame(['blog'], array_keys($deliveries[0]->context));
    }
    ...
    {
        $children = [$this->keyedConsumerOf('child-1', 'blog', 'present')];
        $parent = $this->providerOf('blog', BroadcastDistributionConfig::simple());
    ...
    public function testNullProviderValueDeliversNothing(): void
    {
        ...
        $deliveries = $this->distributor()->distribute($parent, ['blog' => null], $children);
    ...
    public function testOptionalConsumerWithUnresolvablePathGetsAnExplicitNull(): void
    {
        ...
        $deliveries = $this->distributor()->distribute($parent, ['blog' => 'not-a-struct'], $children);

        static::assertSame(['blog.cover' => null], $deliveries[0]->context);
    }
    ...
    public function testParentWithoutChildrenYieldsNoDeliveries(): void
    {
        ...
        $deliveries = $this->distributor()->distribute($parent, ['blog' => 'blog-data'], []);
    ...
    public function testRequiredConsumerWithUnresolvablePathThrowsNamingTheElement(): void
    {
        ...
        $this->distributor()->distribute($parent, ['blog' => 'not-a-struct'], [$child]);
    }
