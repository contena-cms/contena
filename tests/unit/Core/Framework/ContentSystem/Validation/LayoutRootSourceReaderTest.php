...
class LayoutRootSourceReaderTest extends TestCase
{
    ...
    public function testReadsRootSourceFromInBatchCommand(): void
    {
        ...
        $command = $this->layoutCommand(InsertCommand::class, ContentLayoutDefinition::ENTITY_NAME, $layoutId, true, 'blog');

        static::assertSame('blog', $reader->read($layoutId, [$command], Context::createDefaultContext()));
    }
    ...
    public function testFallsBackToCommittedRootSource(string $commandClass, string $readLayoutId, string $commandPrimaryKey, bool $setsRootSource): void
    {
        ...
        $command = $this->layoutCommand($commandClass, ContentLayoutDefinition::ENTITY_NAME, $commandPrimaryKey, $setsRootSource, 'blog');
