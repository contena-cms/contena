...
class PersistedLayoutMutatorTest extends TestCase
{
    ...
    {
        yield 'an entity root source threads its resolved root-ambient context' => ['blog', true];
    ...
    private function repository(?ContentLayoutEntity $entity): EntityRepository
    {
        ...
        $searchResult = new EntitySearchResult($collection->count(), $collection, null, new Criteria(), $context);
    ...
    private function entity(string $id, ?string $updatedAt, string $rootSource = 'blog'): ContentLayoutEntity
    {
        $entity = new ContentLayoutEntity();
        $entity->setId($id);
        $entity->setUniqueIdentifier($id);
        $entity->setRootSource($rootSource);
        $entity->setLayout([new ContentElement('block-a', 'CT:Card'), new ContentElement('block-b', 'CT:Card')]);
    ...
    private function orphaningMutation(string $orphanId): LayoutMutation
    {
        return new class($orphanId) implements LayoutMutation {
            ...
            {
                return [new ContentElement($this->orphanId, 'CT:Block')];
            }
