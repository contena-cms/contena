...
class MutationPipelineTest extends TestCase
{
    ...
    public function testRunForwardsRootContextToBothAnalyzePasses(): void
    {
        ...
        $diagnostics->expects($this->exactly(2))
            ...
            ->willReturnCallback(fn (array $roots): LayoutAnalysis => new LayoutAnalysis(new DiagnosticsReport([]), $this->blogResolutions($roots)));
    ...
    public function testRunReanalyzesTheWiredTree(): void
    {
        ...
        $diagnostics->expects($this->exactly(2))
            ...
            ->willReturnCallback(function (array $roots) use (&$analyzed, &$secondPassResolutions, $secondReport): LayoutAnalysis {
                ...
                if (\count($analyzed) === 1) {
                    return new LayoutAnalysis(new DiagnosticsReport([]), $this->blogResolutions($roots));
                }
                ...
                $secondPassResolutions = $this->blogResolutions($roots, marked: true);
    ...
    public function testReplaceRestoresTheUnwiredConsumer(): void
    {
        ...
        static::assertArrayHasKey('blog', $replaced->contextDefinitions->getAllConsumers());
    }
    ...
    {
        $result = $this->pipeline($this->diagnosticsResolvingBlog())->run(new DuplicateElement('el-1'), $this->unwiredTree(), null);
        ...
        static::assertArrayHasKey('blog', $children[1]->contextDefinitions->getAllConsumers());
    }
    ...
    /**
     * A container holding one element whose `blog` reference stays provable while the element carries no
     ...
     */
    private function blogResolutions(array $roots, bool $marked = false): array
    {
        $resolutions = [];
        $key = $marked ? 'blog-second-pass' : 'blog';
    ...
    private function diagnosticsResolvingBlog(): LayoutDiagnostics
    {
        $diagnostics = static::createStub(LayoutDiagnostics::class);
        $diagnostics->method('analyze')->willReturnCallback(
            fn (array $roots): LayoutAnalysis => new LayoutAnalysis(new DiagnosticsReport([]), $this->blogResolutions($roots))
        );
