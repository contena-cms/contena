...
class ContentSystemDataLoaderMapTest extends TestCase
{
    ...
    {
        $capability = $this->map()->capabilityFor('entity', BlogEntity::class);

        static::assertInstanceOf(LoaderTypeCapability::class, $capability);
        static::assertSame(ChannelBlogEntity::class, $capability->producedType);
    ...
    {
        static::assertNull($this->map()->capabilityFor('unknown', BlogEntity::class));
    }
    ...
    public static function derivesResidualConfigKeysProvider(): iterable
    {
        yield 'a required key the template does not fill as the residual' => [
            ...
            new LoaderTypeCapability(ChannelBlogEntity::class, ['entity' => 'blog']),
            ['property'],
        ];
