...
use Contena\Core\Framework\ContentSystem\Layout\Codec\StoredTreeWiringConstraints;
...
class StoredTreeWiringConstraintsTest extends StoredTreeConstraintsTestCase
{
    ...
    {
        static::assertCount(0, $this->validate([$this->element(['providesContext' => ['blog' => $provider]])]));
    }
    ...
    public function testReportsEveryCollisionAgainstTheFirstHolder(): void
    {
        $violations = $this->validate([$this->element(['acceptsContext' => [
            ...
            'brand' => ['type' => 'single', 'required' => true, 'propertyAlias' => 'blog'],
        ]])]);
        ...
        static::assertSame(
            [
                'This consumer writes the property key blog, which context blog already writes.',
                'This consumer writes the property key blog, which context blog already writes.',
            ],
            $messages
        );
    ...
    {
        $violations = $this->validate([$this->element(['providesContext' => ['blog' => $provider]])]);
    ...
    public function testRejectsANonStringKeyInsideAProviderConfig(): void
    {
        ...
        $violations = $this->validate([$this->element(['providesContext' => ['blog' => $config]])]);
    ...
    public static function rejectsAMalformedWiringTypeProvider(): iterable
    {
        ...
        yield 'a non-array provider entry' => [
            ['providesContext' => ['blog' => 'not-an-array']],
    ...
    public static function rejectsPerConsumerCombinationProvider(): iterable
    {
        ...
        yield 'a property alias carrying dot notation' => [
            ['acceptsContext' => [
                'items' => ['type' => 'single', 'required' => true, 'propertyAlias' => 'blog.name'],
            ]],
    ...
    {
        // Base keys 'blog' and 'category' are distinct, so the base-key rule stays silent and these
        ...
        yield 'two redistributing consumers keyed by dotted paths' => [
            ...
            [
                ['[0][acceptsContext][blog.manufacturer][redistribute]', self::DOTTED_PATH_MESSAGE],
        ...
        yield 'two redistributing consumers whose derived keys authored providers hold' => [
            [
                'providesContext' => [
                    'blog' => ['type' => 'single', 'distribution' => 'broadcast'],
            ...
            [
                ['[0][acceptsContext][blog][redistribute]', self::providerConflictMessage('blog')],
    ...
    public static function rejectsElementLocalWiringProvider(): iterable
    {
        ...
        yield 'a redistributing consumer keyed by a dotted path' => [
            ...
            '[0][acceptsContext][blog.manufacturer][redistribute]',
        ...
        yield 'a redistributing consumer whose context key an authored provider holds' => [
            ...
            '[0][acceptsContext][blog][redistribute]',
            self::providerConflictMessage('blog'),
        ];
    ...
    public static function acceptsCleanElementWiringProvider(): iterable
    {
        ...
        yield 'a root-scoped consumer that does not redistribute' => [
            ['acceptsContext' => ['blog' => ['type' => 'single', 'required' => true, 'scope' => 'root']]],
        ];
        ...
        yield 'a redistributing consumer declaring the parent scope explicitly' => [
            ['acceptsContext' => [
                'blog' => ['type' => 'single', 'required' => true, 'redistribute' => true, 'scope' => 'parent'],
            ]],
        ];
        ...
        yield 'a root-scoped consumer keyed by a dotted path' => [
            ['acceptsContext' => ['blog.cover' => ['type' => 'single', 'required' => true, 'scope' => 'root']]],
        ];
        ...
        yield 'a consumer alias equal to an authored provider key' => [
            [
                ...
                'acceptsContext' => [
                    'blog' => ['type' => 'single', 'required' => true, 'redistribute' => true, 'consumerAlias' => 'item'],
                ],
            ],
        ];
    }
    ...
    public static function reportsBothViolationsProvider(): iterable
    {
        ...
        yield 'a redistributing consumer keyed by a dotted path onto a held base key' => [
            ...
            [
                '[0][acceptsContext][blog.manufacturer]',
                '[0][acceptsContext][blog.manufacturer][redistribute]',
            ],
        ];
        ...
        yield 'a root-scoped consumer redistributing under a dotted key' => [
            ...
            [
                '[0][acceptsContext][blog.manufacturer][scope]',
                '[0][acceptsContext][blog.manufacturer][redistribute]',
            ],
        ];
    ...
    public static function rejectsDistributionProvider(): iterable
    {
        yield 'a provider declaring no distribution, whose strategy fields are skipped' => [
            ['type' => 'single'],
            '[0][providesContext][blog][distribution]',
        ];
        ...
        yield 'a sliced provider with no slice size' => [
            ...
            '[0][providesContext][blog][sliceSize]',
        ];
    }
