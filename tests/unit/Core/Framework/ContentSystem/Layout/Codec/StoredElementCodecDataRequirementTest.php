...
class StoredElementCodecDataRequirementTest extends StoredElementCodecTestCase
{
    ...
    {
        $element = $this->codec()->decode(self::baseWire(['dataRequirements' => ['blogs' => $requirement]]));

        // The map key always stays the outer one; only the requirement's own key falls back to it.
        static::assertSame(['blogs'], array_keys($element->dataRequirements));
        static::assertSame($expected, $element->dataRequirements['blogs']->key);
    }
    ...
    {
        $stored = ['source' => 'entity', 'config' => ['entity' => 'blog', 'property' => 'blogId']];

        $encoded = $this->codec()->decode(self::baseWire(['dataRequirements' => ['blogs' => $stored]]))->jsonSerialize();

        static::assertArrayNotHasKey('key', $stored);
        static::assertSame(
            ['key' => 'blogs', 'source' => 'entity', 'config' => ['entity' => 'blog', 'property' => 'blogId']],
            $encoded['dataRequirements']['blogs'] ?? null,
        );
    }
    ...
    public function testDecodeThrowsWithElementIdWhenSourceUnregistered(): void
    {
        ...
        $wire = self::baseWire([
            'id' => 'el-unregistered',
            'dataRequirements' => [
                'blogs' => ['source' => 'removed_plugin_source', 'config' => []],
            ],
        ]);
    ...
    public function testDecodePropagatesUnrelatedContentSystemExceptionFromDataRequirements(): void
    {
        ...
        $wire = self::baseWire([
            'dataRequirements' => [
                'blogs' => ['source' => 'broken_source', 'config' => []],
            ],
        ]);
