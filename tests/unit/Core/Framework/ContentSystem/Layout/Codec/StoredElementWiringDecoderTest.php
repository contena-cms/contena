...
class StoredElementWiringDecoderTest extends StoredElementCodecTestCase
{
    ...
    {
        $element = $this->codec()->decode(self::baseWire(['providesContext' => ['blog' => $provider]]));

        $config = $element->contextDefinitions->getAllProviders()['blog']->distributionConfig;
    ...
    {
        $element = $this->codec()->decode(self::baseWire(['acceptsContext' => ['blog' => $consumer]]));

        static::assertSame($expected, $element->contextDefinitions->getAllConsumers()['blog']->scope);
    }
    ...
    public function testDecodedConsumerEncodesItsScopeOnlyWhenRoot(array $consumer, ConsumerScope $expected, array $expectedWire): void
    {
        ...
        static::assertSame($expectedWire, $element->contextDefinitions->getAllConsumers()['blog']->jsonSerialize());
    }
    ...
    {
        $expected = ContentSystemException::rootScopeWithRedistribute('blog');

        try {
            $this->codec()->decode(self::baseWire(['acceptsContext' => [
                'blog' => ['type' => 'single', 'required' => true, 'redistribute' => true, 'scope' => 'root'],
            ]]));
    ...
    public static function rejectsElementLocalWiringProvider(): iterable
    {
        yield 'two consumers sharing one base key' => [
            ...
            ContentSystemException::propertyAliasCollision('blog', 'blog', 'category'),
        ];
        ...
        yield 'a redistributing consumer keyed by a dotted path' => [
            ...
            ContentSystemException::redistributeWithDottedPath('blog.manufacturer'),
        ];
        ...
        yield 'a redistributing consumer whose context key an authored provider holds' => [
            ...
            ContentSystemException::redistributeConflict('blog'),
        ];
        ...
        yield 'a redistributing consumer whose property alias an authored provider holds' => [
            self::baseWire([
                'providesContext' => ['blog' => ['type' => 'single', 'distribution' => 'broadcast']],
                'acceptsContext' => [
                    'source' => ['type' => 'single', 'required' => true, 'redistribute' => true, 'propertyAlias' => 'blog'],
                ],
            ]),
    ...
    public static function acceptsCleanElementWiringProvider(): iterable
    {
        yield 'two consumers writing distinct base keys' => [
            ...
            ['blog' => [false, null], 'category' => [false, 'item']],
        ];

        yield 'a redistributing consumer keyed by a base key' => [
            self::baseWire(['acceptsContext' => [
                'blog' => ['type' => 'single', 'required' => true, 'redistribute' => true],
            ]]),
            ['blog' => [true, null]],
        ];
        ...
        yield 'a redistributing consumer whose property alias no provider holds' => [
            self::baseWire([
                'providesContext' => ['blog' => ['type' => 'single', 'distribution' => 'broadcast']],
                'acceptsContext' => [
                    'source' => ['type' => 'single', 'required' => true, 'redistribute' => true, 'propertyAlias' => 'blogList'],
                ],
            ]),
            ['source' => [true, 'blogList']],
        ];
    ...
    public static function pinsElementWiringCheckOrderProvider(): iterable
    {
        ...
        yield 'the base-key rule for an element violating two element-local rules' => [
            ...
            ContentSystemException::propertyAliasCollision('blog', 'blog', 'blog.manufacturer'),
        ];
        ...
        yield 'the dotted-key rule for a consumer violating both redistribute rules at once' => [
            self::baseWire([
                'providesContext' => ['blog.manufacturer' => ['type' => 'single', 'distribution' => 'broadcast']],
                'acceptsContext' => [
                    'blog.manufacturer' => ['type' => 'single', 'required' => true, 'redistribute' => true],
                ],
            ]),
        ...
        yield 'the root-scope rule for a root-scoped consumer redistributing under a dotted key' => [
            ...
            ContentSystemException::rootScopeWithRedistribute('blog.manufacturer'),
        ];
    ...
    public static function rejectsStructurallyMalformedWiringProvider(): iterable
    {
        ...
        yield 'a non-array provider config' => [
            self::baseWire(['providesContext' => ['blog' => 'not-an-array']]),
            ContentSystemException::invalidFieldValueType('providesContext[blog]', 'array', 'string'),
        ];
        ...
        yield 'an unparseable provider context-type' => [
            self::baseWire([
                'providesContext' => ['blog' => ['type' => 'bogus-type', 'distribution' => 'keyed', 'keyProperty' => 'sku']],
            ]),
            ContentSystemException::invalidFieldValueType('providesContext[blog].type', implode('|', ContextType::values()), 'string'),
        ];

        yield 'an unparseable provider distribution strategy' => [
            self::baseWire([
                'providesContext' => ['blog' => ['type' => 'single', 'distribution' => 'bogus-strategy']],
            ]),
            ContentSystemException::invalidFieldValueType('providesContext[blog].distribution', implode('|', DistributionStrategy::values()), 'string'),
        ];
        ...
        yield 'a non-string key inside a provider config' => [
            ...
            ContentSystemException::invalidMapKey('providesContext[blog]', 'int'),
        ];
    }
}
