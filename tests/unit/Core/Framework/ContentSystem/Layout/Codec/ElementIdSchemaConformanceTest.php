...
class ElementIdSchemaConformanceTest extends StoredElementCodecTestCase
{
    ...
    private const CHANNEL_API_PATHS = 'src/Core/Framework/Api/ApiDefinition/Generator/Schema/ChannelApi/paths/content.json';
    ...
    /**
     * The Channel API is a separate document, so its `elementId` query parameter cannot reference the Admin API
     * component and carries the pattern inline. Nothing but this assertion ties the two copies together.
     */
    #[TestDox('the Channel API elementId query parameter carries the same pattern')]
    public function testTheChannelApiQueryParameterCarriesThePublishedPattern(): void
    {
        $patterns = [];

        foreach (self::readSchema(self::CHANNEL_API_PATHS)['paths'] as $operations) {
        ...
        static::assertNotSame([], $patterns, 'the Channel API declares no elementId query parameter');
