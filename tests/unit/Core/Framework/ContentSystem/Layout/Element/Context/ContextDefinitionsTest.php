...
class ContextDefinitionsTest extends TestCase
{
    ...
    public function testWithAddedProvidersMergesImmutablyAndOverwritesOnCollision(): void
    {
        ...
        $original = new ContextDefinitions(
            providers: ['blog' => $existingProvider, 'category' => $existingProvider],
        );
        ...
        static::assertSame(
            ['blog' => $existingProvider, 'category' => $additionalProvider, 'region' => $replacementProvider],
            $merged->getAllProviders()
        );
        static::assertSame(
            ['blog' => $existingProvider, 'category' => $existingProvider],
