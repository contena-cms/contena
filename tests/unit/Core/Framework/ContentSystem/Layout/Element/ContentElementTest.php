...
use Contena\Core\Test\Stub\ContentSystem\OrderTrackingVisitor;
...
class ContentElementTest extends TestCase
{
    ...
    public function testCollectConsumersReturnsDirectChildrenMatchingContextKey(): void
    {
        ...
        $pathResolver = new ContextPathResolver();
        $result = $parent->collectConsumers('blog', $pathResolver);
    ...
    public function testReplacePlaceholdersSubstitutesValuesInStringProperties(): void
    {
        $element = ContentElementBuilder::create('test-component')
            ...
            ->withProperty('description', 'Blog: {{blogId}}')
            ->build();

        $specification = $this->createRenderingSpecification(['name' => 'World', 'blogId' => 'abc123']);

        $element->replacePlaceholders($specification);

        static::assertSame('Hello World!', $element->getProperty('title'));
        static::assertSame('Blog: abc123', $element->getProperty('description'));
    }
    ...
    {
        yield 'exact key match returns true' => ['blog', 'blog', true];
        yield 'prefix path match returns true' => ['blog', 'blog.cover', true];
        yield 'unrelated key returns false' => ['blog', 'category', false];
        yield 'partial prefix without dot returns false' => ['blog', 'blogName', false];
    }
