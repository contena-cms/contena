...
namespace Contena\Tests\Unit\Core\Framework\ContentSystem\Layout\Field;
...
class ContentElementFieldSerializerTest extends TestCase
{
    ...
    public function testDecodeElementWithDataRequirementsReturnsContentElementWithRequirements(): void
    {
        ...
        $data = [
            ...
            'component' => 'blog-card',
            'dataRequirements' => [
                'blog' => ['key' => 'blog', 'source' => 'entity', 'config' => []],
            ],
        ];
        ...
        static::assertArrayHasKey('blog', $result->getDataRequirements());
        static::assertInstanceOf(DataRequirement::class, $result->getDataRequirements()['blog']);
    }
    ...
    public function testDecodeElementWithAttributedSpecificationsReturnsMap(): void
    {
        $result = $this->serializer->decodeElement([
            'id' => 'elem-attr',
            'component' => 'text',
            'attributedSpecifications' => ['blog' => 'binding-spec-1'],
        ]);

        static::assertSame(['blog' => 'binding-spec-1'], $result->getAttributedSpecifications());
    }
    ...
    {
        $config = static::createStub(AbstractContentDataLoaderConfig::class);
        $this->configProvider->method('encode')->willReturn(['entityName' => 'blog']);
        $this->configProvider->method('decode')->willReturn($config);

        $child = ContentElementBuilder::create('text', 'child-rt')->build();
        $original = ContentElementBuilder::create('blog-card', 'elem-rt')
            ->withProperty('title', 'My Blog')
            ->withDataRequirement('blog', 'entity', $config)
            ->withSlot('footer', [$child])
            ->withProvider('myBlog', BroadcastDistributionConfig::simple(), ContextType::Single)
        ...
        static::assertSame('My Blog', $restored->getProperty('title'));
    ...
    {
        $original = ContentElementBuilder::create('blog-card', 'elem-attr-rt')
            ->withAttributedSpecification('blog', 'binding-spec-1')
        ...
        static::assertSame(
            ['blog' => 'binding-spec-1', 'category' => 'binding-spec-2'],
    ...
    {
        $element = ContentElementBuilder::create('blog-card', 'elem-attr')
            ->withAttributedSpecification('blog', 'binding-spec-1')
            ->build();
        ...
        static::assertSame(['blog' => 'binding-spec-1'], $result['attributedSpecifications']);
    }
    ...
    public function testRejectsNonStringAttributionValue(): void
    {
        $element = [
            'id' => 'el',
            'component' => 'text',
            'attributedSpecifications' => ['blog' => 123],
        ];
        ...
        static::assertContains('[attributedSpecifications][blog]', $paths);
    }
    ...
    public function testSerializeContentElementUsesCamelCaseWireFormat(): void
    {
        ...
        $config = new EntityLoaderConfig('blog', 'blog', ['tags']);
        $child = ContentElementBuilder::create('text', 'slot-child-1')->build();

        $element = ContentElementBuilder::create('blog-card', 'rt-elem-1')
            ->withProperty('title', 'My Blog')
            ->withProperty('count', 42)
            ->withDataRequirement('blog', 'entity', $config)
            ->withProvider('blogCtx', BroadcastDistributionConfig::aliased('myAlias'), ContextType::Single)
        ...
        static::assertArrayHasKey('blogCtx', $serialized['providesContext']);
        $providerEntry = $serialized['providesContext']['blogCtx'];
    ...
    public function testDecodeElementRestoresValuesFromCamelCaseWireFormat(): void
    {
        ...
        static::assertSame('blog-card', $decoded->getComponent());
        static::assertSame('My Blog', $decoded->getProperty('title'));
        ...
        static::assertArrayHasKey('blog', $decoded->getDataRequirements());
        $decodedConfig = $decoded->getDataRequirements()['blog']->config;
        static::assertInstanceOf(EntityLoaderConfig::class, $decodedConfig);
        static::assertSame('blog', $decodedConfig->entity);
        static::assertSame('blog', $decodedConfig->property);
        static::assertSame(['tags'], $decodedConfig->associations);
        ...
        static::assertArrayHasKey('blogCtx', $providers);
        static::assertSame(ContextType::Single, $providers['blogCtx']->type);
        $decodedDistribution = $providers['blogCtx']->distributionConfig;
