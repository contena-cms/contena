...
class ContextProvidersFieldSerializerTest extends TestCase
{
    ...
    public function testEncodeWithContextProviderArrayYieldsJson(): void
    {
        ...
        $kvPair = new KeyValuePair('provides_context', ['blog' => $provider], false);
        ...
        static::assertArrayHasKey('blog', $decoded);
        static::assertSame('single', $decoded['blog']['type']);
        static::assertSame('broadcast', $decoded['blog']['distribution']);
        static::assertNull($decoded['blog']['consumerAlias']);
    }
    ...
    public function testEncodeWithPlainArrayPassthroughYieldsJson(): void
    {
        $field = $this->createContextProvidersField();
        $arrayValue = [
            'blog' => ['type' => 'single', 'distribution' => 'broadcast', 'consumerAlias' => null],
        ];
    ...
    public function testDecodeWithJsonStringReturnsContextProviders(): void
    {
        ...
        static::assertInstanceOf(ContextProvider::class, $result['blog']);
        static::assertSame(ContextType::Single, $result['blog']->type);
        static::assertInstanceOf(BroadcastDistributionConfig::class, $result['blog']->distributionConfig);
    ...
    public function testValidationPassesForKnownDistributionType(
        ...
        $violations = $this->validateProviderData(
            ['blog' => ['type' => $type, 'distribution' => $distribution]]
        );
    ...
    {
        $violations = $this->validateProviderData(
            ['blog' => ['type' => 'single', 'distribution' => 'unknown']]
        );
    ...
    {
        $violations = $this->validateProviderData(
            ['blog' => ['type' => 'single', 'distribution' => 'indexed', 'consumerAlias' => 42]]
        );
    ...
    {
        $violations = $this->validateProviderData(
            ['blog' => ['type' => 'collection', 'distribution' => 'iterator', 'consumerAlias' => 42]]
        );
    ...
    public function testValidationReportsViolationForKeyedDistributionMissingKeyProperty(): void
    {
        ...
        $violations = $this->validateProviderData(
            ['blog' => ['type' => 'collection', 'distribution' => 'keyed']]
        );
