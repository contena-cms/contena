...
class PluginExceptionTest extends TestCase
{
    ...
    {
        $exception = PluginException::cannotCreateTemporaryDirectory('/tmp/contena', 'plugin');

        static::assertSame(PluginException::CANNOT_CREATE_TEMPORARY_DIRECTORY, $exception->getErrorCode());
        static::assertSame(Response::HTTP_INTERNAL_SERVER_ERROR, $exception->getStatusCode());
        static::assertSame('Could not create temporary directory in "/tmp/contena" with prefix "plugin"', $exception->getMessage());
        static::assertSame(['targetDirectory' => '/tmp/contena', 'prefix' => 'plugin'], $exception->getParameters());
    }
    ...
    public function testCannotDeleteContenaMigrations(): void
    {
        $exception = PluginException::cannotDeleteContenaMigrations();

        static::assertSame(PluginException::CANNOT_DELETE_CONTENA_MIGRATIONS, $exception->getErrorCode());
        static::assertSame(Response::HTTP_INTERNAL_SERVER_ERROR, $exception->getStatusCode());
        static::assertSame('Deleting Contena migrations is not allowed', $exception->getMessage());
        static::assertSame([], $exception->getParameters());
    }
    ...
    public function testWrongBaseClass(): void
    {
        ...
        static::assertSame(Response::HTTP_BAD_REQUEST, $exception->getStatusCode());
        static::assertSame('"Foo\\Bar" in the container should be an instance of Contena\Core\Framework\Plugin', $exception->getMessage());
        static::assertSame(['baseClass' => 'Foo\\Bar'], $exception->getParameters());
    }
    ...
    {
        $exception = PluginException::couldNotDetectComposerVersion(['foo/bar' => '/var/www/contena/custom/plugins/fooBar/vendor/composer/../../']);

        static::assertSame(PluginException::COULD_NOT_DETECT_COMPOSER_VERSION, $exception->getErrorCode());
        static::assertSame(Response::HTTP_INTERNAL_SERVER_ERROR, $exception->getStatusCode());
        static::assertSame("Could not detect the installed composer version. Checked paths: \nfoo/bar: /var/www/contena/custom/plugins/fooBar/vendor/composer/../../\n", $exception->getMessage());
    ...
    public function testKernelPluginLoaderError(): void
    {
        ...
        static::assertSame('FRAMEWORK__KERNEL_PLUGIN_LOADER_ERROR', $exception->getErrorCode());
        static::assertSame(Response::HTTP_BAD_REQUEST, $exception->getStatusCode());
