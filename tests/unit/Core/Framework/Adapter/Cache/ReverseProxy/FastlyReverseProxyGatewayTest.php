...
class FastlyReverseProxyGatewayTest extends TestCase
{
    ...
    public function testTagWithInstanceTag(): void
    {
        ...
        $gateway = new FastlyReverseProxyGateway($this->client, 'test', 'test', '0', 3, '', 'ct-1234', 'http://localhost', new NullLogger());
        $gateway->tag(['foo', 'bla'], '', $resp);

        static::assertSame('foo bla ct-1234', $resp->headers->get('surrogate-key'));
    }
    ...
    public function testBanAllWithInstanceTag(): void
    {
        ...
        $gateway = new FastlyReverseProxyGateway($this->client, 'test', 'key', '0', 3, '', 'ct-1234', 'http://localhost', new NullLogger());
        $gateway->banAll();
        $gateway->flush();
        ...
        static::assertSame(['ct-1234'], $lastRequest->getHeader('surrogate-key'));
