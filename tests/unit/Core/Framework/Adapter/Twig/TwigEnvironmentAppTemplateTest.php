...
{
    private const PAGE = 'frontend/page.html.twig';
    ...
    protected function setUp(): void
    {
        ...
        $this->filesystemLoader = new ArrayLoader([
            '@Frontend/' . self::PAGE => 'before|{% block content %}core{% endblock %}|after',
        ]);
        ...
        $hierarchy->method('buildHierarchy')->willReturn(['HealthyApp' => 2, 'BrokenApp' => 1, 'Frontend' => 0]);
    ...
    {
        $this->appTemplates['@HealthyApp/' . self::PAGE] = '{% sw_extends "@Frontend/' . self::PAGE . '" %}{% block content %}healthy[{{ parent() }}]{% endblock %}';
        $this->appTemplates['@BrokenApp/' . self::PAGE] = '{% sw_extends "@Frontend/' . self::PAGE . '" %}{% block content %}partial{{ 1 / divisor }}{% endblock %}';
        $name = $this->finder->find('@Frontend/' . self::PAGE);
    ...
    {
        $name = '@BrokenApp/' . self::PAGE;
        $this->appTemplates[$name] = '{% sw_extends "@Frontend/' . self::PAGE . '" %}{% block content %}{% include "missing.html.twig" %}{% endblock %}';
    ...
    {
        $name = '@BrokenApp/' . self::PAGE;
        $this->appTemplates[$name] = '{% sw_extends "@Frontend/' . self::PAGE . '" %}{% block content %}partial{% include "@BrokenApp/frontend/fragment.html.twig" %}{% endblock %}';
        $this->appTemplates['@BrokenApp/frontend/fragment.html.twig'] = '{{ 1 / 0 }}';
    ...
    public function testMultipleBrokenAppsEachGetOneRetry(): void
    {
        ...
        static::assertSame('before|core|after', $this->twig->render($this->finder->find('@Frontend/' . self::PAGE)));
        static::assertCount(2, $this->logs->getRecords());
    }

    public function testStandaloneAppFragmentWithoutFallbackBecomesEmpty(): void
    {
        $this->filesystemLoader->setTemplate('page', 'before|{% include "@BrokenApp/frontend/fragment.html.twig" %}|after');
        $this->appTemplates['@BrokenApp/frontend/fragment.html.twig'] = 'partial{{ 1 / 0 }}';
    ...
    {
        $this->filesystemLoader->setTemplate('page', 'before|{% sw_include "@Frontend/frontend/fragment.html.twig" %}|after');
        $this->appTemplates['@BrokenApp/frontend/fragment.html.twig'] = 'partial{{ 1 / 0 }}';
    ...
    {
        $this->filesystemLoader->setTemplate('page', 'before|{{ sw_include("@BrokenApp/frontend/fragment.html.twig") }}|after');
        $this->appTemplates['@BrokenApp/frontend/fragment.html.twig'] = 'partial{{ 1 / 0 }}';
    ...
    {
        $name = '@BrokenApp/' . self::PAGE;
        $this->filesystemLoader->setTemplate('@Frontend/' . self::PAGE, '{% sw_include "@Frontend/frontend/missing.html.twig" %}');
        $this->appTemplates[$name] = '{% broken %}';
    ...
    {
        $name = '@BrokenApp/' . self::PAGE;
        $this->appTemplates[$name] = '{% embed "@Frontend/' . self::PAGE . '" %}{% block content %}partial{{ 1 / divisor }}{% endblock %}{% endembed %}';
    ...
    {
        $this->filesystemLoader->setTemplate('@Frontend/' . self::PAGE, '{{ 1 / 0 }}');
        $this->appTemplates['@BrokenApp/' . self::PAGE] = '{% sw_extends "@Frontend/' . self::PAGE . '" %}';
