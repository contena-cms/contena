...
class McpJsonRpcResponseTest extends TestCase
{
    ...
    public function testFromJsonReturnsNullWhenInitializeFieldsHaveWrongTypes(): void
    {
        ...
        $json = (string) json_encode([
            ...
            'result' => [
                ...
                'serverInfo' => ['name' => 'contena', 'version' => '6.7.0'],
            ],
        ]);
    ...
    public function testFromJsonParsesResourcesListResponse(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->resourcesListJson([
            ['uri' => 'contena://entities', 'name' => 'contena-entities'],
        ]));
    ...
    {
        $response = McpJsonRpcResponse::fromJson($this->promptsListJson(['contena-context']));
    ...
    {
        $json = $this->resourcesListJson([['uri' => 'contena://entities', 'name' => 'contena-entities']]);
    ...
    public function testFilterResourcesKeepsAllowedResources(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->resourcesListJson([
            ['uri' => 'contena://entities', 'name' => 'contena-entities'],
            ['uri' => 'contena://currencies', 'name' => 'contena-currencies'],
            ['uri' => 'contena://state-machines', 'name' => 'contena-state-machines'],
        ]));
        static::assertNotNull($response);

        $response->filterResources(['contena://entities', 'contena://currencies']);

        $data = json_decode(Json::encode($response), true, 512, \JSON_THROW_ON_ERROR);
        $uris = array_column($data['result']['resources'], 'uri');
        static::assertSame(['contena://entities', 'contena://currencies'], $uris);
    }
    ...
    public function testFilterResourcesReindexesArray(): void
    {
        ...
        $response->filterResources(['contena://currencies']);

        $data = json_decode(Json::encode($response), true, 512, \JSON_THROW_ON_ERROR);
        static::assertArrayHasKey(0, $data['result']['resources']);
        static::assertSame('contena://currencies', $data['result']['resources'][0]['uri']);
    }
    ...
    public function testFilterResourcesIsNoOpForNonResourcesResponse(): void
    {
        ...
        $response->filterResources(['contena://entities']);
    ...
    {
        $response = McpJsonRpcResponse::fromJson($this->promptsListJson(['contena-context', 'contena-developer']));
        static::assertNotNull($response);

        $response->filterPrompts(['contena-context']);

        $data = json_decode(Json::encode($response), true, 512, \JSON_THROW_ON_ERROR);
        $names = array_column($data['result']['prompts'], 'name');
        static::assertSame(['contena-context'], $names);
    }
    ...
    public function testAddContenaMetaReturnsFalseWhenBothNull(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->initializeJson());
        static::assertNotNull($response);

        static::assertFalse($response->addContenaMeta(null, null));
    }

    public function testAddContenaMetaReturnsFalseForNonInitializeResponse(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->toolsListJson(['tool-a']));
        static::assertNotNull($response);

        static::assertFalse($response->addContenaMeta('user-123', null));
    }

    public function testAddContenaMetaAddsUserId(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->initializeJson());
        static::assertNotNull($response);

        $added = $response->addContenaMeta('user-123', null);

        static::assertTrue($added);
        $data = json_decode(Json::encode($response), true, 512, \JSON_THROW_ON_ERROR);
        static::assertSame('user-123', $data['result']['_meta']['contena']['user']['id']);
        static::assertArrayNotHasKey('integration', $data['result']['_meta']['contena']);
    }

    public function testAddContenaMetaAddsIntegrationId(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->initializeJson());
        static::assertNotNull($response);

        $added = $response->addContenaMeta(null, 'integration-456');

        static::assertTrue($added);
        $data = json_decode(Json::encode($response), true, 512, \JSON_THROW_ON_ERROR);
        static::assertSame('integration-456', $data['result']['_meta']['contena']['integration']['id']);
        static::assertArrayNotHasKey('user', $data['result']['_meta']['contena']);
    }

    public function testAddContenaMetaAddsBoth(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->initializeJson());
        static::assertNotNull($response);

        $response->addContenaMeta('user-123', 'integration-456');
    ...
    public function testAddContenaMetaPreservesServerCapabilitiesAsObjects(): void
    {
        $response = McpJsonRpcResponse::fromJson($this->initializeJson());
        static::assertNotNull($response);

        $response->addContenaMeta('user-123', null);
