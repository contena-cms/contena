...
class McpAllowlistListRequestHandlerTest extends TestCase
{
    ...
    {
        $registry = new Registry();
        foreach (['contena-tool-search', 'tool-a', 'tool-b', 'tool-c'] as $toolName) {
            $registry->registerTool($this->tool($toolName), static fn (): string => '');
        }
        ...
        $handler = $this->createHandler(
            ...
            advertisedTools: ['contena-tool-search', 'tool-a', 'tool-c'],
        );

        $firstResult = $this->handleToolsList($handler, null);

        static::assertSame(['contena-tool-search', 'tool-a'], array_map(static fn (Tool $tool): string => $tool->name, $firstResult->tools));
        static::assertSame(base64_encode('2'), $firstResult->nextCursor);
    ...
    {
        $registry = new Registry();
        foreach (['contena-tool-search', 'tool-a'] as $toolName) {
            $registry->registerTool($this->tool($toolName), static fn (): string => '');
        }
        ...
        static::assertSame(['contena-tool-search', 'tool-a'], array_map(static fn (Tool $tool): string => $tool->name, $result->tools));
    }
    ...
    public function testToolsListWithEmptyToolAllowlistOnlyAdvertisesToolSearch(): void
    {
        ...
        $handler = $this->createHandler(
            ...
            advertisedTools: ['contena-tool-search', 'tool-a'],
        );

        $result = $this->handleToolsList($handler, null);

        static::assertSame(['contena-tool-search'], array_map(static fn (Tool $tool): string => $tool->name, $result->tools));
    }

    public function testToolsListWithEmptyToolAllowlistStillAdvertisesDiscoveryMetaTools(): void
    {
        $registry = new Registry();
        foreach (['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL] as $toolName) {
            $registry->registerTool($this->tool($toolName), static fn (): string => '');
        }
        ...
        $handler = $this->createHandler(
            $registry,
            new McpAllowlist(tools: [], resources: [], prompts: []),
            advertisedTools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
        );

        $firstResult = $this->handleToolsList($handler, null);

        static::assertSame(
            ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL],
    ...
    public function testToolsListAdvertisesDiscoveryMetaToolsEvenWhenRestrictedAllowlistOmitsThem(): void
    {
        ...
        $handler = $this->createHandler(
            $registry,
            new McpAllowlist(tools: ['contena-entity-search'], resources: [], prompts: []),
            advertisedTools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
        );
        ...
        static::assertSame(
            ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
            $names,
        );
    }
    ...
    {
        $registry = new Registry();
        foreach (['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL, 'contena-entity-search', 'contena-system-config-read'] as $toolName) {
            $registry->registerTool($this->tool($toolName), static fn (): string => '');
        }
        ...
        $handler = $this->createHandler(
            $registry,
            new McpAllowlist(
                tools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL, 'contena-entity-search', 'contena-system-config-read'],
                resources: [],
                prompts: [],
            ),
            advertisedTools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
        ...
        static::assertSame(['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL], array_map(static fn (Tool $tool): string => $tool->name, $firstResult->tools));
        static::assertSame(base64_encode('2'), $firstResult->nextCursor);

        $secondResult = $this->handleToolsList($handler, $firstResult->nextCursor);

        static::assertSame([McpToolsetRegistry::ENABLE_TOOLSET_TOOL, 'contena-entity-search'], array_map(static fn (Tool $tool): string => $tool->name, $secondResult->tools));
        static::assertNull($secondResult->nextCursor);
    }
    ...
    public function testToolsListKeepsAllowlistAsBoundaryForEnabledToolsetTools(): void
    {
        ...
        $handler = $this->createHandler(
            $registry,
            new McpAllowlist(
                tools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
                resources: [],
                prompts: [],
            ),
            advertisedTools: ['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL],
        ...
        static::assertSame(['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL], array_map(static fn (Tool $tool): string => $tool->name, $result->tools));
    ...
    {
        $registry = new Registry();
        foreach (['contena-tool-search', McpToolsetRegistry::LIST_TOOLSETS_TOOL, McpToolsetRegistry::ENABLE_TOOLSET_TOOL, 'contena-entity-search'] as $toolName) {
            $registry->registerTool($this->tool($toolName), static fn (): string => '');
        }
    ...
    public function testResourcesListFiltersAllowlistBeforePaginationAndReturnsCursorRemainder(): void
    {
        ...
        $handler = $this->createHandler($registry, new McpAllowlist(
            tools: null,
            resources: ['contena://resource-b', 'contena://resource-d', 'contena://resource-e'],
            prompts: null,
        ));
        ...
        static::assertSame(
            ['contena://resource-b', 'contena://resource-d'],
    ...
    public function testResourcesListUsesRegistryPaginationWhenResourceAllowlistAllowsAllResources(): void
    {
        ...
        static::assertSame(
            ['contena://resource-a', 'contena://resource-b'],
            array_map(static fn (ResourceDefinition $resource): string => $resource->uri, $firstResult->resources),
        );
        static::assertSame(base64_encode('2'), $firstResult->nextCursor);
    ...
    public function testResourcesListIgnoresNonResourceReferences(): void
    {
        ...
        static::assertSame(['contena://resource-a'], array_map(static fn (ResourceDefinition $resource): string => $resource->uri, $result->resources));
    }
    ...
    private function createHandler(
        ...
        array $advertisedTools = ['contena-tool-search', 'tool-a', 'tool-b', 'tool-c', 'tool-d', 'tool-e'],
