...
class McpServerControllerTest extends TestCase
{
    ...
    public function testInitializeEnrichmentKeepsEmptyCapabilityObjects(): void
    {
        ...
        $contenaMeta = $meta->contena ?? null;
        static::assertInstanceOf(\stdClass::class, $contenaMeta);

        $integrationMeta = $contenaMeta->integration ?? null;
    ...
    public function testPostWithMalformedJsonBodyDoesNotSetJsonRpcAttributeAndPassesThrough(): void
    {
        ...
        $allowlistProvider->method('forCurrentRequest')->willReturn(new McpAllowlist(tools: ['contena-entity-search'], resources: null, prompts: null));
    ...
    public static function allowedToolCallProvider(): iterable
    {
        yield 'tool explicitly in allowlist' => [
            ...
            new McpAllowlist(tools: ['contena-entity-search'], resources: null, prompts: null),
        ];
    ...
    public function testToolSearchCallIsAllowedEvenWhenToolAllowlistIsEmpty(): void
    {
        $body = json_encode([
            ...
            'id' => 1,
            'method' => 'tools/call',
            'params' => ['name' => 'contena-tool-search', 'arguments' => ['query' => 'entity']],
        ], \JSON_THROW_ON_ERROR);
    ...
    public function testToolCallBlockedWhenNotInAllowlist(): void
    {
        ...
        $body = json_encode([
            ...
            'id' => 1,
            'method' => 'tools/call',
            'params' => ['name' => 'contena-entity-read', 'arguments' => []],
        ], \JSON_THROW_ON_ERROR);
        ...
        static::assertStringContainsString('contena-entity-read', $data['error']['message']);
    ...
    public function testNoToolsAllowedWhenToolsAllowlistIsEmptyArray(): void
    {
        $body = json_encode([
            ...
            'id' => 1,
            'method' => 'tools/call',
            'params' => ['name' => 'contena-entity-schema', 'arguments' => []],
        ], \JSON_THROW_ON_ERROR);
    ...
    public function testResourceReadBlockedWhenNotInAllowlist(): void
    {
        $body = json_encode([
            ...
            'params' => ['uri' => 'contena://state-machines'],
        ], \JSON_THROW_ON_ERROR);

        $psrRequest = new ServerRequest('POST', '/api/_mcp', ['Content-Type' => 'application/json'], $body);
        $allowlistProvider = static::createStub(McpAllowlistProvider::class);
        $allowlistProvider->method('forCurrentRequest')->willReturn(new McpAllowlist(tools: null, resources: ['contena://entities'], prompts: null));
        ...
        $data = json_decode((string) $response->getContent(), true);
        static::assertSame(-32001, $data['error']['code']);
        static::assertStringContainsString('contena://state-machines', $data['error']['message']);
    }
    ...
    public function testPromptGetBlockedWhenNotInAllowlist(): void
    {
        $body = json_encode([
            ...
            'id' => 1,
            'method' => 'prompts/get',
            'params' => ['name' => 'contena-developer'],
        ], \JSON_THROW_ON_ERROR);

        $psrRequest = new ServerRequest('POST', '/api/_mcp', ['Content-Type' => 'application/json'], $body);
        $allowlistProvider = static::createStub(McpAllowlistProvider::class);
        $allowlistProvider->method('forCurrentRequest')->willReturn(new McpAllowlist(tools: null, resources: null, prompts: ['contena-context']));
        ...
        $data = json_decode((string) $response->getContent(), true);
        static::assertSame(-32001, $data['error']['code']);
        static::assertStringContainsString('contena-developer', $data['error']['message']);
    }
    ...
    public function testInitializeEnrichmentSetsUserMetaWhenUserIdPresent(): void
    {
        ...
        static::assertInstanceOf(\stdClass::class, $data->result->_meta->contena);
        $userMeta = $data->result->_meta->contena->user;
