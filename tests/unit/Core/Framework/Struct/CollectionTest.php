...
class CollectionTest extends TestCase
{
    ...
    public function testFilterInstance(): void
    {
        ...
        $countryStruct = new CountryEntity();
        $collection = new TestCollection();
        static::assertCount(0, $collection->filterInstance(MediaEntity::class));

        $collection->add('a');
        $collection->add($mediaStruct);
        $collection->add($countryStruct);

        $filtered = $collection->filterInstance(Struct::class);
        static::assertSame([$mediaStruct, $countryStruct], array_values($filtered->getElements()));
    }
