...
namespace Contena\Tests\Unit\Core\Framework\App\Validation\Requirements;
...
class PublicAccessTest extends TestCase
{
    ...
    {
        $this->setEnvVars(['APP_URL' => 'https://contena.com']);
        $manifest = $this->createManifestMock();
    ...
    public function testValidateReturnsUnmetRequirementWhenHealthCheckReturnsNon200(): void
    {
        ...
        static::assertNotNull($result);
        static::assertSame(
            'Health check at "https://contena.com/api/_info/health-check" returned HTTP 500. Ensure the Contena instance is running and publicly reachable.',
            $result->actionableResolution
        );
    }
    ...
    public function testValidateReturnsHttpStatusWhenRequestExceptionHasResponse(): void
    {
        ...
        $request = new Request('GET', 'https://contena.com/api/_info/health-check');
        ...
        static::assertNotNull($result);
        static::assertSame(
            'Health check at "https://contena.com/api/_info/health-check" returned HTTP 503. Ensure the Contena instance is running and publicly reachable.',
            $result->actionableResolution
        );
    }
    ...
    public function testValidateReturnsUnreachableWhenRequestExceptionHasNoResponse(): void
    {
        ...
        static::assertNotNull($result);
        static::assertSame(
            'Could not reach "https://contena.com/api/_info/health-check". Ensure the Contena instance is publicly accessible at the configured APP_URL.',
            $result->actionableResolution
        );
    }
