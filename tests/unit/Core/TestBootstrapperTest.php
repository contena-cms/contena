...
class TestBootstrapperTest extends TestCase
{
    ...
    {
        $testBootstrapper = new TestBootstrapper();
        $testBootstrapper->addCallingPlugin(__DIR__ . '/Framework/Plugin/Util/_fixture/LocallyInstalledPlugins/CtTest/composer.json');
    ...
    public function testGetClassLoaderRegistersActivePluginAutoloadDevFromKernelPluginLoader(): void
    {
        ...
        $pluginPath = 'vendor/store.contena.cn/ctcmselements';
        $pluginDir = $projectDir . '/' . $pluginPath;

        $pluginLoader = new StaticKernelPluginLoader(new ClassLoader(), null, [
            [
                'name' => 'CtCmsElements',
                'baseClass' => 'CtCmsElements\\CtCmsElements',
                'active' => true,
                'path' => $pluginPath,
                'version' => '1.0.0',
                'autoload' => [],
                'managedByComposer' => true,
                'composerName' => 'store.contena.cn/ctcmselements',
            ],
        ]);
        ...
        try {
            $classLoader = new TestBootstrapper()
                ...
                ->addActivePlugins('CtCmsElements')
                ->getClassLoader();

            static::assertSame([$pluginDir . '/tests/'], $classLoader->getPrefixesPsr4()['CtCmsElements\\Tests\\']);
        } finally {
    ...
    public function testGetPluginPathFindsPluginFromKernelPluginLoader(): void
    {
        $previousKernel = KernelLifecycleAccessor::currentKernel();
        $projectDir = __DIR__ . '/_fixtures/TestBootstrapper/project';
        $vendorPluginPath = 'vendor/store.contena.cn/ctcmselements';
        ...
        try {
            static::assertSame($projectDir . '/' . $vendorPluginPath, new TestBootstrapper()->setProjectDir($projectDir)->getPluginPath('CtCmsElements'));
        } finally {
            KernelLifecycleAccessor::setKernel($previousKernel);
        }
    }
    ...
    public function testGetPluginPathFallsBackToFilesystemForLocalPlugins(): void
    {
        $previousKernel = KernelLifecycleAccessor::currentKernel();
        $projectDir = __DIR__ . '/_fixtures/TestBootstrapper/project';
        $pluginPath = $projectDir . '/custom/static-plugins/CtStaticAnalysis';
        ...
        try {
            static::assertSame($pluginPath, new TestBootstrapper()->setProjectDir($projectDir)->getPluginPath('CtStaticAnalysis'));
        } finally {
            KernelLifecycleAccessor::setKernel($previousKernel);
        }
    }
    ...
    public function testGetClassLoaderRegistersActivePluginAutoloadDevFromFilesystemFallback(): void
    {
        ...
        try {
            $classLoader = new TestBootstrapper()
                ->setProjectDir($projectDir)
                ->addActivePlugins('CtStaticAnalysis')
                ->getClassLoader();

            static::assertSame([$pluginPath . '/tests/'], $classLoader->getPrefixesPsr4()['CtStaticAnalysis\\Tests\\']);
        } finally {
            KernelLifecycleAccessor::setKernel($previousKernel);
        }
    }
