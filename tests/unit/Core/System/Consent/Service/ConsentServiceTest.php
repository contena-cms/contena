...
class ConsentServiceTest extends TestCase
{
    ...
    public function testListSkipsDefinitionsWithUnresolvableScope(): void
    {
        $service = $this->createService(null, [
            new TestDefinition('consent-1', ConsentScope\System::NAME),
            new TestDefinition('frontend-consent', ConsentScope\FrontendVisitor::NAME),
        ]);
        ...
        // The frontend visitor scope cannot be resolved from an admin context,
        // so the frontend consent must be skipped instead of failing the whole list
        $result = $service->list(Context::createDefaultContext(new AdminApiSource('user-123')));

        static::assertArrayHasKey('consent-1', $result);
        static::assertArrayNotHasKey('frontend-consent', $result);
    }
    ...
    public function testListContainsDefinitionsWithResolvableScope(): void
    {
        ...
        $result = $service->list(Context::createDefaultContext(new ChannelApiSource(Uuid::randomHex())));

        static::assertArrayHasKey('frontend-consent', $result);
        static::assertSame(ConsentScope\FrontendVisitor::IDENTIFIER, $result['frontend-consent']->identifier);
        static::assertSame(ConsentStatus::UNSET, $result['frontend-consent']->status);
    }
    ...
    private function createService(?EventDispatcher $eventDispatcher = null, array $definitions = []): ConsentService
    {
        return new ConsentService(
            [
                ...
                new AdminUser(),
                new ConsentScope\FrontendVisitor(),
            ],
