...
class SnippetExceptionTest extends TestCase
{
    ...
    public function testLanguageNotAvailableInChannel(): void
    {
        $exception = SnippetException::languageNotAvailableInChannel('language-id', 'channel-id');

        static::assertSame(Response::HTTP_BAD_REQUEST, $exception->getStatusCode());
        static::assertSame(SnippetException::SNIPPET_LANGUAGE_NOT_AVAILABLE_IN_CHANNEL, $exception->getErrorCode());
        static::assertSame(
            'The language "language-id" is not available for channel "channel-id".',
            $exception->getMessage(),
        );
    }
