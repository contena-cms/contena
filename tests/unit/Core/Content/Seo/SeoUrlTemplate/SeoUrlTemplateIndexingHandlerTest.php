...
class SeoUrlTemplateIndexingHandlerTest extends TestCase
{
    ...
    public function testProcessesFullBatchAndChainsFollowUpMessage(): void
    {
        ...
        $seoUrlUpdater->expects($this->once())
            ->method('update')
            ->with('frontend.navigation.page', $ids, static::isInstanceOf(Context::class));
    ...
    public function testPassesMessageOffsetToIteratorAndStopsOnEmptyBatch(): void
    {
        ...
        $handler = $this->createHandler($seoUrlUpdater, $iteratorFactory, $definitionRegistry, $seoUrlRouteRegistry, $messageBus);
        $handler->__invoke(new SeoUrlTemplateIndexingMessage('frontend.detail.page', 'blog', ['offset' => 4711]));
    }
    ...
    public function testProcessesHeadlessChannelApiRouteNotInTheRouteRegistry(): void
    {
        // Headless channel-api routes are tagged `contena.entity.seo_url.route` and are
        ...
        $seoUrlUpdater->expects($this->once())
            ->method('update')
            ->with('channel-api.blog.detail', $ids, static::isInstanceOf(Context::class));
        ...
        $handler = $this->createHandler(
            ...
            $seoUrlRouteRegistry,
            $messageBus,
            [$this->entitySeoUrlRoute('channel-api.blog.detail', 'blog')]
        );
        $handler->__invoke(new SeoUrlTemplateIndexingMessage('channel-api.blog.detail', 'blog'));
    }
