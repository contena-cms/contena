...
class SeoUrlTemplateChangeSubscriberTest extends TestCase
{
    ...
    public function testIgnoresInsertWithoutTemplateValue(): void
    {
        ...
        // channel; those fall back to the default template, so there is nothing
        // to regenerate.
        $connection = $this->createMock(Connection::class);
        $connection->expects($this->never())->method('fetchAllAssociative');
        $messageBus = $this->createMock(MessageBusInterface::class);
    ...
    public function testDispatchesIndexingMessageForDeletedChannelTemplate(): void
    {
        // Removing a channel override makes the channel fall back to the default
    ...
    public function testDispatchesOneMessagePerRoute(): void
    {
        ...
        $connection->method('fetchAllAssociative')->willReturn([
            ['routeName' => 'frontend.navigation.page', 'entityName' => 'category'],
            ['routeName' => 'frontend.detail.page', 'entityName' => 'blog'],
        ]);
     ...
     */
    private function deleteChangeSet(?string $channelId): ChangeSet
    {
