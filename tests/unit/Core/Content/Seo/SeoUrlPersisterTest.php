...
class SeoUrlPersisterTest extends TestCase
{
    ...
    /**
     * Regression for contena/contena#4413. An existing canonical SEO URL is only replaced when the
    ...
    private function persistSeoUrlUpdate(array $existing, array $update, bool $overwrite): array
    {
        ...
        if ($overwrite) {
            $seoUrlPersister->forceUpdateSeoUrls($context, $routeName, [$foreignKey], [$seoUrl], $channel);
        } else {
            $seoUrlPersister->updateSeoUrls($context, $routeName, [$foreignKey], [$seoUrl], $channel);
        }
