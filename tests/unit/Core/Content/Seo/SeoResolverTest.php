...
class SeoResolverTest extends TestCase
{
    ...
    public function testResolveUrlWithIsCanonical(string $pathInfo, string $expected): void
    {
        ...
        $seoResolver = new SeoResolver($this->getMockConnection($channelId, true, $pathInfo));

        $resolved = $seoResolver->resolveUrl(new SeoUrlRequestContext(Uuid::randomHex(), $channelId, $pathInfo));
    ...
    {
        $channelId = Uuid::randomHex();
        $seoResolver = new SeoResolver($this->getMockConnection($channelId, false, $pathInfo));
    ...
    public function testResolveIgnoresDeletedSeoUrls(): void
    {
        ...
        $channelId = Uuid::randomHex();
        $seoResolver = new SeoResolver($this->createSqliteConnectionWithDeletedSeoUrl($languageId, $channelId));

        $resolved = $seoResolver->resolveUrl(new SeoUrlRequestContext($languageId, $channelId, 'awesome-blog'));
    ...
    public function testResolveUrlWithQueryStringReturnsCanonical(): void
    {
        ...
        $seoResolver = new SeoResolver($this->getMockConnection($channelId, true, $expectedPathInfo));
    ...
    public function testResolveUrlWithoutQueryStringPrefersPlainCanonical(): void
    {
        ...
        $resolved = $seoResolver->resolveUrl(new SeoUrlRequestContext(Uuid::randomHex(), $channelId, 'Main-blog/SWDEMO10001'));
    ...
    public function testResolveUrlWithFlagQueryStringDoesNotSetCanonicalPathInfo(): void
    {
        ...
        $firstResult = FakeResultFactory::createResult([
            [
                ...
                'seoPathInfo' => 'Latest-Blog/SW10005?test12345',
            ],
        ], $connection);
        $secondResult = FakeResultFactory::createResult([], $connection);
    ...
    {
        $channelId = Uuid::randomHex();
        $storedSeoPath = 'Latest-Blog/SW10005?test12345';
    ...
    public function testResolveUrlFallsBackToPlainPathWhenNoRowMatches(): void
    {
        ...
        static::assertSame('/blog-a', $resolved->pathInfo);
    ...
    public function testResolveUrlPrefersCanonicalRowWhoseQueryMatchesRequest(): void
    {
        ...
        // Two canonical rows for the same channel match the request: a plain row and a
        // query-bearing alternative. The query-matching row is returned SECOND, so only the
        ...
        static::assertSame('/detail/alternative', $resolved->pathInfo);
        static::assertSame('Main-blog/SWDEMO10001?test=123', $resolved->seoPathInfo);
        static::assertTrue($resolved->isCanonical);
    }
    ...
    public function testResolveUrlFallbackFindsCanonicalSiblingWhenFirstHitIsNotCanonical(): void
    {
        ...
        $resolved = $seoResolver->resolveUrl(new SeoUrlRequestContext(Uuid::randomHex(), $channelId, 'old-slug'));
    ...
    private function getMockConnection(string $channelId, bool $isCanonical, string $pathInfo): Connection&Stub
    {
    ...
    private function createSqliteConnectionWithDeletedSeoUrl(string $languageId, string $channelId): Connection
    {
