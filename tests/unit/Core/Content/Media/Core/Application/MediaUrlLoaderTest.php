...
class MediaUrlLoaderTest extends TestCase
{
    ...
    public function testCallRemoteThumbnailLoader(): void
    {
        ...
        $context = Context::createTenantContext($ids->get('tenant'));
        $event = new EntityLoadedEvent(new MediaDefinition(), [$entity], $context);
        $remoteThumbnailLoader->expects($this->once())->method('load')->with($event, $context);
