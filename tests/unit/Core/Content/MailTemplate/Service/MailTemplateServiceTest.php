...
class MailTemplateServiceTest extends TestCase
{
    ...
    private MailTemplateContentBuilder&MockObject $mailTemplateContentBuilder;
    ...
    protected function setUp(): void
    {
        ...
        $this->mailTemplateContentBuilder = $this->createMock(MailTemplateContentBuilder::class);
    ...
    public function testLoadTemplate(): void
    {
        ...
        $mailTemplateRepository = StaticEntityRepository::of(
            MailTemplateCollection::class,
    ...
    public function testSimulateUsesSimulatorTemplateDataAndCollectsRenderResults(): void
    {
        ...
        $this->mailDataSimulator->expects($this->once())
            ...
            ->with('user.recovery.request', $context)
    ...
    public function testSimulateDispatchesRenderContextEvent(): void
    {
        ...
        $eventDispatcher->expects($this->once())
            ->method('dispatch')
            ->willReturnCallback(static function (MailTemplateRenderContextEvent $event) use ($context): MailTemplateRenderContextEvent {
    ...
    public function testPreviewCanIncludeHeaderFooter(): void
    {
        ...
        $this->mailTemplateContentBuilder->expects($this->once())
            ->method('build')
            ->with([
                'contentPlain' => 'plain',
            ...
            ->willReturn([
                'contentPlain' => 'H {{ foo }} plain F {{ foo }}',
                'contentHtml' => '<header>{{ foo }}</header><p>html</p><footer>{{ foo }}</footer>',
            ]);
    ...
    public function testPreviewDispatchesRenderContextEvent(): void
    {
        ...
        $eventDispatcher->expects($this->once())
            ...
            ->willReturnCallback(static function (MailTemplateRenderContextEvent $event) use ($context): MailTemplateRenderContextEvent {
                ...
                $event->addTemplateData('frontendContext', 'context');
        ...
        $this->templateRenderer->expects($this->exactly(4))
            ->method('render')
            ->willReturnCallback(
                static function (string $value, array $templateData): string {
                    static::assertSame([
                        'foo' => 'bar',
    ...
    public function testAvailableVariables(string $fieldPath, array $expected): void
    {
        $this->mailDataSimulator->expects($this->once())
            ->method('getTemplateData')
            ->with('user.recovery.request', static::isInstanceOf(Context::class))
            ->willReturn([
                'foo' => 'value',
                'bar' => [
                    'foobar' => 'value',
        ...
        $result = $mailTemplateService->getAvailableVariables('user.recovery.request', Context::createDefaultContext(), $fieldPath);
    ...
    private function createService(
        ...
        $mailTemplateRepository ??= StaticEntityRepository::of(MailTemplateCollection::class);
