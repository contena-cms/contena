...
class FinishControllerTest extends TestCase
{
    ...
    public function testFinishLocksSystemAndRedirectsWithCookie(): void
    {
        ...
        static::assertSame('https://www.contena.cn/admin', $response->headers->get('Location'));
    ...
    public function testFinishUsesCustomAdminPathNameForRedirectAndCookiePath(): void
    {
        ...
        $controller = $this->createController($client, 'https://example.com/contena', 'custom-admin');

        $response = $controller->finish($request);

        static::assertSame('https://example.com/contena/custom-admin', $response->headers->get('Location'));

        $cookies = $response->headers->getCookies();
        static::assertCount(1, $cookies);

        static::assertSame('/contena/custom-admin', $cookies[0]->getPath());
    }
    ...
    private function createController(
        Client $client,
        string $appUrl = 'https://www.contena.cn',
