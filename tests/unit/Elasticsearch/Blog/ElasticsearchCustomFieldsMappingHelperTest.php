...
class ElasticsearchCustomFieldsMappingHelperTest extends TestCase
{
    ...
    {
        $indexDetector = $this->createMock(ElasticsearchOutdatedIndexDetector::class);
        $indexDetector->expects($this->once())->method('getAllUsedIndices')->willReturn(['sw_blog_index']);
        ...
        $indices->expects($this->once())
            ...
            ->with(static::callback(static function (array $params) {
                return $params['index'] === 'sw_blog_index'
    ...
    public function testCreateFieldsInIndicesThrowsOnTypeChange(): void
    {
        ...
        $indices = $this->createMock(IndicesNamespace::class);
        $indices->expects($this->once())->method('get')->willReturn(['sw_blog_index' => ['mappings' => []]]);
        $indices->expects($this->once())
            ->method('putMapping')
        ...
        $this->expectExceptionObject(ElasticsearchBlogException::cannotChangeCustomFieldType(new BadRequestHttpException('')));
