...
Users, access keys, personal configurations, password recoveries, and user tag assignments now retain tenant ownership. Usernames, email addresses, and generated user codes are unique within each tenant or the platform instead of globally. Password login without a resolved tenant only considers platform users; on a tenant domain it prefers that tenant's user and falls back to a platform user when no tenant user has the same username.
