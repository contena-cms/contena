...
`AbstractPaymentRouteResolver::resolve()` now accepts `PaymentRoutingRequest`. Use tagged route providers/selection strategies or `PaymentRouteCandidateEvent` for routing customization. `PaymentRouteResolvedEvent` is an observation of the selected route, not a mutable route override. Provider-notification inputs/results moved from `Struct` to `Notification\Struct`, and `PaymentNotificationTypes` moved to `Notification`; update gateway handler imports accordingly. `Struct\PaymentRoute` moved to `Routing\PaymentRoute`.

`PaymentOrderConverter::convert()` returns order data directly, without `order`, `orderId`, or `transactionId` wrappers or embedded execution records. Modify the converted array through `Event\PaymentOrderConvertedEvent`. IDs, numbers and initial execution records are managed by persistence. Order creation events now occur before the explicit creation of the primary execution record; the primary record exists before provider I/O.

Payment queries reconcile the existing primary `payment_order_transaction`; they no longer create query audit rows. The unused `payment_order_transaction.type` and `payment_order_transaction.operator` DAL fields were removed, and their columns are removed by the destructive migration. Consumers that counted query rows or inspected `type = create|query` must use gateway lifecycle events for technical-call telemetry and treat the transaction as the provider-facing payment attempt.

Gateway capability interfaces now return `Struct\GatewayResult`. It contains the normalized status, optional `PaymentAction`, and `GatewayResponse`; provider identifiers, result codes/messages and raw data moved into that response object. Business service contracts still return `Struct\PaymentResult`, now containing the platform number, external number, the `GatewayResult`, and an optional payment transaction number. Replace direct gateway-field access and calls to the removed `PaymentResult::withResource()` accordingly.
...
Webhook delivery now uses a dedicated `webhook` Messenger transport. Add it to your `messenger:consume` receiver list and to `contena.admin_worker.transports` if you override that key.
...
If you override `contena.admin_worker.transports`, prepend `webhook`:
...
The PHP methods `Contena\Storefront\Framework\Twig\Extension\ConfigExtension::config()` and `Contena\Storefront\Framework\Twig\TemplateConfigAccessor::config()` were removed.
Use `Contena\Core\System\SystemConfig\SystemConfigService` directly in PHP code.
...
The Agentic Commerce sales channel features — including product export providers, sales channel tracking, and related classes — have been removed from Contena's core and are no longer available out of the box.

> Install the **Agentic Commerce extension (CtAgenticCommerce)** from the Contena Store **before** updating to 6.8 to retain this functionality and preserve any already configured Agentic Commerce sales channels.

## Document rendering no longer falls back to the Storefront browser timezone

When no Sales Channel business timezone is configured, document rendering no longer uses the Storefront browser timezone in Contena 6.8. Documents now render with Twig's configured default timezone (`UTC` unless changed via `twig.date.timezone`) regardless of how they are generated. Set the Sales Channel business timezone if documents should use a merchant-controlled timezone.
...
When calling `/api/_action/mail-template/send`, arbitrary unknown top-level payload keys are no longer forwarded to the mail service in Contena 6.8.
...
The Store API route `/store-api/document/download` returns now a standard Contena domain exception with status code `404` and the code `DOCUMENT_FILETYPE_UNAVAILABLE` when the document has no generated document with the requested mime type, instead of returning a `204` status code.
...
## Administrator security settings page removed

The generic **Settings > Administrator security** page and its `src/Core/System/Resources/config/userPermission.xml` configuration page have been removed. Extensions and operators that still need to change the Administration minimum password length should use the `core.userPermission.passwordMinLength` system configuration key through the Admin API; its default value remains 8.

## `country_state` replaced by hierarchical `region` entities

The flat `country_state` and `country_state_translation` DAL entities and their PHP classes have been removed. Use `region` and `region_translation` instead. The new model supports arbitrary hierarchy depth through `parentId`, `level`, `path`, `childCount`, and the `parent` / `children` associations.

Update integrations and extensions as follows:

- replace the `country_state` entity name and `/api/country-state` endpoints with `region` and `/api/region`
- replace the `country.states` association with `country.regions`
- use `region.countryId` to scope a hierarchy to a country and `region.parentId` to navigate levels
- assign custom-field sets to the `region` entity name
- replace Administration ACL privileges beginning with `country_state.` with `region.viewer`, `region.editor`, `region.creator`, and `region.deleter`

The legacy division import commands and temporary `country_state_import` entity have also been removed. Region fixtures must be imported through DAL writes or migrations.

## Country address-format and unused document templates removed

The unused translated `country.addressFormat` field, `CountryDefinition::DEFAULT_ADDRESS_FORMAT`, and the bundled Core document/address rendering Twig templates have been removed. Extensions must provide and own any document or address formatting templates they need instead of extending files below `Core/Framework/Resources/views/documents` or `Core/Framework/Resources/views/snippets`.
...
The execution orchestration of `Contena\Core\Framework\MessageQueue\ScheduledTask\ScheduledTaskHandler::__invoke()` (loading the task, marking it running or failed, and rescheduling it) was moved into the new `Contena\Core\Framework\MessageQueue\ScheduledTask\ScheduledTaskExecutor` service.
...
If you previously overrode `rescheduleTask()` to compute a custom next execution time, implement the `Contena\Core\Framework\MessageQueue\ScheduledTask\DynamicallyScheduledTaskHandler` interface instead. The executor asks the handler for the next execution time and persists it for you — the handler only answers the "when", not the "how":
...
```
## Removal of `contena.cache.cache_compression` and `contena.cache.cache_compression_method` config options

The deprecated `contena.cache.cache_compression` and `contena.cache.cache_compression_method` configuration options were removed. Please use the new `contena.cache.compress` and `contena.cache.compression_method` options instead.
...
Contena no longer requires the following Composer packages:
...
`Contena\Core\System\NumberRange\ValueGenerator\NumberRangeValueGeneratorInterface` was removed.
Use `Contena\Core\System\NumberRange\ValueGenerator\AbstractNumberRangeValueGenerator` instead.
...
The default write scopes of `Contena\Core\Framework\DataAbstractionLayer\Field\CreatedByField` and `Contena\Core\Framework\DataAbstractionLayer\Field\UpdatedByField` now include `Context::CRUD_API_SCOPE` in addition to `Context::SYSTEM_SCOPE`.
...
The blocked-promotion handling in `\Contena\Core\Checkout\Promotion\Cart\Extension\CartExtension` has been removed.

## Removal of `$options` parameter in custom validator's constraints

The `$options` of all Contena's custom validator constraint are removed, if you use one of them, please use named argument instead
...
```
\Contena\Core\Checkout\Customer\Validation\Constraint\CustomerEmailUnique
\Contena\Core\Checkout\Customer\Validation\Constraint\CustomerPasswordMatches
\Contena\Core\Checkout\Customer\Validation\Constraint\CustomerVatIdentification
\Contena\Core\Checkout\Customer\Validation\Constraint\CustomerZipCode
\Contena\Core\Framework\DataAbstractionLayer\Validation\EntityExists
\Contena\Core\Framework\DataAbstractionLayer\Validation\EntityNotExists
```

## Removal of `StoreApiRouteCacheKeyEvent` and `StoreApiRouteCacheTagsEvent` and all it's child classes

With the removal of the separate Store-API caching layer with Contena 6.7, those events where not used and emitted anymore, therefore we are removing them now without any replacement.

The concrete events being removed:
- `\Contena\Core\Framework\Adapter\Cache\StoreApiRouteCacheKeyEvent`
- `\Contena\Core\Framework\Adapter\Cache\StoreApiRouteCacheTagsEvent`
- `\Contena\Core\Content\Category\Event\CategoryRouteCacheKeyEvent`
- `\Contena\Core\Content\Category\Event\CategoryRouteCacheTagsEvent`
- `\Contena\Core\System\Country\Event\CountryRouteCacheKeyEvent`
- `\Contena\Core\System\Country\Event\CountryRouteCacheTagsEvent`
- `\Contena\Core\System\Country\Event\CountryStateRouteCacheKeyEvent`
- `\Contena\Core\System\Country\Event\CountryStateRouteCacheTagsEvent`
- `\Contena\Core\Content\Product\Events\CrossSellingRouteCacheKeyEvent`
- `\Contena\Core\Content\Product\Events\CrossSellingRouteCacheTagsEvent`
- `\Contena\Core\System\Currency\Event\CurrencyRouteCacheKeyEvent`
- `\Contena\Core\System\Currency\Event\CurrencyRouteCacheTagsEvent`
- `\Contena\Core\Content\LandingPage\Event\LandingPageRouteCacheKeyEvent`
- `\Contena\Core\Content\LandingPage\Event\LandingPageRouteCacheTagsEvent`
- `\Contena\Core\System\Language\Event\LanguageRouteCacheKeyEvent`
- `\Contena\Core\System\Language\Event\LanguageRouteCacheTagsEvent`
- `\Contena\Core\Content\Category\Event\NavigationRouteCacheKeyEvent`
- `\Contena\Core\Content\Category\Event\NavigationRouteCacheTagsEvent`
- `\Contena\Core\Checkout\Payment\Event\PaymentMethodRouteCacheKeyEvent`
- `\Contena\Core\Checkout\Payment\Event\PaymentMethodRouteCacheTagsEvent`
- `\Contena\Core\Content\Product\Events\ProductDetailRouteCacheKeyEvent`
- `\Contena\Core\Content\Product\Events\ProductDetailRouteCacheTagsEvent`
- `\Contena\Core\Content\Product\Events\ProductListingRouteCacheKeyEvent`
- `\Contena\Core\Content\Product\Events\ProductListingRouteCacheTagsEvent`
- `\Contena\Core\Content\Product\Events\ProductSearchRouteCacheKeyEvent`
- `\Contena\Core\Content\Product\Events\ProductSearchRouteCacheTagsEvent`
- `\Contena\Core\Content\Product\Events\ProductSuggestRouteCacheKeyEvent`
- `\Contena\Core\Content\Product\Events\ProductSuggestRouteCacheTagsEvent`
- `\Contena\Core\System\Salutation\Event\SalutationRouteCacheKeyEvent`
- `\Contena\Core\System\Salutation\Event\SalutationRouteCacheTagsEvent`
- `\Contena\Commercial\AISearch\ImageUploadSearch\Event\SearchTerm\SearchTermRouteCacheKeyEvent`
- `\Contena\Commercial\AISearch\ImageUploadSearch\Event\SearchTerm\SearchTermRouteCacheTagsEvent`
- `\Contena\Commercial\AISearch\NaturalLanguageSearch\Event\SearchTerm\SearchTermRouteCacheKeyEvent`
- `\Contena\Commercial\AISearch\NaturalLanguageSearch\Event\SearchTerm\SearchTermRouteCacheTagsEvent`
- `\Contena\Core\Checkout\Shipping\Event\ShippingMethodRouteCacheKeyEvent`
- `\Contena\Core\Checkout\Shipping\Event\ShippingMethodRouteCacheTagsEvent`
- `\Contena\Core\Content\Sitemap\Event\SitemapRouteCacheKeyEvent`
- `\Contena\Core\Content\Sitemap\Event\SitemapRouteCacheTagsEvent`

## Theme Configuration Changes

As part of optimizing theme configuration loading, several changes are being made to the theme system:

* The `\Contena\Storefront\Theme\CachedResolvedConfigLoader` has been removed. This class was previously used to cache theme configurations but has been replaced by a more efficient database-based solution using the new `theme_runtime_config` table.
* The `\Contena\Storefront\Theme\Exception\ThemeAssignmentException` has been removed. Instead, use `\Contena\Storefront\Theme\Exception\ThemeException::themeAssignmentException` for handling theme assignment errors.
* The `\Contena\Storefront\Theme\ThemeLifecycleService` is now marked as final and cannot be extended. Additionally, its `refreshTheme` method now accepts an optional `$configurationCollection` parameter.

## `filterByActiveRules` in Payment- and ShippingMethodCollection removed

The `filterByActiveRules` methods in `Contena\Core\Checkout\Payment\PaymentMethodCollection` and `Contena\Core\Checkout\Shipping\ShippingMethodCollection` were removed.
Use the new `Contena\Core\Framework\Rule\RuleIdMatcher` instead.
...
## Removal of helper methods in `\Contena\Core\Framework\DataAbstractionLayer\Dbal\EntityDefinitionQueryHelper`

Following helper methods have been removed from the `EntityDefinitionQueryHelper`:
- \Contena\Core\Framework\DataAbstractionLayer\Dbal\EntityDefinitionQueryHelper::columnExists
- \Contena\Core\Framework\DataAbstractionLayer\Dbal\EntityDefinitionQueryHelper::columnIsNullable
- \Contena\Core\Framework\DataAbstractionLayer\Dbal\EntityDefinitionQueryHelper::tableExists

## Behavior change in migration helper traits for missing tables

`\Contena\Core\Framework\Migration\ColumnExistsTrait::columnExists` no longer throws a `\Doctrine\DBAL\Exception\TableNotFoundException` when the given table does not exist — it returns `false` instead.
`\Contena\Core\Framework\Migration\AddColumnTrait::addColumn` still throws a `\Doctrine\DBAL\Exception\TableNotFoundException` for missing tables (from the executed `ALTER TABLE` statement).

## Cache improvements

### Only rules relevant for product prices are considered in the `ct-cache-hash`

In the default Contena setup the `ct-cache-hash` cookie will only contain rule ids which are used to alter product prices, in contrast to previous all active rules, which might only be used for a promotion.

If the Storefront content changes depending on a rule, the corresponding rule ids should be added using the extension `Contena\Core\Framework\Adapter\Cache\Http\Extension\ResolveCacheRelevantRuleIdsExtension`.
...
If some custom entity has a relation to a rule, which might alter the storefront, you should add them to either an existing area, or your own are using the DAL flag `Contena\Core\Framework\DataAbstractionLayer\Field\Flag\RuleAreas` on the rule association.

### Removed unused `RuleAreas` constants

The constants `Contena\Core\Framework\DataAbstractionLayer\Field\Flag\RuleAreas::{CATEGORY_AREA,LANDING_PAGE_AREA}` are not used anymore and will therefore be removed

### Removed `ct-states` and `ct-currency` cache cookie handling

The `ct-states` and `ct-currency` cache cookie handling is removed, which means by default the HTTP-Cache is also active for logged in customers or when the cart is filled.
Due to the rework of the contained rules in the cache hash (see above), this becomes efficiently possible.
The complete caching behaviour is now controlled by the `ct-cache-hash` cookie.

You should rework your extensions to also work with enabled cache for logged in customers and when the cart is filled.
To modify the default behaviour there are several extension points you can hook into, for a detailed explanation please take a look at the [caching docs](https://developer.contena.cn/docs/guides/plugins/plugins/framework/caching/#manipulating-the-cache-key).

The following classes and constants were removed as they are no longer used:
  * `\Contena\Core\Framework\Adapter\Cache\Http\CacheStateValidator`
  * `\Contena\Core\Framework\Adapter\Cache\CacheStateSubscriber`
  * `\Contena\Core\Framework\Adapter\Cache\Http\HttpCacheKeyGenerator::SYSTEM_STATE_COOKIE`
  * `\Contena\Core\Framework\Adapter\Cache\Http\HttpCacheKeyGenerator::INVALIDATION_STATES_HEADER`
  * `\Contena\Core\Framework\Adapter\Cache\Http\HttpCacheKeyGenerator::CURRENCY_COOKIE`
  * `\Contena\Core\Framework\Adapter\Cache\CacheStateSubscriber::STATE_LOGGED_IN`
  * `\Contena\Core\Framework\Adapter\Cache\CacheStateSubscriber::STATE_CART_FILLED`

Additionally, the following configuration was removed:
* `contena.cache.invalidation.http_cache`
...
The properties `$mediaPath` and `$mediaUpdatedAt` from `Contena\Core\Content\Media\Extension\ResolveRemoteThumbnailUrlExtension` were removed.
Set the values directly into the `mediaEntity` property.

## Improved fetching of language information for SalesChannelContext

The `\Contena\Core\System\SalesChannel\Context\BaseSalesChannelContextFactory` now uses the language repository directly to fetch language information.
...
The method `\Contena\Core\Checkout\Document\Zugferd\ZugferdDocument::getPrice()` was removed, replace calls to `ZugferdDocument::getPrice()` with `ZugferdDocument::getPriceWithFallback()`.

## Removed `TaskScheduler::getNextExecutionTime()`

The `\Contena\Core\Framework\MessageQueue\ScheduledTask\Scheduler\TaskScheduler::getNextExecutionTime()` method was not used anymore and was removed.
...
The class `\Contena\Core\Framework\MessageQueue\ScheduledTask\MessageQueue\RegisterScheduledTaskMessage` and it's accompanying handler `\Contena\Core\Framework\MessageQueue\ScheduledTask\MessageQueue\RegisterScheduledTaskHandler` were removed, as the message was no longer dispatched.
...
```diff
 ...
 use Contena\Core\Content\Product\ProductDefinition;
...
Please use granular permissions instead, a list of them can be found in `Contena\Core\Checkout\CheckoutPermissions`.
...
The method `\Contena\Core\Content\Category\SalesChannel\NavigationRoute::buildName()` was removed, navigation routes are now only tagged with `NavigationRoute::ALL`.

## Remove method Contena\Core\Content\Seo\SalesChannel\SeoResolverData::get

The method `Contena\Core\Content\Seo\SalesChannel\SeoResolverData::get` was removed as it's no longer used because it only returns the first entity found, which can lead to inconsistencies when multiple items share the same entity and identifier.
A new method `Contena\Core\Content\Seo\SalesChannel\SeoResolverData::getAll` was introduced which returns all items with the given entity and identifier.
...
- The `$label` property and the following methods in `Contena\Core\Content\ImportExport\ImportExportProfileEntity` have been removed:
  - `getLabel()`
  - `setLabel()`
  - `getTranslations()`
  - `setTranslations()`
- The following classes have been removed:
  - `Contena\Core\Content\ImportExport\ImportExportProfileTranslationCollection`
  - `Contena\Core\Content\ImportExport\ImportExportProfileTranslationDefinition`
  - `Contena\Core\Content\ImportExport\ImportExportProfileTranslationEntity`
- The `importExportProfileTranslations` association has been removed from `Contena\Core\System\Language\LanguageDefinition`, and the following methods in `Contena\Core\System\Language\LanguageEntity` have been removed:
  - `getImportExportProfileTranslations()`
  - `setImportExportProfileTranslations()`
- `createLog()` and `getConfig()` in `Contena\Core\Content\ImportExport\Service\ImportExportService` now use `$technicalName` instead of `$label` when generating filenames.
- `generateFilename()` in `Contena\Core\Content\ImportExport\Service\FileService` now uses `$technicalName` instead of `$label` as profile name.

## ApiClient confidential flag

* You must explicitly pass a boolean value to the `confidential` parameter  of `\Contena\Core\Framework\Api\OAuth\Client\ApiClient`.
...
* `\Contena\Core\Content\ImportExport\Exception\LogNotWritableException`
* `\Contena\Core\Content\ImportExport\Exception\MappingException`

## SystemConfig is global-only

Core SystemConfig no longer defines `scope_type` / `scope_id` storage columns or accepts scope arguments in its PHP, Admin API, CLI, MCP, event, and Administration component contracts. Static configuration under `contena.system_config` is now a flat key-value map.

Extensions that need contextual configuration must own that domain contract. A plugin may add a nullable foreign key such as `sales_channel_id` through a DAL `EntityExtension`, provide the matching migration and constraints, and define its own contextual loader/service interface and inheritance rules. That implementation can compose the global `AbstractSystemConfigLoader` as its fallback. Global-only loading can be replaced through the `AbstractSystemConfigLoader` service alias or extended by decorating the default `SystemConfigLoader` chain. Core does not interpret plugin-owned contextual fields.

## SystemConfigService: `$silent` parameter changed default value from `false` to `true`

`SystemConfigService::set()`, `setMultiple()`, and `delete()` changed the default value for the `$silent` parameter from `false` to `true`, meaning config writes **no longer invalidate the HTTP cache** (`system.config-global` tag) by default. The internal config cache (`system-config`) is always cleared regardless.
...
```php
$this->systemConfigService->set('MyPlugin.config.showBanner', true, false);
```
...
* `\Contena\Core\System\SystemConfig\Exception\InvalidDomainException`
* `\Contena\Core\System\SystemConfig\Exception\InvalidKeyException`
* `\Contena\Core\System\SystemConfig\Exception\InvalidSettingValueException`

Use the respective factory methods in `\Contena\Core\System\SystemConfig\SystemConfigException` instead.

## Removal of SystemConfigService tracing methods

The methods `\Contena\Core\System\SystemConfig\SystemConfigService::trace()` and `\Contena\Core\System\SystemConfig\SystemConfigService::getTrace()` were removed.
...
`Contena\Core\Checkout\Cart\Price\Struct\FilterableInterface`, which defines the required `getFilter()` method.
...
The `$result` property of `Contena\Core\Content\Cms\Events\CmsPageLoadedEvent` now enforces the `Contena\Core\Content\Cms\CmsPageCollection` type instead of the generic `Contena\Core\Framework\DataAbstractionLayer\EntityCollection`.
...
## Removal of `\Contena\Core\Framework\Test\TestCaseHelper\ReflectionHelper`

Refection has significantly improved in particular since PHP 8.1, therefore the `Contena\Core\Framework\Test\TestCaseHelper\ReflectionHelper` was removed, see below for the explicit replacements:
...
`Contena\Core\Checkout\Cart\Error\ErrorRoute` is specific to the standard Storefront and therefore should not be in the Core package.
At the same time, the Storefront does not properly use this class.
Therefore, the class, and the `route` property of `Contena\Core\Checkout\Cart\Error\CartError` have been removed.

## Removal of string parameter in `DomainRuleStruct` constructor

The deprecated string parameter in the `Contena\Storefront\Page\Robots\Struct\DomainRuleStruct` constructor was removed.
If your plugin or theme instantiates `DomainRuleStruct` with a string parameter, it will no longer work.
Use `Contena\Storefront\Page\Robots\Parser\RobotsDirectiveParser::parse()` to create a `ParsedRobots` object instead.
...
The `$states` property in `Contena\Core\Framework\Adapter\Cache\Http\CacheAttribute` is removed.
...
`\Contena\Core\Framework\Script\Api\ResponseCacheConfiguration::invalidationState()` were removed with no replacement.
...
The deprecated class `\Contena\Core\Content\Seo\SeoUrlTemplate\TemplateGroup` has been removed.

**Removed components:**

- `IncrementGatewayRegistry::MESSAGE_QUEUE_POOL` constant and related `message_queue` increment
- `contena.admin_worker.enable_queue_stats_worker` configuration option
- `contena.increment.message_queue` configuration section
...
The following events now require `Context` as the last constructor parameter and implement `ContenaEvent`.
...
- `Contena\Core\Content\ImportExport\Event\EnrichExportCriteriaEvent`
- `Contena\Core\Content\ImportExport\Event\ImportExportBeforeExportRecordEvent`
- `Contena\Core\Content\ImportExport\Event\ImportExportExceptionImportExportHandlerEvent`
- `Contena\Core\Content\Seo\Event\SeoUrlUpdateEvent`
- `Contena\Core\Content\Media\Event\MediaFileExtensionWhitelistEvent`
- `Contena\Core\Content\Media\Event\UnusedMediaSearchEvent`
- `Contena\Storefront\Theme\Event\ThemeAssignedEvent`
- `Contena\Storefront\Theme\Event\ThemeConfigChangedEvent`
- `Contena\Storefront\Theme\Event\ThemeConfigResetEvent`
...
* `\Contena\Core\System\NumberRange\Exception\IncrementStorageNotFoundException` -> `\Contena\Core\System\NumberRange\Exception\NumberRangeException::incrementStorageNotFound()`
* `\Contena\Core\System\NumberRange\Exception\NoConfigurationException` -> `\Contena\Core\System\NumberRange\NumberRangeException::noConfigurationForEntity()`

### Removed non-used `MAIL_TEMPLATE_SALES_CHANNEL_*_EVENT` constants

Removed the constants `Contena\Core\Content\MailTemplate\MAIL_TEMPLATE_SALES_CHANNEL_{WRITTEN,DELETED,LOADED,SEARCH_RESULT_LOADED,AGGREGATION_LOADED,ID_SEARCH_RESULT_LOADED}_EVENT` as the entity has been removed with Contena 6.5 and the events were not fired anymore.

## `render()` removed from the core script `response` service

`Contena\Core\Framework\Script\Api\ScriptResponseFactoryFacade::render()` has been removed.
Rendering Storefront templates from scripts is only available in Storefront script hooks (the `/storefront/script/{hook}` endpoint), where the `response` service is provided by `Contena\Storefront\Framework\Script\Api\StorefrontScriptResponseFactoryFacade`.

Type the script `response` service for the hook you implement:
use `Contena\Core\Framework\Script\Api\ScriptResponseFactoryFacade` for admin-api and store-api hooks and return JSON or redirects there;
use `Contena\Storefront\Framework\Script\Api\StorefrontScriptResponseFactoryFacade` for Storefront hooks that render Twig templates.

```twig
{# admin-api and store-api hooks #}
{# @var services.response \Contena\Core\Framework\Script\Api\ScriptResponseFactoryFacade #}

{# Storefront hooks #}
{# @var services.response \Contena\Storefront\Framework\Script\Api\StorefrontScriptResponseFactoryFacade #}
```
...
* Before: `Contena\Core\Framework\DataAbstractionLayer\Dbal\Exception\UnmappedFieldException`
* After: `Contena\Core\Framework\DataAbstractionLayer\Exception\UnmappedFieldException`

Update your `use` and `catch` statements accordingly:

```php
// Before
use Contena\Core\Framework\DataAbstractionLayer\Dbal\Exception\UnmappedFieldException;

// After
use Contena\Core\Framework\DataAbstractionLayer\Exception\UnmappedFieldException;
```

## `AbstractTranslationLoader::pluginTranslationExists()` removed

The locale-agnostic method `pluginTranslationExists(Plugin $plugin): bool` has been removed from `Contena\Core\System\Snippet\Service\AbstractTranslationLoader`.
...
* `\Contena\Core\System\SalesChannel\SalesChannelEntity`: `getMaintenanceIpWhitelist()` / `setMaintenanceIpWhitelist()` were removed. Use `getMaintenanceIpAllowlist()` / `setMaintenanceIpAllowlist()` instead.
...
* `\Contena\Core\SalesChannelRequest`: the constant `ATTRIBUTE_SALES_CHANNEL_MAINTENANCE_IP_WHITLELIST` was removed. Use `ATTRIBUTE_SALES_CHANNEL_MAINTENANCE_IP_ALLOWLIST` instead.
* `\Contena\Core\Framework\Adapter\Kernel\HttpCacheKernel`: the constant `MAINTENANCE_WHITELIST_HEADER` was removed. Use `MAINTENANCE_ALLOWLIST_HEADER` instead.
...
## Administration extension tooling is enabled by default

Contena 6.8 provides one shared TypeScript and ESLint toolchain for installed Administration extensions. The `setup` Composer workflow now generates the root projections and per-extension bridges automatically. Run `composer admin:setup-extension-tooling:check` to detect configuration drift and `composer admin:check-extensions` to type-check and lint installed extensions.

Extension Administration projects should extend their generated `.contena/tsconfig.json` and compose `.contena/eslint.mjs` instead of maintaining copies of Contena's compiler and lint configuration. Generated root `tsconfig.json`, `eslint.config.mjs`, and `.zed/` projections are local build artifacts and must not be committed.

## Administration component templates are Vue SFC-only

Administration component templates now use native `.vue` SFCs and the native `ct-block` extension system exclusively. The TwigJS component runtime, `Contena.Template`, `.html.twig` component templates, and the Twig-to-Vue lint/test/codemod tooling have been removed.

Extensions must migrate Administration components to native `<script setup>` SFCs. Base components declare their supported extension state with one top-level `ctDefinePublic({ ... })`; `.override.vue` components consume `useCtPreviousState()` and declare replacements with one top-level `ctDefineOverride({ ... })`. The build transform owns the internal runtime lowering; extension authors must not add `createExtendableSetup` wrappers or `ComponentPublicApiMapping` entries. Replace Twig block overrides with named `<ct-block>` / `<ct-block-parent>` extension points. This does not affect server-rendered Twig templates outside the Administration component runtime, including the Administration bootstrap shell and Frontend templates.
...
#### ct-cms-el-config-buy-box.html.twig
...
#### ct-cms-el-config-cross-selling.html.twig
...
#### ct-cms-el-config-product-box.html.twig
...
#### ct-cms-el-config-product-description-reviews.html.twig
...
#### ct-cms-el-config-product-slider.html.twig
...
#### ct-product-cross-selling-assignment.html.twig
...
Starting with Contena 6.7, core components are gradually being migrated from Options API to Composition API using `createExtendableSetup()`. When a component you override has been converted, a backward-compatibility shim keeps your existing `Contena.Component.override()` call working — but logs a deprecation warning. In Contena 6.8, all fully-migrated components will require the new `overrideComponentSetup()` API.

This guide shows how to migrate your plugin override to `Contena.Component.overrideComponentSetup()` so it works natively against Composition API components.

> **Note:** Only migrate overrides for components that have already been converted to use `createExtendableSetup()`. If the target component still uses Options API, keep using `Contena.Component.override()` as-is.
...
import type SwProductList from 'src/module/ct-product/page/ct-product-list';

Contena.Component.overrideComponentSetup<typeof SwProductList>()(
    'ct-product-list',
    (previousState, props) => {
...
| `components` / `directives` | Register globally via `Contena.Component.register()` / `Contena.Directive.register()` |
...
## Removal of `loadConfigSettingGroups()` in `ct-product-detail-variants`
...
## Removal of `items` prop in `ct-entity-listing` component

The `items` prop in the `ct-entity-listing` component has been removed.
Please use the `dataSource` prop instead to align with the parent `ct-data-grid` component.
...
## Axios 1 is the only Administration HTTP client

The Axios 0 client and dual-client dispatcher have been removed. The injected Administration HTTP client always uses Axios 1.

- Remove the `useAxiosV1` request option; it is no longer supported.
- Replace `CancelToken` with `AbortController` and pass its `signal` to the request.
- Replace access to `interceptorsV0`, `interceptorsV1`, `defaultsV0`, or `defaultsV1` with the standard `interceptors` and `defaults` properties.
- Review error handling for Axios 1 error codes such as `ERR_CANCELED`.

See `src/Administration/Resources/app/administration/technical-docs/09-security/axios-migration-guide.md` for current examples.

## Removal of `ct-empty-state`

The old `ct-empty-state` component has been removed.
Please use the new `mt-empty-state` component instead.

Before:
```html
<ct-empty-state title="short title" subline="longer subline" />
```
After:
```html
...
* The `$tc` function has been completely removed
...
- The following Twig blocks have been removed:
  - `sw_import_export_edit_profile_general_container_name` (`ct-import-export-edit-profile-general.html.twig`)
  - `sw_import_export_view_profile_profiles_listing_column_label` (`ct-import-export-view-profiles.html.twig`)
  - `sw_import_export_language_switch` (`ct-import-export.html.twig`)

## Removed admin notification entity + related classes

You should update your code to reference the new classes:

* `Contena\Core\Framework\Notification\NotificationCollection`
* `Contena\Core\Framework\Notification\NotificationDefinition`
* `Contena\Core\Framework\Notification\NotificationEntity`

The old classes are removed:

* `Contena\Administration\Notification\NotificationCollection`
* `Contena\Administration\Notification\NotificationDefinition`
* `Contena\Administration\Notification\NotificationEntity`

## Removed notification controller

`\Contena\Administration\Controller\NotificationController` has been moved to core: `\Contena\Core\Framework\Notification\Api\NotificationController` - if you type hint on this class, please refactor, it is now internal.
The HTTP route is still the same. The old class has been removed.

## Removal of snippets

The following snippet keys have been removed:
* `global.ct-condition.condition.cartTaxDisplay`
* `global.ct-condition.condition.lineItemOfTypeRule`
* `global.ct-condition.condition.promotionCodeOfTypeRule`
* `global.ct-condition.condition.dayOfWeekRule`
...
## Removement of component ct-newsletter-recipient-filter-switch
`administration/src/module/ct-newsletter-recipient/component/ct-newsletter-recipient-filter-switch` are removed without replacement

## File accessibility changed from public to private
* `administration/src/module/ct-newsletter-recipient/page/ct-newsletter-recipient-list/index.js`
* `src/Storefront/Resources/app/administration/src/modules/ct-settings-storefront/index.js`
* `src/Storefront/Resources/app/administration/src/modules/ct-settings-storefront/page/ct-settings-storefront-index/index.js`

## Removal of component ct-settings-storefront-configuration
`ct-settings-storefront-configuration` is removed without replacement.
...
-   `administration/static/img/ct-login-background.png`
...
-   `administration/static/fixtures/ct-login-background.png`
-   `administration/static/fixtures/ct-test-image.png`
-   `administration/static/fixtures/ct-login-background-2.png`
-   `administration/src/module/ct-login/page/index/assets/ct-login-background.png`
-   `administration/src/module/ct-settings-usage-data/component/ct-usage-data-consent-banner/assets/data-consent-background.png`

Update image references to their `.webp` equivalents.
For example instead of `administration/static/img/ct-login-background.png` use `administration/static/img/ct-login-background.webp`

## Mail template component changes

The mail template index page now uses separate tabs for templates and headers/footers.

Changes in `ct-mail-template-list` and `ct-mail-header-footer-list`:
...
Changes in `ct-mail-template-index`:
...
- `enableQueueStatsWorker` property from `Contena.Context.app.config.adminWorker`
...
This information is no longer configured per document type in `ct-settings-document-detail`.
...
- customized the old company-information UI in `ct-settings-document-detail`
...
* `src/module/ct-settings-document/page/ct-settings-document-detail`:
...
* `src/module/ct-settings-document/page/ct-settings-document-list`
  * computed `countryRepository` was deprecated without replacement

## Mail template preview component changes

The mail template preview modal was extracted into its own Administration component: `ct-mail-template-preview-modal`.

If you extend the legacy preview footer blocks in `ct-mail-template-detail`, migrate those customizations to the new component.
The following legacy blocks are removed in Contena 6.8:
...
## Deprecated unused methods in `ct-order-document-card`

- deprecated method `documentTypeAvailable()` in `src/Administration/Resources/app/administration/src/module/ct-order/component/ct-order-document-card/index.js` without replacement
- deprecated method `invoiceExists()` in `src/Administration/Resources/app/administration/src/module/ct-order/component/ct-order-document-card/index.js` without replacement

## Removed `ct-select-base.computePath()`

The deprecated `computePath()` method on the Administration component `ct-select-base` has been removed.
...
`Contena\Storefront\Framework\Routing\AbstractDomainLoader::load()` (and the `DomainLoader` / `CachedDomainLoader` implementations) have been removed. Use `loadDomains()` instead, which returns a `Contena\Storefront\Framework\Routing\Struct\DomainCollection` of `Contena\Storefront\Framework\Routing\Struct\DomainStruct` objects, keyed by domain URL, instead of `array<string, array<string, string>>`.
...
The following exceptions were removed:
* `\Contena\Storefront\Framework\Media\Exception\MediaValidatorMissingException`
* `\Contena\Storefront\Theme\Exception\InvalidThemeBundleException`

Use the respective factory methods of the following domain exceptions instead
* `\Contena\Storefront\Framework\StorefrontFrameworkException`
* `\Contena\Storefront\Theme\Exception\ThemeException`

## Removal of DomAccess Helper

We removed DomAccess Helper, because it does not add much value compared to native browser APIs and to reduce Contena specific code complexity.
...
Additionally, you should use the Contena specific `currency`, instead of the native `format_currency` filter, to already handle configured rounding etc.
...
The `\Contena\Storefront\Framework\Cookie\CookieProviderInterface` and all its implementations were removed.
Use the `\Contena\Core\Content\Cookie\Event\CookieGroupCollectEvent` instead to register new cookie groups and cookie entries.
...
Examples:
* Tab: `ct-theme.<technicalName>.<tabName>.label`
  * e.g.: `ct-theme.ct-shape-theme.colorTab.label`
* Block: `ct-theme.<technicalName>.<tabName>.<blockName>.label`
  * e.g.: `ct-theme.ct-shape-theme.colorTab.primaryColorsBlock.label`
* Section: `ct-theme.<technicalName>.<tabName>.<blockName>.<sectionName>.label`
  * e.g.: `ct-theme.ct-shape-theme.colorTab.primaryColorsBlock.homeSection.label`
* Field:
  * `ct-theme.<technicalName>.<tabName>.<blockName>.<sectionName>.<fieldName>.label`
    * e.g.: `ct-theme.ct-shape-theme.colorTab.primaryColorsBlock.homeSection.ct-color-primary-dark.label`
  * `ct-theme.<technicalName>.<tabName>.<blockName>.<sectionName>.<fieldName>.helpText`
    * e.g.: `ct-theme.ct-shape-theme.colorTab.primaryColorsBlock.homeSection.ct-color-primary-dark.helpText`
* Options: `ct-theme.<technicalName>.<tabName>.<blockName>.<sectionName>.<fieldName>.<index>.label`
  * e.g.: `ct-theme.ct-shape-theme.colorTab.primaryColorsBlock.homeSection.ct-color-primary-dark.0.label`

## ThemeEntity::label & ThemeEntity::helpText removal

Both deprecated fields `label` & `helpText` of `Contena\Storefront\Theme\ThemeEntity` are removed.
Please use the snippet keys to be found in `\Contena\Storefront\Theme\ThemeService::getThemeConfigurationStructuredFields` instead.
...
The `\Contena\Storefront\Theme\Message\DeleteThemeFilesMessage` and its handler `\Contena\Storefront\Theme\Message\DeleteThemeFilesHandler` are removed.
Unused theme files are deleted by using the `\Contena\Storefront\Theme\ScheduledTask\DeleteThemeFilesTask` scheduled task.
...
Use `Contena\Core\Framework\App\Lifecycle\AbstractAppLifecycle::STATE_SKIP_THEME_COMPILATION` instead.
</details>
...
<custom-fields xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
               xsi:noNamespaceSchemaLocation="https://raw.githubusercontent.com/contena/contena/trunk/src/Core/System/CustomField/Schema/custom-fields-1.0.xsd">
    <custom-field-set>
        <name>ct_example_set</name>
        ...
    </custom-field-set>
...
- `CONTENA_HTTP_DEFAULT_TTL` environment variable
- `contena.http.cache.default_ttl` parameter
- `contena.http_cache.stale_while_revalidate` parameter
- `contena.http_cache.stale_if_error` parameter
...
+          cache_control:
+            public: true
+            ## replaces contena.http.cache.default_ttl parameter (and related env var)
+            s_maxage: 7200
+            # replaces contena.http_cache.stale_while_revalidate parameter
+            stale_while_revalidate: 120
+            # replaces contena.http_cache.stale_if_error parameter
+            stale_if_error: 360
+    default_policies:
+      storefront:
...
- `Contena\Core\Framework\Adapter\Cache\Http\Event\BeforeCacheControlEvent`
- `Contena\Administration\Controller\AdministrationController::CACHE_ID_HEADER` and `::CACHE_ID_ADMINISTRATION`

The `X-Contena-Cache-Id: administration` response header is therefore no longer emitted for administration responses.

Migration: A listener on `BeforeCacheControlEvent` that called `skipCacheControl()` to protect its own `Cache-Control` headers is no longer needed, because those headers are now sent as-is; remove the listener. If you matched on the `X-Contena-Cache-Id` response header (for example in a CDN or reverse-proxy rule) to detect administration responses, match on other attributes.
...
- `contena.http_cache.reverse_proxy.use_varnish_xkey`
- `contena.http_cache.reverse_proxy.ban_method`
- `contena.http_cache.reverse_proxy.ban_headers`
- `contena.http_cache.reverse_proxy.purge_all.ban_method`
- `contena.http_cache.reverse_proxy.purge_all.ban_headers`
- `contena.http_cache.reverse_proxy.purge_all.urls`
...
Contena no longer supports configuring multiple OpenSearch hosts as a comma-separated list in `OPENSEARCH_URL` or `ADMIN_OPENSEARCH_URL`.
This configuration path used the deprecated OpenSearch PHP `ClientBuilder` host pool and was only kept temporarily in 6.7 for backwards compatibility while Contena moved to the newer OpenSearch PHP client transport.
...
If you need failover or load distribution across multiple OpenSearch nodes, expose them through a single load-balanced endpoint and configure that endpoint in Contena.

## Changed default Elasticsearch shard and replica counts for Admin ES

The default values for `CONTENA_ADMIN_ES_NUMBER_OF_SHARDS` and `CONTENA_ADMIN_ES_NUMBER_OF_REPLICAS` changed from `3` to empty (meaning Elasticsearch defaults are used). If you relied on the previous defaults, set these environment variables explicitly in your `.env` file:

```
CONTENA_ADMIN_ES_NUMBER_OF_SHARDS=3
CONTENA_ADMIN_ES_NUMBER_OF_REPLICAS=3
```
...
- `contena.cache.invalidation.product_listing_route`
- `contena.cache.invalidation.product_detail_route`
- `contena.cache.invalidation.product_review_route`
- `contena.cache.invalidation.product_search_route`
- `contena.cache.invalidation.product_suggest_route`
- `contena.cache.invalidation.product_cross_selling_route`
- `contena.cache.invalidation.payment_method_route`
- `contena.cache.invalidation.shipping_method_route`
- `contena.cache.invalidation.navigation_route`
- `contena.cache.invalidation.category_route`
- `contena.cache.invalidation.landing_page_route`
- `contena.cache.invalidation.language_route`
- `contena.cache.invalidation.currency_route`
- `contena.cache.invalidation.country_route`
- `contena.cache.invalidation.country_state_route`
- `contena.cache.invalidation.salutation_route`
- `contena.cache.invalidation.sitemap_route`
...
- `customer.group.registration.accepted` (`\Contena\Core\Checkout\Customer\Event\CustomerGroupRegistrationAccepted`)
- `customer.group.registration.declined` (`\Contena\Core\Checkout\Customer\Event\CustomerGroupRegistrationDeclined`)
...
`\Contena\Core\Content\ProductStream\Service\ProductStreamBuilderInterface` and its `buildFilters()` method have been removed. Use `\Contena\Core\Content\ProductStream\Service\AbstractProductStreamBuilder::enrichCriteria()` instead, which applies both the stream filters and the grouping state to the passed `Criteria`.

If your extension decorates the `ProductStreamBuilder` service or applies variant grouping manually, `extends AbstractProductStreamBuilder` and respect `\Contena\Core\Content\Product\SalesChannel\Listing\ProductListingLoader::STATE_SKIP_ADD_GROUPING` on the `Criteria` to keep matching variants ungrouped.
