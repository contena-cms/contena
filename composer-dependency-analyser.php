...
return $config
    ...
    /** Only used if `class_exists` is successful in @see \Contena\Core\Profiling\Integration\Datadog */
    ...
    /** Test plugins used in @see \Contena\Core\Framework\Test\Plugin\PluginIntegrationTestBehaviour */
    ->ignoreUnknownClassesRegex('~^Ct~')
    /** Only used if `class_exists` is successful in @see \Contena\Core\Profiling\Integration\Tideways */
    ->ignoreUnknownClasses(['Tideways\Profiler'])
    /** Only used if compression method is `zstd` @see \Contena\Core\Framework\Adapter\Cache\CacheValueCompressor */
    ...
    ], [ErrorType::SHADOW_DEPENDENCY]) // faker lib via `contena/dev-tools`
    ...
    ->ignoreErrorsOnPackage('contena/conflicts', [ErrorType::UNUSED_DEPENDENCY])

    /** Used by 3rd party libraries */
    ->ignoreErrorsOnExtensions(['ext-pdo_mysql', 'ext-sodium', 'ext-xml'], [ErrorType::UNUSED_DEPENDENCY])

    /** Only used if `function_exists` is successful @see \Contena\Core\Framework\Plugin\ExtensionExtractor */
    ...
    /** Only used if `function_exists` is successful @see \Contena\Core\Content\Media\Thumbnail\ThumbnailService */
...
{
    /** Optional dependency and only used if `extension_loaded` is successful in @see \Contena\Core\Content\Media\DependencyInjection\ThumbnailProcessorCompilerPass */
...
/**
 * Keep XML service symbols visible to the analyser until all service definitions use PHP configuration.
 */
function considerXMLServiceConfigFiles(Configuration $config): void
{
    ...
    // No need to add Contena classes
    $classNames = array_values(array_unique(array_filter(
        $classNames,
        static fn (string $className): bool => !str_starts_with($className, 'Contena\\')
    )));
