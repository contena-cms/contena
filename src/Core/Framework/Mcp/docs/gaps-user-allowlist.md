...
| Integration + `ct-app-user-id` (Copilot) | `integration.mcp_allowlist` only | intersect(`integration.mcp_allowlist`, `user.mcp_allowlist`) |
...
Accepts both `'integration'` and `'user'` key origins. When neither `ct-access-key` nor `ct-secret-access-key` header is present, the listener returns without throwing — standard bearer JWT auth handles the request downstream.
...
**ACL entry** in `ct-users-permissions/acl/index.js`: `api_action_user_mcp-allowlist` under `editor`.
...
| Modify | `src/Administration/.../ct-users-permissions-user-detail.html.twig` |
| Modify | `src/Administration/.../ct-users-permissions-user-detail/index.js` |
| Modify | `src/Administration/.../ct-users-permissions/acl/index.js` |
| Modify | `src/Administration/.../ct-users-permissions/snippet/en.json` + `de.json` |
