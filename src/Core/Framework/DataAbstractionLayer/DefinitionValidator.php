    ...
    private function findEntityNotices(string $struct, EntityDefinition $definition): array
    {
        ...
        foreach ($reflection->getProperties() as $property) {
            ...
            if ($property->isReadOnly()) {
                $notices[] = \sprintf('Field %s in entity struct should not be readonly in %s, as it needs to be writable by the DAL, see https://developer.contena.cn/docs/guides/plugins/plugins/framework/data-handling/add-custom-complex-data.html#entity-class', $property->getName(), $definition->getClass());
            }
            if ($property->isPrivate()) {
                $notices[] = \sprintf('Field %s in entity struct should not be private in %s, as it needs to be accessible by the DAL, see https://developer.contena.cn/docs/guides/plugins/plugins/framework/data-handling/add-custom-complex-data.html#entity-class', $property->getName(), $definition->getClass());
            }
