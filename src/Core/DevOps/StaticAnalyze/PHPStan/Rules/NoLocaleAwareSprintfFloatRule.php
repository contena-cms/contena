...
{
    private const string ERROR_MESSAGE = 'Do not use the locale-aware %f specifier in sprintf(). Use %F for locale-independent float serialization.';
            ...
            if (self::containsLocaleAwareFloatSpecifier($format->getValue())) {
                return [
                    RuleErrorBuilder::message(self::ERROR_MESSAGE)
                        ->identifier('contena.noLocaleAwareSprintfFloat')
                        ->build(),
                ];
            }
        }
