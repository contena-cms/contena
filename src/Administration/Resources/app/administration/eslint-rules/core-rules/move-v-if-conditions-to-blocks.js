...
/**
 * Overview
 * This custom ESLint rule enforces that the v-if, v-else or v-else-if attributes are moved to the <ct-block> element
 * when it has a single child.
 *
 * How It Works
 * The rule identifies <ct-block> elements with a single child and checks if the child element has v-if, v-else or
 * v-else-if attributes. If it finds any of these attributes, it reports an error and suggests moving the attribute to
 * the <ct-block> element.
 *
 * Example
 * ```
 * // before
...
module.exports = {
    meta: {
        type: 'problem',
        docs: {
            description: '<ct-block> with single child should move v-if, v-else or v-else-if attributes to the <ct-block> element',
