    ...
    if (shorthandSyntaxSlot && !itemsAttribute) {
        context.report({
            ...
            *fix(fixer) {
                ...
                /**
                 * Get all previous slot children components with "ct-tabs-item" and keep following values in an array:
                 * - Text content inside the component
                 * - "name" attribute
                 */
                const slotChildren = shorthandSyntaxSlot.children.filter((child) => {
                    return child.type === 'VElement' && child.name === 'ct-tabs-item';
                });
                ...
                const items = slotChildrenValues.map((child) => {
                    ...
                    // Examples:
                    //  - {{ $tc('ct-cms.elements.general.config.tab.content') }}
                    //  - {{ $t('ct-cms.elements.general.config.tab.example) }}
                    // Read just the snippet and use it as label:
                    // - ct-cms.elements.general.config.tab.content
                    // - ct-cms.elements.general.config.tab.example
            ...
            <template>
                <mt-tabs>
                    <template #default="{ active }">
                        <ct-tabs-item name="tab1">Tab 1</ct-tabs-item>
                        <ct-tabs-item name="tab2">Tab 2</ct-tabs-item>
                    </template>
                </mt-tabs>
            </template>`,
        output: `
    ...
    },
    {
        ...
        code: `
            <template>
                <mt-tabs>
                    <template #default="{ active }">
                        <ct-tabs-item name="tab1">{{ $tc('ct-cms.elements.general.config.tab.one') }}</ct-tabs-item>
                        <ct-tabs-item name="tab2">{{ $tc('ct-cms.elements.general.config.tab.two') }}</ct-tabs-item>
                    </template>
                </mt-tabs>
            </template>`,
        output: `
            <template>
                <mt-tabs :items="[
    {
        'label': 'ct-cms.elements.general.config.tab.one',
        'name': 'tab1'
    },
    {
        'label': 'ct-cms.elements.general.config.tab.two',
        'name': 'tab2'
    }
]">
                    <!-- TODO Codemod: This slot is not used anymore. Please use the "items" property instead. -->
    ...
    },
    {
        ...
        code: `
            <template>
                <mt-tabs
                    v-if="productId"
                    class="ct-product-detail-page__tabs"
                    position-identifier="ct-product-detail"
                >
                    <ct-tabs-item
                    ...
                    <ct-tabs-item
                        class="ct-product-detail__tab-specifications"
                        :route="{ name: 'ct.product.detail.specifications', params: { id: $route.params.id } }"
                        :title="$tc('ct-product.detail.tabSpecifications')"
                    >
                        {{ $tc('ct-product.detail.tabSpecifications') }}
                    </ct-tabs-item>
                </mt-tabs>
            </template>`,
        output: `
            <template>
                <mt-tabs :items="[
    {
        'label': 'ct-product.detail.tabGeneral',
        'name': 'ct.product.detail.base'
    },
    {
        'label': 'ct-product.detail.tabSpecifications',
        'name': '{ name: \\'ct.product.detail.specifications\\', params: { id: $route.params.id } }'
    }
]"
                    v-if="productId"
                    class="ct-product-detail-page__tabs"
