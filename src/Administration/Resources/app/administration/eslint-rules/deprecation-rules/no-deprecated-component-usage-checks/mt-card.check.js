        ...
        context.report({
            ...
            *fix(fixer) {
                ...
                yield fixer.insertTextAfter(
                    node.startTag,
                    `\n${' '.repeat(indent)}<slot name="title"><ct-ai-copilot-badge /></slot>\n${' '.repeat(indent)}`
                );
            }
        });
    }
    ...
    if (aiBadgeAttributeExpression) {
        context.report({
            ...
            *fix(fixer) {
                ...
                yield fixer.insertTextAfter(
                    node.startTag,
                    `\n${' '.repeat(indent)}<slot name="title"><ct-ai-copilot-badge v-if="${aiBadgeAttributeExpressionText}" /></slot>\n${' '.repeat(indent)}`
                );
            }
        });
    }
...
const mtCardValidTests = [
    {
        name: '"ct-card" usage is allowed',
        filename: 'test.html.twig',
        code: `
            <template>
                <ct-card>Hello World</ct-card>
            </template>`
    }
]
...
const mtCardInvalidTests = [
    {
        ...
        output: `
            <template>
                <mt-card >
                    <slot name="title"><ct-ai-copilot-badge /></slot>
                    Hello World</mt-card>
            </template>`,
        errors: [
            {
    ...
    },
    {
        ...
        output: `
            <template>
                <mt-card >
                    <slot name="title"><ct-ai-copilot-badge v-if="1 == 1" /></slot>
                    Hello World</mt-card>
            </template>`,
        errors: [
            {
