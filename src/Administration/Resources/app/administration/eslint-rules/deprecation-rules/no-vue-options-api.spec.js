...
'        };',
      ...
      export default {
        ...
        },
        {
            ...
            code: `
import template from './ct-users-permissions-role-view-general.html.twig';

const { mapPropertyErrors } = Contena.Component.getComponentHelper();

export default {
    template,

    compatConfig: Contena.compatConfig,
    ...
    computed: {
        ...
        },
        {
            ...
            code: `
import template from './ct-users-permissions-user-detail.html.twig';
import './ct-users-permissions-user-detail.scss';
...
`,
        ...
        },
        {
            ...
            code: `
import template from './ct-settings-tax-detail.html.twig';
import './ct-settings-tax-detail.scss';
...
export default {
    ...
    computed: {
        ...
        tooltipSave() {
            if (!this.allowSave) {
                return {
                    message: this.$t('ct-privileges.tooltip.warning'),
        ...
        label() {
            return this.isContenaDefaultTax ? this.$t(\`global.tax-rates.\${this.tax.name}\`) : this.tax.name;
        },
    ...
    methods: {
        ...
        onSave() {
            ...
            return this.taxRepository.save(this.tax).then(() => {
                ...
                if (!this.taxId) {
                    this.$router.push({ name: 'ct.settings.tax.detail', params: { id: this.tax.id } });
                }
                ...
                this.createNotificationError({
                    message: this.$t('ct-settings-tax.detail.messageSaveError'),
                });
                this.isLoading = false;
            });
        },
        ...
        onCancel() {
            this.$router.push({ name: 'ct.settings.tax.index' });
        },
        ...
        onChangeLanguage(languageId) {
            Contena.State.commit('context/setApiLanguageId', languageId);
            this.createdComponent();
        },
...
'            };\n' +
'        });\n' +
'          const isContenaDefaultTax = computed(() => {\n' +
"            return this.$te(`global.tax-rates.${tax.name}`, 'en-GB');\n" +
'        });\n' +
'          const label = computed(() => {\n' +
'            return isContenaDefaultTax.value ? this.$t(`global.tax-rates.${tax.name}`) : tax.name;\n' +
'        });\n' +
...
'                if (!props.taxId) {\n' +
"                    this.$router.push({ name: 'ct.settings.tax.detail', params: { id: tax.id } });\n" +
'                }\n' +
'\n' +
...
import template from './ct-settings-entry-list.html.twig';
import './ct-settings-entry-list.scss';

const { Mixin, Data: { Criteria } } = Contena;
...
export default {
    ...
    mixins: [
        Mixin.getByName('ct-settings-list'),
    ],
    ...
    computed: {
        ...
        contextMenuEditEntry() {
            return this.acl.can('entry.editor') ?
        ...
        activeFilters() {
            ...
            if (this.filterSettings.editedEntrys) {
                filter = { ...filter, edited: true };
            }
            if (this.filterSettings.addedEntrys) {
                filter = { ...filter, added: true };
            }
            if (this.filterSettings.emptyEntrys) {
    ...
    methods: {
        async createdComponent() {
            ...
            this.entrySetRepository.search(this.entrySetCriteria)
                .then((sets) => {
                    this.entrySets = sets;
                });
            ...
            const filterItems = await this.entryService.getFilter();
            this.filterItems = filterItems.data;

            const authorFilters = await this.entrySetService.getAuthors();
            this.authorFilters = authorFilters.data;

            await this.getFilterSettings();

            if (this.hasActiveFilters) {
                this.initializeEntrySet(this.activeFilters);
            }
        },
        ...
        getColumns() {
            const columns = [{
                property: 'id',
                label: 'ct-settings-entry.list.columnKey',
                inlineEdit: true,
                allowResize: true,
                rawData: true,
                primary: true,
        ...
        initializeEntrySet(filter = this.filter) {
            ...
            this.entrySetService.getCustomList(this.page, this.limit, filter, sort).then((response) => {
        ...
        prepareGrid(grid) {
            function prepareContent(items) {
                const content = items.reduce((acc, item) => {
                    ...
                    acc[item.setId] = item;
                    acc.isCustomEntry = item.author.includes('user/');
                    return acc;
                }, {});
...
"      const entrySetService = inject('entrySetService');\n" +
...
'            return filter;\n' +
'        });\n' +
'          const contextMenuEditEntry = computed(() => {\n' +
"            return acl.can('entry.editor') ?\n" +
"                this.$t('global.default.edit') :\n" +
"                this.$t('global.default.view');\n" +
'        });\n' +
...
'            }\n' +
'\n' +
'            if (filterSettings.value.editedEntrys) {\n' +
'                filter = { ...filter, edited: true };\n' +
'            }\n' +
'            if (filterSettings.value.addedEntrys) {\n' +
'                filter = { ...filter, added: true };\n' +
'            }\n' +
'            if (filterSettings.value.emptyEntrys) {\n' +
'                filter = { ...filter, empty: true };\n' +
'            }\n' +
'\n' +
'            filter = { ...filter, author: [] };\n' +
...
'            });\n' +
'\n' +
'            const filterItems = await entryService.getFilter();\n' +
'            filterItems.value = filterItems.data;\n' +
'\n' +
'            const authorFilters = await entrySetService.getAuthors();\n' +
'            authorFilters.value = authorFilters.data;\n' +
'\n' +
'            await this.getFilterSettings();\n' +
'\n' +
'            if (hasActiveFilters.value) {\n' +
'                initializeEntrySet(activeFilters.value);\n' +
'            }\n' +
'        };\n' +
...
'            return columns;\n' +
'        };\n' +
'      const initializeEntrySet = (filter = filter.value) => {\n' +
'            if (!this.$route.query.ids) {\n' +
'                this.backRoutingError();\n' +
'                return;\n' +
'            }\n' +
...
'            };\n' +
'\n' +
'            entrySetService.getCustomList(this.page, this.limit, filter, sort).then((response) => {\n' +
...
'};\n'
        },
        {
            ...
            code: `
            import template from './ct-sales-channel-detail-domains.html.twig';
import './ct-sales-channel-detail-domains.scss';

const { Mixin, Context } = Contena;
    ...
    computed: {
        ...
        currentDomainModalTitle() {
            if (this.currentDomain?.isNew()) {
                return this.$t('ct-sales-channel.detail.titleCreateDomain');
            }

            return this.$t('ct-sales-channel.detail.titleEditDomain', 0, {
    ...
    methods: {
        ...
        setCurrentDomainBackup(domain) {
            this.currentDomainBackup = {
                ...
                entrySet: domain.entrySet,
                entrySetId: domain.entrySetId,
            };
        },
        ...
        resetCurrentDomainToBackup() {
            ...
            this.currentDomain.entrySet = this.currentDomainBackup.entrySet;
            this.currentDomain.entrySetId = this.currentDomainBackup.entrySetId;
        },
    },
};
            `,
...
'            currentDomain.value.entrySet = currentDomainBackup.entrySet;\n' +
'            currentDomain.value.entrySetId = currentDomainBackup.entrySetId;\n' +
'        };\n' +
'\n' +
'          return {\n' +
'            currentDomain,\n' +
