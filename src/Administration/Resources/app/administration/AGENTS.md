...
- **Ant Design Vue is the primary and authoritative Administration UI library.** Use its native components, interaction patterns, icons, and theme tokens whenever they cover the requirement. Do not hand-roll an equivalent control or introduce another UI library without a concrete capability gap.
- Name project-owned Administration components and template tags with the `ct-*` prefix. Create a `ct-*` abstraction only when it provides reusable Contena behavior, extension points, or domain semantics; use Ant components directly for one-off composition.
- Meteor `mt-*` components are legacy and will be removed. Do not add, extend, newly reference, or build compatibility layers for them. When migrating an Administration area, replace its touched `mt-*` controls with Ant Design Vue or focused `ct-*` abstractions.
...
- Keep all component-specific styles in the owning `.vue` file with a `<style lang="scss">` block. Do not create or retain a separate same-named `.scss` file for an Administration component; when modifying a component that still has one, move its styles into the SFC and delete the SCSS file. Reserve shared SCSS files for genuinely global tokens, mixins, resets, and cross-component utilities.
- Make every SFC extendable with native `<script setup>`. Base components declare the supported public bindings once with `swDefinePublic({ ... })`; unlisted top-level bindings remain private. Override components use the `.override.vue` filename convention, read the base state through `useSwPreviousState()`, and declare replacements with `swDefineOverride({ ... })`. Do not author `createExtendableSetup` wrappers or `ComponentPublicApiMapping` entries; the native setup transform owns that runtime plumbing.
- Expose plugin-facing template seams with named `<ct-block name="sw_...">` elements around meaningful page, content, state, and action regions. The native setup transform injects `:data="$dataScope"`; do not write that binding by hand. Use snake_case block names with the `sw_` prefix; plugin overrides should use `<ct-block extends="...">` and `<ct-block-parent />` when preserving the base output.
