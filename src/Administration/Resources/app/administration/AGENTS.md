...
- Expose plugin-facing template seams with named `<ct-block name="ct_...">` elements around meaningful page, content, state, and action regions. The native setup transform injects `:data="$dataScope"`; do not write that binding by hand. Use snake_case block names with the `ct_` prefix for new blocks; existing `sw_` names remain valid legacy extension points. Plugin overrides should use `<ct-block extends="...">` and `<ct-block-parent />` when preserving the base output. Structural `ct-page`/`ct-block` tags are allowed, but legacy `ct-*` UI widgets are not.
