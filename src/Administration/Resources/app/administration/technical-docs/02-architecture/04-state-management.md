...
The administration interface uses **Pinia** as its state management solution, having migrated from Vuex as of Contena 6.7. Pinia provides better TypeScript support, a more intuitive API, and improved developer experience.

## Core Concepts

### Store Registration

Stores are registered using the `Contena.Store.register()` method:

```typescript
const myStore = Contena.Store.register({
    id: 'myStore',
    state: () => ({
        items: [],
        loading: false,
...
Access stores using `Contena.Store.get()`:
...
- **Simplified API**: `Contena.Store.get()` instead of `Contena.State.get()`
...
```typescript
// Old Vuex pattern
Contena.State.commit('myStore/setItems', items);

// New Pinia pattern
Contena.Store.get('myStore').setItems(items);
```
