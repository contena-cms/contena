...
sequenceDiagram
    ...
    participant CoreContena as core/contena.ts
    ...
    IndexTS->>CoreContena: Dynamic import core/contena
    Note right of CoreContena: Creates DI container, factories, global Contena object
    CoreContena-->>IndexTS: Contena instance ready
...
// Dynamic import of core Contena instance
void import('src/core/contena').then(async ({ ContenaInstance }) => {
    // Set global Contena instance
...
The `src/core/contena.ts` file creates the foundation:
...
4. **Factory Registration**: Registers all core factories (component, module, state, etc.)
5. **Global Contena Object**: Creates the global `Contena` object with all APIs
...
When `window.startApplication()` is called, it triggers `Contena.Application.start()`:

### Initial Setup
1. **State Initialization**: `initState()` ensures the pre-initializer state container exists
2. **Configuration Registration**: `registerConfig()` sets up `Contena.Context.api` and `Contena.Context.app`
...
4. **Session Flag**: Set `ct-login-should-reload` for post-login refresh
...
- `ct-admin` - Root administration component
- `ct-admin-menu` - Main navigation
- `ct-button`, `ct-button-process` - Core UI elements
...
5. **Development Tools**: Contena devtools initialize in development mode
...
if (firstRunWizard && loginService.isLoggedIn() &&
    !router?.currentRoute?.value?.name?.startsWith('ct.first.run.wizard')) {
    router.push({ name: 'ct.first.run.wizard.index' });
}
```
...
- **Contena Devtools**: Enhanced debugging capabilities
