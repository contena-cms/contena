...
```typescript
// Service registration
Contena.Service().register('serviceName', (container) => {
...
// Service usage
const customService = Contena.Service('serviceName');
```
...
class CustomApiService extends ApiService {
    customMethod() {
        ...
        // - ct-context-token: <context-token>
...
```typescript
Contena.Service().registerDecorator('systemConfigApiService', (service) => {
...
Contena.Service().register('customDomainService', (container) => {
    const initContainer = Contena.Application.getContainer('init');
...
```typescript
Contena.Service().registerMiddleware('loggingMiddleware', (service, serviceName) => {
