...
The Contena 6 Administration provides an internationalization (i18n) system that enables multi-language support for the admin interface. This system is built around a locale factory, static translation files, and an API for loading installed Administration translations.
...
- **Snippet API Service** (`core/service/api/snippet.api.service.ts`) - Loads Administration translations and installed locales
...
```
src/app/snippet/
├── zh.json     # Simplified Chinese (default interface)
...
const snippetService = Contena.Service('snippetService');
...
- **Module Level**: `ct-[module-name].[section].[key]`
- **Global Level**: `global.[category].[key]`
- **Component Level**: `ct-[component-name].[key]`

### Examples

```javascript
// Module-specific translations
'ct-product.list.title'
'ct-customer.detail.generalCard'
...
'ct-data-grid.actions.edit'
'ct-modal.footer.cancel'
```
...
The system uses `zh-CN` as the default interface locale and `en-GB` as its translation fallback:
...
3. **Default Locale**: Uses `zh-CN` if no registered browser locale matches
4. **Translation Fallback**: Uses `en-GB` when a Chinese snippet is missing
...
```javascript
const localeHelper = Contena.Service('localeHelper');
...
## Administration Translation Loading

The Snippet API Service loads the static Administration translations exposed by the backend:

```javascript
const snippetService = Contena.Service('snippetService');

// Load all Administration translations for a locale
await snippetService.getSnippets(localeFactory, localeCode);

// List installed Administration locales
...
This comprehensive i18n system ensures that the Contena 6 Administration can be effectively localized for international users while maintaining consistency and ease of development.
