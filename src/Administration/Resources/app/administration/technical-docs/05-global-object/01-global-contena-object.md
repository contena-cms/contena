# Global Contena Object

The global Contena object serves as the central registry and facade for core extensibility in the Contena 6 Administration. It provides a unified interface for plugin authors and core developers to access various registries, services, and utilities.

## Why a Global Object?

The global Contena object exists for **practical reasons**:
...
2. **Backward Compatibility**: Maintains API stability across Contena versions for existing plugins
3. **Discoverability**: Easy to explore available APIs via `window.Contena` in browser dev tools
4. **Universal Access**: Available everywhere without imports or injection - crucial for runtime plugin loading

The object is exposed as `window.Contena` during application bootstrap, making it globally accessible to all code running in the administration.

## Core Structure

The global Contena object exposes the following essential registries:
...
### Service Layer
```javascript
Contena.Service = ServiceFactory; // Service factory for dependency injection
```

### State Management
```javascript
Contena.State = StateFactory(); // Legacy Vuex state (deprecated)
Contena.Store = Store.instance; // Current Pinia store instance
```
...
### Essential Utilities
```javascript
Contena.Utils = utils;           // Collection of utility functions
Contena.Data = data;             // DAL utilities and repositories
Contena.Context = useContext();  // Current application context
Contena.Defaults = { /* ... */ }; // System default IDs and values
```

## Creation Process

1. **Initialization**: Created in `src/core/contena.ts` as `ContenaClass` singleton
2. **Container Setup**: Built on BottleJS dependency injection container
3. **Global Assignment**: Set as `window.Contena` in `src/index.ts` during bootstrap
...
Contena.Component.register('my-plugin-component', {
    template: '<div>My Component</div>'
});

// Registering modules
Contena.Module.register('my-plugin-module', {
    routes: { /* ... */ },
    navigation: { /* ... */ }
});
```
...
### Feature Flag Checks
```javascript
if (Contena.Feature.isActive('MY_FEATURE')) {
...
The global object represents a **pragmatic compromise** between developer experience and modern architecture patterns, prioritizing ecosystem stability and ease of use for the large Contena plugin community.
