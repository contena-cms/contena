...
**Base.** The author body stays exactly as written — plain `<script setup>`, macros in place, nothing hoisted or wrapped. The transform only (1) renames each top-level runtime binding to a reserved `__swSetupAuthor_<name>` alias and (2) appends a generated `Contena.Component.attachOverrides({ public, private })` footer that re-declares the original names from the override wrapper. Templates read overrideable state exactly as before.
...
The base transform also adds `:data="$dataScope"` to every `<ct-block name="...">`, forwarding the generated data scope to block overrides without every base author writing it by hand. The `data` binding and the default slot scope of `<ct-block>` are owned by the transform: authoring `data`, `#default`, or a `v-bind` object on `<ct-block>` is rejected.
