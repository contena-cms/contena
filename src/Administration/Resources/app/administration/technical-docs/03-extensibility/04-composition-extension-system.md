...
**Source files:**
- `src/app/adapter/composition-extension-system/index.ts` — `createExtendableSetup`, `attachOverrides`, `overrideComponentSetup`, `_overridesMap`
...
```mermaid
flowchart TD
    pluginOptions["Plugin: Contena.Component.override()"]
    pluginComposition["Plugin: Contena.Component.overrideComponentSetup()"]
    overrideRegistry["Component Override Registry (Contena.Component.getOverrideRegistry())"]
...
| `ComponentPublicApiMapping` | Internal TypeScript fallback | Supports runtime implementation tests; component authors do not extend it |

---

## `createExtendableSetup` (internal)

**Location:** `composition-extension-system.ts`

Wraps a component's setup function to make it extendable at runtime. Generated integration and runtime tests use it; component authors use the native setup macros instead.
...
### Legacy internal TypeScript mapping

The runtime retains an internal `ComponentPublicApiMapping` fallback for its implementation tests. Do not add application component entries; the native setup transform derives each component's state from its SFC bindings.
...
2. An async IIFE reads all pending overrides from `Contena.Component.getOverrideRegistry()` for this component name. Each pending override that contains Options API patterns is converted via the shim (see below); the resulting Composition API function is pushed into `_overridesMap[name]`.
...
Plugin authors use this function to register a Composition API override for a specific component. It is exposed on `Contena.Component`.
...
```javascript
Contena.Component.overrideComponentSetup()('ct-my-component', (previousState, props, context) => {
    // ... return overrides
});
```
...
The shim is a backward-compatibility layer that allows existing plugins using the Options API `Contena.Component.override()` pattern to continue working when the target component has been migrated to Composition API with `createExtendableSetup`.
...
```typescript
Contena.Component.overrideComponentSetup()('ct-my-component', (previousState) => {
