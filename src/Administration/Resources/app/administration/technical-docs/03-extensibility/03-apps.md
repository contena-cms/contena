...
Apps represent Contena's cloud-native extension approach, using the Meteor Admin Extension SDK to provide iframe-based integrations within the administration interface.
...
- **Controlled API Access**: Limited, permission-based access to Contena APIs
...
ui.componentSection.add({
    component: 'card',
    positionId: 'ct-product-detail__tabs',
    props: {
...
Access Contena's data using the repository pattern:
...
```javascript
import { notification, toast } from '@contena/meteor-admin-sdk';
...
if (location.is('ct-product-detail')) {
...
import { notification, data, ui } from '@contena/meteor-admin-sdk';
...
```javascript
import { data, notification } from '@contena/meteor-admin-sdk';
...
} else if (location.is('ct-product-detail')) {
