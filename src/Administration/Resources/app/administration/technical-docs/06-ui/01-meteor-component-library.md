...
The **Meteor Component Library** is Contena's modern, open-source design system that serves as the foundation for all Contena commerce solutions. It provides a comprehensive collection of Vue.js components designed to create elegant, delightful, and accessible user experiences across the Contena ecosystem.

### Purpose

- **Modern replacement** for legacy `ct-*` components in the Contena 6 Administration
...
- `mt-button` (replaces `ct-button`)
- `mt-card` (replaces `ct-card`)
- `mt-text-field` (replaces `ct-text-field`)
- `mt-data-table` (replaces `ct-data-table`)

### External Repository

The Meteor Component Library is maintained as a separate package:

- **Repository**: https://github.com/contena/meteor
- **Package**: `@contena/meteor-component-library`
- **Documentation**: https://meteor-component-library.vercel.app/

### Design Token Integration

Meteor components consume design tokens from `@contena/meteor-tokens`, providing:
...
Contena 6 uses native SFC wrappers to integrate Meteor components while preserving extension points:
...
import { MtCard as MtCardOriginal } from '@contena/meteor-component-library';
</script>
```
...
<mt-card-original v-bind="$attrs">
    <template #before-card>
        <ct-extension-component-section v-if="positionIdentifier" :position-identifier="positionIdentifier + '__before'" />
    ...
    <template #after-card>
        ...
        <ct-extension-component-section v-if="positionIdentifier" :position-identifier="positionIdentifier + '__after'" />
    </template>
</mt-card-original>
```
...
```javascript
import { MtButton, MtCard, MtTextField } from '@contena/meteor-component-library';
...
<mt-card title="User Profile">
    <mt-text-field label="Username" v-model="username" :required="true" />

    <mt-button variant="primary" @click="saveUser"> Save Changes </mt-button>
</mt-card>
```
...
| ---------------- | ---------------- | ----------------------------------------- |
| `ct-button`      | `mt-button`      | Updated prop names, new variants          |
| `ct-card`        | `mt-card`        | New slot structure, improved styling      |
| `ct-text-field`  | `mt-text-field`  | Enhanced validation, better accessibility |
| `ct-data-table`  | `mt-data-table`  | New API, improved performance             |
| `ct-select`      | `mt-select`      | Simplified props, better UX               |
...
- **GitHub Repository**: https://github.com/contena/meteor
- **Design System**: https://contena.design/
- **NPM Package**: https://www.npmjs.com/package/@contena/meteor-component-library
