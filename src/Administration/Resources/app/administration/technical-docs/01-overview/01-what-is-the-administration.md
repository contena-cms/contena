...
status: Draft
description: High-level overview of the Contena 6 Administration application – purpose, positioning, architecture, capabilities and extensibility surface.
---

# What Is The Administration?

> The Administration is the merchant & operator facing single-page application of Contena 6: the place where business users manage catalog, content, orders, customers, configuration and commercial extensions. It is not the storefront (buyer experience) and not the raw headless API platform, it is an application built *on top* of those platform APIs.

## Positioning & Purpose

The Administration provides a cohesive UI layer over Contena's domain APIs. Core goals:
...
6. Productivity & consistency through shared component library (Meteor components + custom `ct-` components)
...
| Runtime Globals | Central `Contena` object exposes factories, service container, feature flag API, component registry. See [Global Contena Object](../05-runtime/01-global-contena-object.md).                                                        |
...
| UI Library | Migration from `ct-` components to meteor library components, plus design tokens. See [Meteor Component Library](../06-ui/01-meteor-component-library.md).                                                                             |
...
* Native Vue component setup overrides through `createExtendableSetup` and `overrideComponentSetup`
* Native `ct-block` extension points inside Vue SFC templates
...
| Meteor Component Library | Modern component library replacing current `ct-*` components |
