...
The actual sequence (simplified) derived from `index.ts`, `core/contena.ts`, `app/main.ts`, and `core/application.ts`:

1. Browser requests `/admin` → Twig shell responds with minimal HTML + injected `apiContext` & `appContext` (incl. bundles metadata). If a global `_features_` object is present it is consumed immediately in `core/contena.ts` for an early `Feature.init` before the formal `Application.start()` sequence.
2. `index.ts` loads global styles, dynamically imports `core/contena` (creates Bottle DI container, factories, global `Contena` object, early feature init if `_features_` present).
...
4. `index.ts` calls `window.startApplication()` (mapped to `Contena.Application.start(config)` by the shell) passing runtime contexts.
5. `Application.start()`:
    * `initState()` ensures pre-initializer state container exists.
    * `registerConfig()` sets `Contena.Context.api` & `Contena.Context.app`.
...
7. If logged in → `bootFullApplication()` executes in order:
   ...
   * Await `Contena.Plugin.getBootPromises()` (async plugin boot tasks).
...
sequenceDiagram
    ...
    participant SW as core/contena (Contena.Application)
    ...
    I->>SW: dynamic import core/contena (create DI, factories, early Feature.init)
    ...
    alt Not logged in
        ...
        P-->>A: boot promises resolved (Contena.Plugin.getBootPromises)
