...
import ContenaSetupPlugin from './build/vite-plugins/contena-setup';
...
process.env.SERVICE_REGISTRY_URL = process.env.SERVICE_REGISTRY_URL ?? 'https://registry.services.contena.io';
...
export default defineConfig(({ command }) => {
    ...
    const useSourceMap =
        (isDev && process.env.CONTENA_ADMIN_SKIP_SOURCEMAP_GENERATION !== '1') ||
    ...
    return {
        ...
        plugins: (() => {
            ...
            const sharedPlugins = [
                // Contena plugins: build/vite-plugins
                ...
                // Administration dependencies load Node core modules in the browser.
                nodePolyfills({
        ...
        resolve: {
            alias: [
                {
                    // The polyfill plugin injects these imports into local file: dependencies.
                    // Pin them to the Administration installation instead of the linked package's workspace.
                ...
                {
                    find: /^vue-i18n$/,
                    replacement: path.resolve(__dirname, 'node_modules/vue-i18n/dist/vue-i18n.mjs'),
                },
                {
                    find: /^vue-router$/,
                    replacement: path.resolve(__dirname, 'node_modules/vue-router/dist/vue-router.mjs'),
                },
                {
                    find: /^src\//,
                    replacement: '/src/',
