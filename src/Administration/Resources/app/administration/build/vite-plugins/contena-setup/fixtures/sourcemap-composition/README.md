...
This fixture intentionally tests a Vite build that runs the Contena setup pre-transform together with `@vitejs/plugin-vue`.

The shared transform has focused unit coverage for the sourcemap it returns directly. That is not enough for editor and browser debugging, because real Administration builds do not consume that map in isolation. Vite first receives the Contena setup map, then Vue's SFC plugin compiles the transformed `.vue` file and composes another map on top.
...
- `src/ct-nested-component.vue` contains authored `<script setup>` code whose generated output is moved by the Contena setup transform and then compiled by Vue.
