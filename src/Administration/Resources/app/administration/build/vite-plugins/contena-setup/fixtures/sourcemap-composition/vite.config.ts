...
const adminRoot = process.env.CONTENA_ADMIN_ROOT as string;
...
export default (async () => {
    ...
    const ContenaSetupPlugin = jiti(path.join(adminRoot, 'build/vite-plugins/contena-setup/index.ts')).default;

    return {
        root: here,
        logLevel: 'silent',
        plugins: [
            ContenaSetupPlugin({ administrationRoot: adminRoot }),
            vue(),
        ],
        build: {
            write: true,
