...
import type { transformContenaSetupSfc as transformContenaSetupSfcRuntime } from '../../vue-setup-transform';
import { createVirtualSetupSourcemapContext } from './virtual-sfc-sourcemap';

type ContenaSetupTransformModule = {
    transformContenaSetupSfc: typeof transformContenaSetupSfcRuntime;
};
type ContenaSetupTransformImport =
    | ContenaSetupTransformModule
    | {
          default: ContenaSetupTransformModule;
      };
type ContenaSetupTransformResult = NonNullable<ReturnType<typeof transformContenaSetupSfcRuntime>>;
...
async function importContenaSetupTransform(administrationRoot: string): Promise<typeof transformContenaSetupSfcRuntime> {
 ...
 */
export default function contenaSetupPlugin(options: Options): Plugin {
    ...
    let transformPromise: Promise<typeof transformContenaSetupSfcRuntime> | null = null;

    function loadContenaSetupTransform(): Promise<typeof transformContenaSetupSfcRuntime> {
        transformPromise ??= importContenaSetupTransform(options.administrationRoot);
    ...
    async function transformSource(code: string, fileName: string): Promise<ContenaSetupTransformResult | null> {
        const transformContenaSetupSfc = await loadContenaSetupTransform();

        return transformContenaSetupSfc(code, fileName);
    }

    function assertUniqueBaseComponent(result: ContenaSetupTransformResult, fileName: string): void {
        if (result.mode !== 'base') {
            return;
        }
    ...
    return {
        name: 'contena-vite-plugin-contena-setup',
        enforce: 'pre',

        /**
         * Redirects a setup `.vue` import to a virtual `<realpath>.contena-setup.vue` id so the rewritten
        ...
        configResolved(config) {
            ...
            // from GENERATE_SOURCEMAPS / CONTENA_ADMIN_SKIP_SOURCEMAP_GENERATION. Reading it here keeps
        ...
        /**
         * Collapses the virtual `.contena-setup.vue` source paths back to the real files in the shipped maps.
         *
