...
import contenaSetupPlugin from './index';
...
function createPlugin(options: { administrationRoot: string } = pluginOptions): CallableSetupPlugin {
    return contenaSetupPlugin(options) as unknown as CallableSetupPlugin;
}
const execFileAsync = promisify(execFile);

async function createVueFile(source: string, fileName = 'component.vue') {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ct-setup-vite-plugin-'));
...
describe('build/vite-plugins/contena-setup', () => {
    it('returns a pre-load Vite plugin', () => {
        const plugin = createPlugin();

        expect(plugin.name).toBe('contena-vite-plugin-contena-setup');
    ...
    it('delegates Contena setup Vue files to the shared transform', async () => {
        const plugin = createPlugin();
...
ctDefinePublic({ count });
        ...
        expect(resolvedId).toBe(`${vueFile}.contena-setup.vue`);
        expect(loaded).toHaveProperty('code');
        expect(loaded?.code).toContain('Contena.Component.attachOverrides(');
        expect(loaded?.code).toContain("name: 'ct-my-component'");
        expect(loaded?.map.sources).toContain(vueFile);
        expect(loaded?.map.sources).not.toContain('ct-my-component.vue');
...
ctDefineOverride({});
        ...
        expect(loaded).toHaveProperty('code');
        expect(loaded?.code).toContain("Contena.Component.overrideComponentSetup()('ct-my-component'");
    });
...
ctDefinePublic({ count });
        ...
        await expect(plugin.transform(source, '/b/ct-my-component.vue')).rejects.toThrow(
            'Duplicate native setup base component name "ct-my-component"',
        );
    });
...
ctDefinePublic({ count });
        ...
        await expect(plugin.transform(source, '/example/ct-my-component.vue')).resolves.toHaveProperty('code');
    });
...
ctDefinePublic({ count });
        ...
        plugin.watchChange('/old/ct-my-component.vue', { event: 'delete' });

        await expect(plugin.transform(source, '/new/ct-my-component.vue')).resolves.toHaveProperty('code');
    });
...
ctDefinePublic({ count });
        ...
        await plugin.transform(source, '/a/ct-my-component.vue');
        plugin.watchChange('/somewhere/else/ct-other-component.vue', { event: 'delete' });
    ...
    it('maps the written sourcemap back to the authored SFCs, for base and override alike', async () => {
        ...
        const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ct-setup-vite-map-'));
        ...
        const { stdout } = await execFileAsync(
            ...
            ],
            {
                ...
                env: {
                    ...process.env,
                    CONTENA_ADMIN_ROOT: process.cwd(),
                },
            },
        );
        ...
        // shipped plugin map still named the virtual `.contena-setup.vue` files.
        ...
        const sides: { probe: ProbeSide; file: string }[] = [
            { probe: base, file: 'src/ct-nested-component.vue' },
            { probe: override, file: 'src/ct-nested-component.override.vue' },
        ];
    ...
    it('rejects Vue files without a script setup block', async () => {
        ...
        // resolveId runs the transform to decide whether the file is a Contena setup SFC, so the
        ...
        ).rejects.toThrow('A Contena setup component needs a <script setup> block.');
    });
...
ctDefinePublic({ count });
        ...
        await expect(plugin.transform(source, '/example/ct-first.vue')).rejects.toThrow(
