...
/**
 * The sourcemap shape the contena-setup transform emits for its `<script setup>` rewrite.
 *
 ...
 */
type ContenaSetupTransformMap = NonNullable<ReturnType<typeof transformContenaSetupSfcRuntime>>['map'];
...
type Remapping = (
    map: { sources: string[] },
    loader: (source: string) => ContenaSetupTransformMap | null,
    excludeContent?: boolean,
) => ContenaSetupTransformMap;
...
const virtualSetupSuffix = '.contena-setup.vue';
 ...
 *
 * The virtual id is `<realpath>.contena-setup.vue` rather than a NUL-prefixed (`\0`) Rollup virtual
...
function createVirtualSetupSourcemapContext(administrationRoot: string) {
    ...
    const setupMapByVirtualFileName = new Map<string, ContenaSetupTransformMap>();
     ...
     */
    function rememberSetupMap(virtualFileName: string, map: ContenaSetupTransformMap): void {
