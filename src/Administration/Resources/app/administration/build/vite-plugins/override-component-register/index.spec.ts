...
describe('build/vite-plugins/override-component-register', () => {
    ...
    it('should return an object with a name, configResolved and transform property', () => {
        ...
        expect(plugin.name).toBe('contena-vite-plugin-override-component');
    ...
    it('should transform entry file correctly', () => {
        ...
        expect(result?.code).toContain('/* Start: auto generated by Contena */');
        expect(result?.code).toContain(
            "from './build/vite-plugins/override-component-register/_fixtures/ct-spec_foo.override.vue';",
        );
        expect(result?.code).toContain(
            "from './build/vite-plugins/override-component-register/_fixtures/ct-setup-example.override.vue';",
        );
        expect(result?.code).toMatch(/import _ctOverride\d+ from/);
        expect(result?.code).toMatch(/Contena\.Component\.registerOverrideComponent\(_ctOverride\d+\);/);
        expect(result?.code).toContain('/* End: auto generated by Contena */');
    });
    ...
    it('registers same-named overrides from different directories with distinct bindings', () => {
        ...
        // Overrides that share a basename (first/ and second/ct-same-name.override.vue) must both
        // be imported, and the generated module must parse - a repeated binding would be a SyntaxError.
        expect(result!.code).toContain('first/ct-same-name.override.vue');
        expect(result!.code).toContain('second/ct-same-name.override.vue');
    ...
    it('emits a stable order so the generated entry does not depend on filesystem walk order', () => {
        ...
        const paths = [...first!.code.matchAll(/import _ctOverride\d+ from '\.\/(.+?)';/g)].map((match) => match[1]);
