...
export default function viteAssetPlugin(isProd: boolean, adminDir: string, extensions: ExtensionDefinition[] = []): Plugin {
    ...
    if (isProd) {
        return {
            name: 'contena-vite-plugin-copy-static-assets',
    ...
    return {
        name: 'contena-vite-plugin-serve-multiple-static',
        ...
        configureServer(server) {
            ...
            server.middlewares.use((req, res, next) => {
                ...
                // Add a custom route for ct-plugin-dev.json
                ...
                if (originalUrl.endsWith('ct-plugin-dev.json')) {
                    const pluginDevContent = fs.readFileSync(path.resolve(adminDir, '../../ct-plugin-dev.json'), 'utf8');
