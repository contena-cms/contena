/**
 * Covers the Contena marker macros and override runtime helpers: `swDefinePublic()` /
...
describe('build/vue-setup-transform setup markers', () => {
    it.each([
        ...
        [
            ...
            '"__swOverride" is reserved for Contena override-private state and cannot be exposed with swDefinePublic().',
        ],
    ...
    it('requires swDefineOverride() in override mode', () => {
        ...
        expect(() => transformContenaSetupSfc(source, 'missing-override.override.vue')).toThrow(
            'swDefineOverride() must be called exactly once at the top level of an override Contena setup block.',
        );
    });
    ...
    it.each([
        [
            ...
            'Only one swDefinePublic() call is allowed in a base Contena setup block.',
        ],
        [
            'override.override.vue',
            'swDefineOverride({ count });\nswDefineOverride({});',
            'Only one swDefineOverride() call is allowed in an override Contena setup block.',
        ],
    ...
    it.each([
        ...
        [
            ...
            '"__swOverride" is reserved for Contena override-private state and cannot be exposed with swDefineOverride().',
        ],
    ...
    it('rejects useSwPreviousState() in base mode', () => {
        ...
        expect(() => transformContenaSetupSfc(source, 'base-previous-state.vue')).toThrow(
            'useSwPreviousState() is only supported in override Contena setup blocks.',
        );
    });
});
