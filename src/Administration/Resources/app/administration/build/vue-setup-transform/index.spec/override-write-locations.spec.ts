...
describe('build/vue-setup-transform override write locations', () => {
    it.each([
        ...
        const error = captureTransformError(source, 'ct-write.override.vue');
    ...
    it.each([
        ...
        const error = captureTransformError(source, 'ct-model.override.vue');
    ...
    it.each([
        ...
        expect(transformOrFail(overrideSource(markup), 'ct-model.override.vue')).toBeDefined();
    });
    ...
    it.each([
        'let count = 0;',
        'const count = useCtPreviousState();',
        'const count = useCtProps();',
        'const count = useCtContext();',
    ])('rejects v-model on private state and input aliases: %s', (declaration) => {
        const source = `<script setup>${declaration} ctDefineOverride({});</script>
<template><ct-block extends="ct_example"><input v-model="count" /></ct-block></template>`;
        const error = captureTransformError(source, 'ct-model.override.vue');
    ...
    it('allows v-model on a base setup binding', () => {
        const source = `<script setup>let count = 0; ctDefinePublic({ count });</script>
<template><ct-block name="ct_example"><input v-model="count" /></ct-block></template>`;

        expect(transformOrFail(source, 'ct-base.vue')).toBeDefined();
    });
});
