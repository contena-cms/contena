 ...
 * references of whole subtrees while respecting that nesting, and identify the ct-block elements the
...
type DirectiveNode = CoreDirectiveNode & {
    ...
    // ct-block-bindings falls back to `v-${name}`, and that fallback must stay reachable per the type.
    rawName?: string;
};
...
/**
 * Collects references from descendants of a ct-block override slot.
 *
 */
