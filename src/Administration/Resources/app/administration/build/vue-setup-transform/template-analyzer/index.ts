/**
 * Analyzes Contena setup templates for data-scope and override-private state wiring.
 *
 * Base templates need a data scope on each `<ct-block>`, and override templates must forward the setup
...
/**
 * One `<ct-block extends>` whose content reads override-local bindings, and which therefore needs a
 ...
 *
 * Both analyzers share this walk; each supplies only its per-element work (the ct-block guard plus its
 ...
 */
function analyzeOverrideTemplate(block: ContenaSetupBlock, analysis: OverrideSetupScriptAnalysis): TemplateAnalysis {
    if (!block.template) {
    ...
    // An override template may only carry <ct-block extends> blocks at its top level.
 ...
 *
 */
function analyzeBaseTemplate(block: ContenaSetupBlock): TemplateAnalysis {
    if (!block.template) {
        return emptyTemplateAnalysis();
    }
