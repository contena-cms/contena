 ...
 * halves of the file, so each override files its locals under the shared `__ctOverride` channel without
 * needing a globally unique name.
 */
describe('build/vue-setup-transform/template-analyzer private namespace', () => {
    const source = `<template>
    <ct-block extends="ct_body">{{ info }}</ct-block>
</template>
...
ctDefineOverride({});
    ...
    it('declares the namespace symbol at module root, outside the override callback', () => {
        ...
        expect(result).toContain("const __ctSetupNamespace = Symbol('ct-thing.override');");
        ...
        expect(result.indexOf('const __ctSetupNamespace')).toBeLessThan(
    ...
    it('uses the symbol as a computed key in both the slot scope and the returned state', () => {
        ...
        expect(result).toContain('#default="{ __ctOverride: { [__ctSetupNamespace]: { info } } }"');
        expect(result).toContain('[__ctSetupNamespace]: {');
    });
    ...
    it('carries no filename or hash into the generated key', () => {
        ...
        expect(result).not.toContain('ct_thing_override');
        expect(result).not.toMatch(/__ctOverride: \{ [A-Za-z_$][A-Za-z0-9_$]*_[a-f0-9]{5}:/u);
    });
