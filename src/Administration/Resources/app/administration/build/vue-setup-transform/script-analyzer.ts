/**
 * Builds the normalized script model consumed by the Contena setup lowerers.
 *
...
/**
 * Describes one parsed Contena setup script after macro validation and binding classification.
 *
 ...
 */
type ContenaSetupScriptAnalysis = SharedScriptAnalysis & (BaseScriptAnalysis | OverrideScriptAnalysis);
...
function assertSupportedLang(lang: string, scriptOffset: number): void {
    if (!SUPPORTED_SCRIPT_LANGS.has(lang)) {
        throw new ContenaSetupTransformError(
            `Unsupported <script setup lang="${lang}"> in a Contena setup block. Supported languages are js, jsx, ts, and tsx.`,
            scriptOffset,
        );
    }
}
 ...
 */
function classifyTopLevelStatements(ast: BabelFile, mode: ContenaSetupMode, scriptOffset: number): ClassifiedStatements {
    ...
    ast.program.body.forEach((statement) => {
        ...
        // The Contena marker statements are compile-time only: their entries are extracted later and
 ...
 */
function analyzeContenaSetupScript(script: string, options: AnalyzerOptions): ContenaSetupScriptAnalysis {
...
export {
    ...
    type ContenaSetupScriptAnalysis,
    analyzeContenaSetupScript,
};
