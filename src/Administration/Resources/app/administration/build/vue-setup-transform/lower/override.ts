/**
 * Lowers override Contena setup scripts into hidden components that register setup overrides.
 *
 * The generated block stays a plain `<script setup>` whose body registers the override callback: the
 * hidden component mounts once at boot (ct-admin renders all registered override components in a
 * hidden container), which runs the registration and renders the template so `<ct-block extends>`
...
/**
 * The generated `#default` slot scope that carries override-local bindings into `<ct-block extends>`
 * content.
 *
 ...
 * that Symbol rather than by a literal name. Authoring `#default` on `<ct-block>` is rejected, so there
 ...
 *
 * Emits the script content, one slot scope per `<ct-block extends>` that forwards bindings, and a
...
function buildOverrideScript(
    ...
    // Only needed when this override actually forwards private locals into a <ct-block extends> scope;
    ...
    chunks.push(
        generated(
            `Contena.Component.overrideComponentSetup()('${escapeSingleQuoted(block.componentName)}', (${previousStateName}, ${propsName}, ${contextName}) => {`,
        ),
        generated('\n'),
        ...body,
        generated('\n});\n'),
        ...
        : [
              {
                  ...
                  replacement: '<template><!-- Contena override registration component --></template>\n',
              },
          ];
