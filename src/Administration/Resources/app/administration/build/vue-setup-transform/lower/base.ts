/**
 * Lowers base Contena setup scripts by keeping the author's body native and
 * appending a generated `Contena.Component.attachOverrides(...)` footer.
 *
...
import { CONTENA_SETUP_INTERNAL_PREFIX } from '../script-analyzer/macros';
...
function toAuthorAlias(localName: string): string {
    return `${CONTENA_SETUP_INTERNAL_PREFIX}Author_${localName}`;
}
 ...
 * attribute per `<ct-block>` the template analysis located.
 */
function buildBaseScript(
    ...
    const footer = [
        ...
        '});',
        '',
        // ctDefinePublic() is the parent-facing surface too, so the call is generated here and authoring
        ...
        `defineExpose(${formatObjectProperties(
            [
                '...Contena.Component.getExposedProps()',
