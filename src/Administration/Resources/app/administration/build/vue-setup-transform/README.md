...
Lowers Contena setup SFCs — plain Vue `<script setup>` files following the filename convention —
into the Composition-API extension contracts before Vue compiles them. Base components (`ct-thing.vue`)
keep their author body native and gain a generated `Contena.Component.attachOverrides(...)` footer;
overrides (`ct-thing.override.vue`) lower to hidden components that register
`Contena.Component.overrideComponentSetup()` callbacks.
...
- **Marker macro** — `ctDefinePublic({...})` / `ctDefineOverride({...})`. Compile-time only; removed
  from the generated output after their entries are extracted. `ctDefinePublic()`'s entries feed the
  ...
  `Contena.Component.getExposedProps()` reads off the instance.
...
- **Source edit** — a range of the SFC plus the chunks that replace it. **Lowering produces every one of
  ...
  `<ct-block>`'s `:data="$dataScope"`, an override block's `#default` slot scope). Both analyses report
