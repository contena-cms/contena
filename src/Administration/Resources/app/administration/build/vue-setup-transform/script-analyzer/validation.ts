/**
 * Enforces the semantic limits of the Contena setup dialect.
 *
 ...
 * (`Contena.Component.attachOverrides(...)` in base mode, `overrideComponentSetup()` in override
 * mode); a user binding of this name would shadow it.
 */
const RESERVED_RUNTIME_GLOBAL_NAME = 'Contena';
 ...
 */
function assertNoUnsupportedSyntax(ast: BabelFile, mode: ContenaSetupMode, scriptOffset: number): void {
    ...
    walk(ast.program, (node, ancestors) => {
        ...
        if (node.type === 'CallExpression' && node.callee.type === 'Identifier') {
            ...
            if (wrongModeCheck) {
                throw new ContenaSetupTransformError(wrongModeCheck.message, absoluteRange(node, scriptOffset));
            }
        }

        // Reject top-level await, in both its forms - `await x` and `for await (… of …)`.
        //  Vue rewrites top-level await into async setup() with context preservation. Contena setup keeps
...
function assertReservedMacroNames(bindings: NamedBinding[], scriptOffset: number): void {
    ...
    bindings.forEach((binding) => {
        ...
        if (helpers.has(binding.name)) {
            throw new ContenaSetupTransformError(
                `"${binding.name}" is reserved by the Contena setup transform and must not be declared or imported.`,
        ...
        if (binding.name === RESERVED_OVERRIDE_STATE_NAME) {
            throw new ContenaSetupTransformError(
                `"${binding.name}" is reserved for Contena override-private state and must not be declared or imported.`,
                absoluteRange(binding.node, scriptOffset),
            );
        }

        if (binding.name.startsWith(CONTENA_SETUP_INTERNAL_PREFIX)) {
            throw new ContenaSetupTransformError(
                `"${binding.name}" uses the reserved "${CONTENA_SETUP_INTERNAL_PREFIX}" prefix of the Contena setup transform and must not be declared or imported.`,
                absoluteRange(binding.node, scriptOffset),
            );
        }
        ...
        if (binding.name === '__proto__') {
            throw new ContenaSetupTransformError(
                '"__proto__" cannot be a Contena setup binding: it collides with prototype-setter syntax in the ' +
                    'generated state object and would be silently dropped. Rename the binding.',
                absoluteRange(binding.node, scriptOffset),
            );
        }

        // The generated footer destructures its state from `Contena.Component.attachOverrides(...)`,
        // so a top-level binding named `Contena` would land on the destructure's left-hand side and
        ...
        if (binding.name === RESERVED_RUNTIME_GLOBAL_NAME) {
            throw new ContenaSetupTransformError(
                `"${RESERVED_RUNTIME_GLOBAL_NAME}" is reserved by the Contena setup transform, which generates code that reads the global "${RESERVED_RUNTIME_GLOBAL_NAME}" object. Rename your binding.`,
                absoluteRange(binding.node, scriptOffset),
            );
        }
    });
