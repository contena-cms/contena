...
type MacroRule = {
    ...
    required?: { modes: ContenaSetupMode[]; message: string };
...
const MACRO_RULES: Record<MacroName, MacroRule> = {
    defineProps: {
        vueBuiltin: true,
        modes: ['base'],
        wrongModeMessage: 'defineProps() is only supported in base Contena setup blocks.',
        setupInput: true,
        exposable: true,
    },
    withDefaults: {
        vueBuiltin: true,
        modes: ['base'],
        wrongModeMessage: 'withDefaults() is only supported in base Contena setup blocks.',
        setupInput: true,
        exposable: true,
    },
    defineEmits: {
        vueBuiltin: true,
        modes: ['base'],
        wrongModeMessage: 'defineEmits() is only supported in base Contena setup blocks.',
        setupInput: true,
        exposable: true,
    },
    defineSlots: {
        vueBuiltin: true,
        modes: ['base'],
        wrongModeMessage: 'defineSlots() is only supported in base Contena setup blocks.',
        setupInput: true,
        exposable: true,
    },
    defineExpose: {
    ...
    defineOptions: {
        vueBuiltin: true,
        modes: ['base'],
        wrongModeMessage: 'defineOptions() is only supported in base Contena setup blocks.',
    },
    defineModel: {
        vueBuiltin: true,
        modes: [],
...
function assertMacroRules(entries: MacroCallEntry[], mode: ContenaSetupMode, scriptOffset: number): void {
    ...
    MACRO_NAMES.forEach((name) => {
        ...
        if (rule.statementOnly && declaration) {
            throw new ContenaSetupTransformError(rule.statementOnly.message, absoluteRange(declaration.call, scriptOffset));
        }
    });
    ...
    MACRO_NAMES.forEach((name) => {
        ...
        if (rule.required?.modes.includes(mode) && entriesFor(name).length === 0) {
            throw new ContenaSetupTransformError(rule.required.message, scriptOffset);
        }
    });
}
...
function getRuntimeInputAliasNames(mode: ContenaSetupMode): Set<string> {
...
function getWrongModeWalkChecks(mode: ContenaSetupMode): { name: MacroName; message: string }[] {
