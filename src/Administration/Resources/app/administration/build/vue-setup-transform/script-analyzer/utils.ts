/**
 * Shared Babel parsing and traversal helpers for the Contena setup analyzer.
 *
...
function getNodeRange(node: BabelNode): SourceRange {
    if (typeof node.start !== 'number' || typeof node.end !== 'number') {
        throw new ContenaSetupTransformError('Missing source range metadata while transforming Contena setup.');
    }
 ...
 * script block's content), while `ContenaSetupTransformError` positions are absolute SFC offsets. This
 ...
 */
function absoluteRange(node: BabelNode, blockOffset: number): ContenaSetupErrorPosition {
