/**
 * Defines and validates Contena setup compiler macros.
 *
...
function extractStaticObjectMarker(
    ...
    return publicObject.properties.map((property) => {
        ...
        if (localName === RESERVED_OVERRIDE_STATE_NAME) {
            throw new ContenaSetupTransformError(
                `"${localName}" is reserved for Contena override-private state and cannot be exposed with ${macroName}().`,
                absoluteRange(property, scriptOffset),
            );
        }
        ...
        if (seenKeys.has(localName)) {
            throw new ContenaSetupTransformError(
                `Duplicate ${entryType} Contena setup binding key "${localName}".`,
                absoluteRange(property, scriptOffset),
            );
        }
