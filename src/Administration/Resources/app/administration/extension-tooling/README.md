...
a Composer install, and a production system all carry the identical toolchain -
matching the installed Contena version. Only the way you invoke the commands
...
| `admin-types.d.ts`   | The one type surface: imports the live `global.types.ts` and generated `entity-schema-definition.d.ts`. Injected into every extension program via `files`.                                                                                                                        |
| `eslint.mjs`         | Parameterized flat-config factory `contenaAdminExtension(options)`. All plugins resolve from the Administration's `node_modules`.                                                                                                                                                 |
...
**In a Composer/Flex-installed system** (the [official installation guide](https://developer.contena.cn/docs/guides/installation/) layout, where the Administration lives under `vendor/contena/administration`), drive the toolchain through `bin/console`:

```bash
# One-time: install the Administration's Node dependencies (they are not part
# of the Composer package). Re-run only after a Contena update.
( cd vendor/contena/administration/Resources/app/administration && npm ci )
...
The console commands resolve the project root automatically and forward everything
...
composer admin:check-extensions -- --only=CtPayPal,MyPlugin   # check specific ones, no prompt
...
projections, so IDEs and a project-wide `eslint .` see exactly what the check command
...
- A git-ignored, self-explaining `.contena/` bridge lands beside each extension's
...
- An extension without configs gets two small ones scaffolded beside the bridge:
    ...
    ```jsonc
    // tsconfig.json
    { "extends": "./.contena/tsconfig.json", "include": ["src/**/*.ts", "src/**/*.vue"] }
    ```

    ```js
    // eslint.config.mjs
    import contena from './.contena/eslint.mjs';
    export default [
        ...contena /* your own rules */,
    ];
    ```
...
second `vue-tsc` pass over a generated spec tsconfig (`.contena/tsconfig.specs.json`) that
...
3. **Annotate** parameters and return types. Use the global `Contena` object and
...
per-file, developer-driven step. `--fix` still applies the ESLint autofixes and Contena
...
This writes a committed `.contena-admin-baseline.json` at the plugin root (custom/plugins
...
permanently, delete the plugin's `.contena-admin-baseline.json`.
...
- **In-repo bundles** (`src/Frontend`, `src/Administration`, …) are writable and their
  findings _are_ fatal, but they are not plugins and must not collect per-developer debt files.
...
| --------------------------------------------------------------------------------- | --------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `var/admin-extension-tooling/manifest.json`                                       | disposable host state       | no                              | Rewritten every run; git-ignored in an installation.                                                                                                                                       |
| Project-root `tsconfig.json` / `eslint.config.mjs` / `.vscode/` / `.zed/`         | disposable host projections | no                              | The project-wide IDE/CLI view; the tsconfig covers whatever no extension config governs. Marker-owned and git-ignored (the platform monorepo commits its own, so setup stands down there). |
| `<plugin>/…/.contena/` (`tsconfig.json`, `eslint.mjs`, `.gitignore`, `README.md`) | git-ignored bridge          | **no**                          | Machine-specific paths into the installed Administration; self-ignoring (`*`) and self-explaining. One per directory that owns a config.                                                   |
...
| `<plugin>/.contena-admin-baseline.json`                                           | committable plugin data     | **yes**                         | Findings baseline; travels with the plugin.                                                                                                                                                |
...
`git-ignored .contena/ bridge file(s)`. Above the list, a `project-root projection`
...
| `⊘ skipped — own tsconfig does not reach the Contena type surface` | The printed `why:` names the exact cause: an own `"files"` array replaces the bridge's type-surface injection (remove it), or the `extends` chain never reaches the preset (add `"extends": "./.contena/tsconfig.json"`). Re-run setup if the `.contena/` bridge is missing.                    |
| `⊘ skipped — own config does not compose the Contena factory`      | Compose the bridge: `import contena from './.contena/eslint.mjs'; export default [ ...contena ];`                                                                                                                                                                                               |
...
`ct-deprecation-rules/*`) turn them into feedback. Internal plugins that
