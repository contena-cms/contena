...
async function createWrapper(existingLanguages = []) {
    ...
    const getMeta = jest.fn().mockResolvedValue({
        ...
        communityTranslationsUrl: 'https://translate.contena.cn',
        documentationUrlSnippetKey: 'ct-settings-language.addModal.docsUrl',
        completenessThreshold: 90,
    });

    const wrapper = mount(await wrapTestComponent('ct-settings-language-add-modal', { sync: true }), {
        global: {
            provide: {
            ...
            stubs: {
                'ct-modal': {
                    template: '<div class="ct-modal"><slot></slot><slot name="modal-footer"></slot></div>',
                },
...
describe('module/ct-settings-language/component/ct-settings-language-add-modal', () => {
    ...
    it('disables the language select while loading', async () => {
        ...
        const select = wrapper.find('.ct-settings-language-add-modal__language-select');
    ...
    it('marks translations as available when completeness is at or above the threshold', async () => {
        ...
        expect(wrapper.vm.translationsHintTextKey).toBe('ct-settings-language.addModal.translationsAvailable');
    });
    ...
    it('warns about missing translations when completeness is below the threshold', async () => {
        ...
        expect(wrapper.vm.translationsHintTextKey).toBe('ct-settings-language.addModal.translationsIncomplete');
    });

    it('reads the documentation url snippet key from the meta response', async () => {
        const { wrapper } = await createWrapper();

        expect(wrapper.vm.documentationUrlSnippetKey).toBe('ct-settings-language.addModal.docsUrl');
    });
