...
describe('module/ct-mail-template/view/ct-mail-template-view-header-footer', () => {
    beforeAll(async () => {
    ...
    it('renders the list and forwards getList to its load method', async () => {
        ...
        const wrapper = shallowMount(await wrapTestComponent('ct-mail-template-view-header-footer', { sync: true }), {
            global: {
                stubs: {
                    'ct-block': defineComponent({ template: '<div><slot /></div>' }),
                    'ct-mail-header-footer-list': defineComponent({
                        name: 'ct-mail-header-footer-list',
                        setup(_, { expose }) {
                            expose({ getList });
                            return {};
                        },
                        template: '<div class="mail-header-footer-list" />',
                    }),
                },
            },
        });

        expect(wrapper.find('.mail-header-footer-list').exists()).toBe(true);
