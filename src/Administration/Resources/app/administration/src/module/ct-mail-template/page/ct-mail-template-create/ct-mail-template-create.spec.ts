...
describe('module/ct-mail-template/page/ct-mail-template-create', () => {
    beforeAll(async () => {
    ...
    it('resets the Administration context to the system default language', async () => {
        ...
        shallowMount(await wrapTestComponent('ct-mail-template-create', { sync: true }), {
            global: {
                stubs: {
                    'ct-block': defineComponent({ template: '<div><slot /></div>' }),
                    'ct-language-switch': true,
                    'ct-mail-template-detail': true,
                },
            },
        });
