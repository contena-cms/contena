...
describe('src/module/ct-experience-studio/store/experience-studio-layout-preset.store.ts', () => {
    ...
    const getStore = () => Contena.Store.get('experienceStudioLayoutPreset' as never) as ExperienceStudioLayoutPresetStore;
    ...
    it('loads presets into an id-keyed map', async () => {
        jest.spyOn(Contena, 'Service').mockReturnValue({ getPresets: () => Promise.resolve(presets) } as never);
    ...
    it('does not reload once loaded unless forced', async () => {
        ...
        jest.spyOn(Contena, 'Service').mockReturnValue({ getPresets } as never);
    ...
    it('records a load error and stays unloaded', async () => {
        jest.spyOn(Contena, 'Service').mockReturnValue({ getPresets: () => Promise.reject(new Error('boom')) } as never);
