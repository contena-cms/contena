...
async function createWrapper(privileges = [], options = {}) {
    return mount(
        await wrapTestComponent('ct-permissions-role-view-general', {
            sync: true,
        }),
        {
            props: {
            ...
            global: {
                ...
                stubs: {
                    ...
                    'ct-permissions-role-access': true,
                    'ct-permissions-role-mcp-reference-modal': true,
                },
                provide: {
...
describe('module/ct-permissions/view/ct-permissions-role-view-general', () => {
    it('should disable the button and fields when no aclPrivileges exists', async () => {
        const wrapper = await createWrapper();

        const fieldRoleName = wrapper.find('input[aria-label="ct-permissions.roles.detail.labelName"]');
        const fieldRoleDescription = wrapper.find('mt-textarea-stub[label="ct-permissions.roles.detail.labelDescription"]');
        const permissionsGrid = wrapper.find('ct-permissions-role-access-stub');
