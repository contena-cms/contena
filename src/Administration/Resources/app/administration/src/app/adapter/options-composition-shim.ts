    ...
    return new Proxy(
        {},
        {
            get(_target: object, prop: string | symbol): unknown {
                ...
                // Forward Vue instance properties ($emit, $t, $route, $router, $refs, $nextTick, etc.)
                if (prop.startsWith('$')) {
...
function logDeprecationWarning(componentName: string): void {
    console.warn(
        `[Deprecation Warning] Component "${componentName}" is being overridden with Options API patterns, ` +
            ...
            `Please migrate your override to use Contena.Component.overrideComponentSetup(). ` +
            `See: https://developer.contena.cn/docs/resources/references/core-reference/administration-reference/composition-api`,
    );
}
