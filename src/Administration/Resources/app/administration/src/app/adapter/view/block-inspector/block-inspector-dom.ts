 ...
 * devtools glue in `ct-vue-devtools-block-inspector.ts` only wires these pieces to the devtools API.
 */

import { BLOCK_MARKER_ATTRIBUTE, parseBlockMarker } from 'src/core/factory/block-inspector';

const OVERLAY_CLASS = 'ct-block-inspector-overlay';
const LABEL_CLASS = 'ct-block-inspector-overlay__label';
...
export function createBlockOverlay(doc: Document = document): BlockOverlay {
    ...
    return {
        show(blockName, elements) {
            ...
            label.textContent = `<ct-block name="${blockName}">`;
            startListening();
            refresh();
        },
        hide() {
