 ...
 * @private
 *
 * "Contena Extension Blocks" inspector for the Vue devtools.
 *
 ...
 *
 * The inspector reads the `data-ct-block` markers rendered by `<ct-block>`. Markers are opt-in and
 * need a reload, which the inspector's power action takes care of.
 */
 ...
 * @private
 */
export const BLOCK_INSPECTOR_ID = 'ct-admin-extension-block-inspector';
...
function countNativeExtensions(block: InspectedBlock): number {
    const blockContext = useBlockContext();
    const scopedBlockName = block.component === 'unknown' ? block.name : `${block.component} ${block.name}`;

    return blockContext.getBlocks(scopedBlockName).length || blockContext.getBlocks(block.name).length;
}

function isExtended(block: InspectedBlock): boolean {
    return countNativeExtensions(block) > 0;
}
...
function collectTreeBlocks(): TreeBlock[] {
    return Array.from(collectMarkedBlocks().keys()).map((blockName) => {
        ...
        return { name: block.name, component: block.component, extended: isExtended(block) };
    });
}
...
function buildState(blockName: string): CustomInspectorState {
    ...
    return {
        Block: [
            ...
            { key: 'Kind', value: 'Native <ct-block>' },
        ...
        Extensions: [{ key: 'Native <ct-block extends>', value: countNativeExtensions(block) }],
        Snippets: [{ key: 'Native extension', value: nativeSnippet(block) }],
    };
}
...
/**
 * Adds the block inspector to the Contena devtools plugin. `root` is the mounted root instance
...
export default function setupBlockInspector(api: DevtoolsPluginApi<unknown>, root?: unknown): void {
    ...
    api.addInspector({
        ...
        label: 'Contena Extension Blocks',
        icon: 'view_quilt',
