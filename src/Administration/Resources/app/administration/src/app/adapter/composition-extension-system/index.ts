 ...
 * this module for them. Reachable on the `Contena.Component` global only because generated code has to
 * resolve it at runtime.
 *
 * The wrapper separates public and private setup state, applies all registered Composition API and
 * Options API shim overrides once, and returns a data scope that `ct-block` can read during slot
 * rendering.
 */
// eslint-disable-next-line ct-deprecation-rules/private-feature-declarations
...
>(
    options: {
        ...
        props: TProps;
        context?: TContext;
        preservePropNamedSetupState?: boolean;
    },
    originalSetup: (
        props: TProps,
        context: TContext,
    ...
    Object.keys(options.props).forEach((key) => {
        // Native <script setup> may legitimately declare a local binding with the same name as a prop.
        // The generated attachOverrides() footer must keep it so its destructuring does not replace the
        // author's binding with undefined. Legacy wrappers still reject and remove returned props.
        if (options.preservePropNamedSetupState) {
            return;
        }
    ...
    // Plugins always register overrides (via Contena.Component.override()) before the Vue
    // application mounts, so all pending overrides are present in the registry at this point.
    void (async () => {
        try {
            const overrideRegistry = Contena?.Component?.getOverrideRegistry?.();
            if (!overrideRegistry) {
                // Contena global not available (e.g. in unit tests that don't bootstrap the app)
                return;
            }
 ...
 *
 * The generated `defineExpose()` of a base component spreads these in front of its ctDefinePublic()
...
export function getExposedProps(): Record<string, ComputedRef<unknown>> {
    const props = (vueGetCurrentInstance()?.props ?? {}) as Record<string, unknown>;
    ...
    return createExtendableSetup(
        {
            ...
            preservePropNamedSetupState: true,
        },
        () =>
            ({
