...
jest.mock('src/app/adapter/view/ct-vue-devtools', () => {
    return jest.fn();
});
...
describe('ASYNC app/adapter/view/vue.adapter.js', () => {
    ...
    beforeEach(async () => {
        ...
        if (!Contena.Service('loginService')) {
        ...
        if (!Contena.Service('localeToLanguageService')) {
            Contena.Service().register('localeToLanguageService', () => {
                return {
        ...
        Contena.Store.get('system').locales = [
            'en-GB',
            'zh-CN',
        ];

        Contena.Store.get('session').setAdminLocaleState({
            locales: [
                'en-GB',
                'zh-CN',
            ],
    ...
    it('setLocaleFromUser should not set the user when user does not exist', async () => {
        ...
        expect(Contena.Service('localeHelper').setLocaleWithId).not.toHaveBeenCalled();
    });

    it('setLocaleFromUser should set the user when user does not exist', async () => {
        Contena.Store.get('session').setCurrentUser({ localeId: '12345' });
        vueAdapter.setLocaleFromUser();

        expect(Contena.Service('localeHelper').setLocaleWithId).toHaveBeenCalled();
    });

    it('setLocaleFromUser should call the service with the user id from the store', async () => {
        const expectedId = '12345678';
        Contena.Store.get('session').setCurrentUser({ localeId: expectedId });

        vueAdapter.setLocaleFromUser();

        expect(Contena.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith(expectedId);
    });
    ...
    it('initLocales should watch for user changes and recall the "setLocaleWithId"', async () => {
        ...
        expect(Contena.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith('german-id');
    });

    it('should resolve mixins by explicit Mixin get by name call', async () => {
        Contena.Mixin.register('foo1', {
            methods: {
                fooBar() {
                    return this.title;
                },
        ...
        Contena.Component.register('test-component1', {
            template: '<div></div>',
        ...
        Contena.Component.markComponentAsSync('test-component1');
    ...
    it('should resolve mixins by explicit Mixin get by name call with override', async () => {
        ...
        Contena.Component.register('test-component2', {
            template: '<div></div>',
        ...
        Contena.Component.override('test-component2', {
            data() {
                return {
        ...
        Contena.Component.markComponentAsSync('test-component2');
    ...
    it('should resolve mixins by string', async () => {
        ...
        Contena.Component.register('test-component3', {
            template: '<div></div>',
        ...
        Contena.Component.markComponentAsSync('test-component3');
    ...
    it('should resolve mixins by string with override', async () => {
        ...
        Contena.Component.register('test-component4', {
            template: '<div></div>',
        ...
        Contena.Component.override('test-component4', {
            data() {
                return {
        ...
        Contena.Component.markComponentAsSync('test-component4');
    ...
    it('should resolve mixins for component in combination with overrides', async () => {
        Contena.Mixin.register('foo-with-data', {
            data() {
                return {
                    sortBy: null,
                };
        ...
        Contena.Component.register('test-component-foobar-with-mixin', {
            template: '<div></div>',
            name: 'test-component',
            data() {
                return {
        ...
        Contena.Component.markComponentAsSync('test-component-foobar-with-mixin');
        ...
        Contena.Component.override('test-component-foobar-with-mixin', {});
    ...
    it('should extend mixins', async () => {
        ...
        Contena.Component.register('extendable-component', {
        ...
        Contena.Component.markComponentAsSync('ct-test-component-extended');
        const buildComp = await vueAdapter.createComponent('ct-test-component-extended');
        const wrapper = shallowMount(await buildComp);
    ...
    it('should allow multi-inheritance with multiple mixins and lifecycle hooks are only executed once', async () => {
        ...
        Contena.Mixin.register('first-mixin', {
            created() {
                lifecycleSpy();
            },
            methods: {
        ...
        Contena.Mixin.register('second-mixin', {
            methods: {
                bar() {
                    return 'bar';
                },
        ...
        Contena.Component.register('base-component', {
            template: '<div class="base-component"></div>',
        });

        Contena.Component.override('base-component', {
            mixins: ['first-mixin'],
        });
        ...
        Contena.Component.markComponentAsSync('base-component');
    ...
    it('should compose route guards from extends, mixins and component config for route-loaded components', async () => {
        ...
        const suffix = Contena.Utils.createId();
        ...
        Contena.Mixin.register(firstMixinName, {
        ...
        Contena.Mixin.register(secondMixinName, {
            beforeRouteEnter(to, from, next) {
        ...
        Contena.Component.register(baseComponentName, {
            template: '<div></div>',
        ...
        Contena.Component.extend(componentName, baseComponentName, {
            template: '<div></div>',
            name: componentName,
            mixins: [
                firstMixinName,
    ...
    it('should ignore synchronous errors thrown after next() in callback-style route guards', async () => {
        ...
        Contena.Component.register(componentName, {
            template: '<div></div>',
            name: componentName,
            beforeRouteUpdate(to, from, next) {
                next();
    ...
    describe('should initialize everything correctly', () => {
        ...
        beforeEach(async () => {
            ...
            const locale = Contena.Application.getContainer('factory').locale;
            if (!locale.getLocaleByName('en-GB')) {
                locale.register('en-GB', {
                    global: {
                        'ct-admin-menu': {
                            textContenaAdmin: 'Text Contena Admin',
                        },
                        my: {
                            mock: {
                                title: 'Mock title',
            ...
            if (!Contena.Filter.getByName('my-mock-filter')) {
                Contena.Filter.register('my-mock-filter', () => {
                    return 'mocked';
                });
            }

            if (!Contena.Directive.getByName('my-mock-directive')) {
                Contena.Directive.register('my-mock-directive', () => {
                    return {
                        bind() {},
                        inserted() {},
                        update() {},
            ...
            // add main component
            if (!Contena.Component.getComponentRegistry().has('ct-admin')) {
                Contena.Component.register('ct-admin', {
                    template: '<div class="ct-admin"></div>',
                });
            }

            // add VueAdapter to Contena object
            Contena.Application.setViewAdapter(vueAdapter);
        ...
        it('should initialize the plugins correctly', async () => {
            ...
            expect(rootComponent.config.globalProperties.$t).toBeDefined();
        ...
        it('should have correct working createTitle method', () => {
            ...
            expect(result).toBe('Test | Mock title | Text Contena Admin');
        });

        it('should add all components to the root component', () => {
            expect(rootComponent._context.components['ct-admin']).toBeDefined();
        });
        ...
        it('should setup the devtools in development environment', async () => {
            expect(setupContenaDevtools).toHaveBeenCalled();
        });
