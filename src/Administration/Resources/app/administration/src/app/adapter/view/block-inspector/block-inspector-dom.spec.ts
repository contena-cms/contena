...
describe('adapter/view/block-inspector/block-inspector-dom', () => {
    ...
    describe('finding marked elements', () => {
        it('reads block names innermost first', () => {
            render('<div id="el" data-ct-block="inner outer"></div>');
        ...
        it('finds the closest marked ancestor of a target', () => {
            render('<div id="block" data-ct-block="a"><p><b id="leaf">x</b></p></div><i id="free"></i>');
        ...
        it('lists every enclosing block from the inside out', () => {
            render(
                '<div data-ct-block="outer"><div data-ct-block="middle inner-of-middle"><span id="leaf" data-ct-block="leaf"></span></div></div>',
            );
        ...
        it('groups marked elements by block name in document order', () => {
            render(
                '<p id="one" data-ct-block="a"></p><p id="two" data-ct-block="b a"></p><p id="three" data-ct-block="b"></p>',
            );
    ...
    describe('containedBlockTree', () => {
        it('nests the blocks inside a block the way their elements nest', () => {
            render(
                '<div id="outer" data-ct-block="b_outer"><p data-ct-block="b_inner"><b data-ct-block="b_leaf"></b></p></div>',
            );
        ...
        it('lists siblings next to each other', () => {
            render('<div id="outer" data-ct-block="b_outer"><p data-ct-block="a"></p><p data-ct-block="b"></p></div>');
        ...
        it('treats blocks that share the element as nested, innermost first', () => {
            render('<div id="el" data-ct-block="b_inner b_outer"></div>');
        ...
        it('lists a block name that starts in several places only once', () => {
            render('<div id="outer" data-ct-block="b_outer"><p data-ct-block="dup"></p><p data-ct-block="dup"></p></div>');
        ...
        it('is empty for a block without blocks inside it', () => {
            render('<div id="leaf" data-ct-block="only"><span>text</span></div>');
    ...
    describe('documentBlockTree', () => {
        it('nests every block of the page, outermost blocks as roots', () => {
            render(
                '<div data-ct-block="root_a"><p data-ct-block="inner_a"><b data-ct-block="leaf_a"></b></p></div>' +
                    '<div data-ct-block="root_b"></div>',
            );
        ...
        it('nests blocks that share an element, innermost last', () => {
            render('<div data-ct-block="inner outer"><p data-ct-block="deep"></p></div>');
        ...
        it('makes a block whose enclosing block is out of scope a root of its own', () => {
            render('<div data-ct-block="outside"><section id="scope"><p data-ct-block="inside"></p></section></div>');
    ...
    describe('overlay', () => {
        it('frames the elements and labels them with the block name', () => {
            ...
            overlay.show('ct_block_name', [document.getElementById('target')!]);

            const frame = document.querySelector<HTMLElement>('.ct-block-inspector-overlay')!;
            ...
            expect(frame.textContent).toBe('<ct-block name="ct_block_name">');
            expect(frame.classList.contains('ct-block-inspector-overlay--label-inside')).toBe(false);
            ...
            expect(document.querySelector('.ct-block-inspector-overlay')).toBeNull();
        });
        ...
        it('moves the label inside the frame when the frame touches the top edge', () => {
            ...
            const overlay = createBlockOverlay();
            mockRect(document.querySelector('.ct-block-inspector-overlay__label')!, {
                top: 0,
                left: 0,
                width: 120,
                height: 20,
            ...
            expect(
                document
                    .querySelector('.ct-block-inspector-overlay')!
                    .classList.contains('ct-block-inspector-overlay--label-inside'),
            ).toBe(true);
        ...
        it('keeps the label on screen for a block that starts left of the viewport', () => {
            ...
            const overlay = createBlockOverlay();
            const label = document.querySelector<HTMLElement>('.ct-block-inspector-overlay__label')!;
        ...
        it('hides the frame when the elements lose their box', () => {
            ...
            expect(document.querySelector<HTMLElement>('.ct-block-inspector-overlay')!.hidden).toBe(true);
    ...
    describe('picking', () => {
        it('reports the innermost block under the pointer and picks it on click without the page seeing the click', () => {
            render('<div data-ct-block="outer"><button id="btn" data-ct-block="inner outer">x</button></div>');
        ...
        it('reports leaving all blocks and cancels on Escape', () => {
            render('<div id="block" data-ct-block="a"></div><div id="free"></div>');
            const onHover = jest.fn();
            const onPick = jest.fn();
            const onStop = jest.fn();
            startBlockPicking({ onHover, onPick, onStop });
