...
const createWrapper = async (
    ...
    return shallowMount(tooltipComponent, {
        ...
        global: {
            components,
            directives: {
                tooltip: Contena.Directive.getByName('tooltip'),
            },
        },
    });
};
...
describe('directives/tooltip', () => {
    it('should show and hide tooltip', async () => {
        ...
        let tooltips = document.body.getElementsByClassName('ct-tooltip');
        ...
        tooltips = document.body.getElementsByClassName('ct-tooltip');
        // Tooltip gets rendered
    ...
    it('should not be created when target element gets deleted before creation of tooltip', async () => {
        ...
        const tooltips = document.body.getElementsByClassName('ct-tooltip');
        expect(tooltips).toHaveLength(0);
    });
    ...
    it('should not disappear if you hover the tooltip itself', async () => {
        ...
        const tooltip = document.body.getElementsByClassName('ct-tooltip').item(0);
        ...
        expect(document.body.getElementsByClassName('ct-tooltip').item(0)).not.toBeNull();
    });
    ...
    it('should clamp bottom tooltip horizontally when it would overflow the viewport', async () => {
        ...
        const offsetWidthSpy = jest.spyOn(HTMLElement.prototype, 'offsetWidth', 'get').mockImplementation(function () {
            return this.classList.contains('ct-tooltip') ? 100 : 0;
        });
        const offsetHeightSpy = jest.spyOn(HTMLElement.prototype, 'offsetHeight', 'get').mockImplementation(function () {
            return this.classList.contains('ct-tooltip') ? 30 : 0;
        });
        const getBoundingClientRectSpy = jest
            .spyOn(HTMLElement.prototype, 'getBoundingClientRect')
            .mockImplementation(function () {
                if (this.classList.contains('ct-tooltip')) {
        ...
        try {
            ...
            expect(tooltip).not.toBeNull();
            expect(tooltip.classList.contains('ct-tooltip--bottom')).toBe(true);
