...
describe('src/app/component/entity/ct-meteor-entity-data-table PR 1 behavior', () => {
    it('creates a repository from the entity prop when no repository override is provided', async () => {
        const repository = {
            search: jest.fn(() => Promise.resolve(createSearchResult([{ id: 'tag-1' }], 1))),
        };
        ...
        expect(repositoryFactory.create).toHaveBeenCalledWith('tag');
