...
async function createWrapper(slots) {
    return mount(await wrapTestComponent('ct-multi-select-filter', { sync: true }), {
        global: {
            stubs: {
                'ct-base-filter': {
                    template: '<div class="ct-base-filter"><slot></slot></div>',
                    props: [
                        'showResetButton',
                        'active',
                    ],
                ...
                'ct-entity-multi-select': {
                    template:
                        '<div class="ct-entity-multi-select"><slot name="selection-label-property"></slot><slot name="result-item"></slot></div>',
                    props: [
                        'value',
                        'options',
                        'labelProperty',
                ...
                'mt-select': {
                    template: '<div class="mt-select"><slot></slot></div>',
                    props: [
                        'modelValue',
                        'options',
                        'labelProperty',
...
describe('src/app/component/filter/ct-multi-select-filter', () => {
    it('Should display title and placeholder', async () => {
        const wrapper = await createWrapper();

        expect(wrapper.get('.ct-base-filter').attributes('title')).toBe('Test');
        expect(wrapper.getComponent('.ct-entity-multi-select').props('placeholder')).toBe('placeholder');
    });

    it('should emit `filter-update` event when user chooses entity', async () => {
        const wrapper = await createWrapper();

        await wrapper.getComponent('.ct-entity-multi-select').vm.$emit('update:entity-collection', entities);
    ...
    it('should emit `filter-update` event when user chooses translated entity', async () => {
        ...
        await wrapper.getComponent('.ct-entity-multi-select').vm.$emit('update:entity-collection', translatedEntities);
    ...
    it('should emit `filter-reset` event when click Reset button', async () => {
        ...
        await wrapper.getComponent('.ct-base-filter').vm.$emit('filter-reset');
        expect(wrapper.emitted('filter-update')).toBeFalsy();
        expect(wrapper.emitted('filter-reset')).toBeTruthy();
    });

    it('should should pass `active` to the `ct-base-filter`', async () => {
        const wrapper = await createWrapper();

        await wrapper.setProps({ active: true });

        expect(wrapper.getComponent('.ct-base-filter').props('active')).toBe(true);
    });
    ...
    it('should display mt-select if filter has options', async () => {
        const wrapper = await createWrapper();
        ...
        expect(wrapper.getComponent('.mt-select').props('options')).toStrictEqual(filter.options);
    });

    it('should emit filter-update with correct value when filter is mt-select', async () => {
        const wrapper = await createWrapper();
        ...
        await wrapper.getComponent('.mt-select').vm.$emit('update:modelValue', [filter.options[0].key]);
    ...
    it('should show product variant when displayVariants attribute of filter is true', async () => {
        ...
        expect(wrapper.getComponent('.ct-entity-multi-select').props('displayVariants')).toBe(true);
    });
    ...
    it('should reset filter if no value is selected', async () => {
        ...
        await wrapper.getComponent('.mt-select').vm.$emit('update:modelValue', []);
