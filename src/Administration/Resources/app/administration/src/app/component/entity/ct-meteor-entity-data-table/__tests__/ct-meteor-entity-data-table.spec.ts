...
describe('src/app/component/entity/ct-meteor-entity-data-table', () => {
    afterEach(() => {
    ...
    it('renders loading state while repository search is pending', async () => {
        ...
        searchDeferred.resolve(createSearchResult([{ id: 'tag-1', name: 'Contena' }], 1));
        await flushPromises();
    ...
    it('ignores stale repository results when a newer load has already started', async () => {
        ...
        secondSearch.resolve(createSearchResult([{ id: 'tag-2', name: 'Meteor' }], 1));
        await reloadPromise;
        await flushPromises();

        firstSearch.resolve(createSearchResult([{ id: 'tag-1', name: 'Contena' }], 1));
        await flushPromises();

        expect(wrapper.vm.records).toEqual([{ id: 'tag-2', name: 'Meteor' }]);
    ...
    it('translates column labels before passing them to mt-data-table', async () => {
        const wrapper = await createWrapper(
            {
                columns: [
                    {
                        property: 'name',
                        label: 'ct-settings-tag.list.columnName',
                    },
                ],
            },
            {
        ...
        expect(wrapper.vm.resolvedColumns[0].label).toBe('translated: ct-settings-tag.list.columnName');
    });
    ...
    it('renders the preview fallback when a preview image value is missing', async () => {
        ...
        expect(wrapper.get('.ct-meteor-entity-data-table__preview-image').attributes('src')).toBe(
    ...
    it('routes preview fallback assets through the configured assets path', async () => {
        const previousAssetsPath = Contena.Context.api.assetsPath;
        Contena.Context.api.assetsPath = 'https://cdn.example.test/';
        ...
        try {
            ...
            Contena.Context.api.assetsPath = previousAssetsPath;
        }
    });

    it('keeps real media preview URLs unchanged when an assets path is configured', async () => {
        const previousAssetsPath = Contena.Context.api.assetsPath;
        const mediaUrl = 'https://media.example.test/tag.png';
        Contena.Context.api.assetsPath = 'https://cdn.example.test/';
        ...
        try {
            ...
            expect(wrapper.get('.ct-meteor-entity-data-table__preview-image').attributes('src')).toBe(mediaUrl);
        } finally {
            Contena.Context.api.assetsPath = previousAssetsPath;
        }
    });
    ...
    it('stops loading and returns an empty result when criteriaResolver returns null', async () => {
        const repository = {
            search: jest.fn(() => Promise.resolve(createSearchResult([{ id: 'tag-1' }]))),
        };
    ...
    it('forwards column-property slots with Meteor scope values only', async () => {
        ...
        expect(wrapper.get('.meteor-column-slot').text()).toBe('Contena|name|true|true|true');
    });
    ...
    it('forwards preview-property slots with Meteor scope values', async () => {
        ...
        expect(wrapper.text()).toContain('Contena');
    });
