...
describe('components/base/ct-version', () => {
    let wrapper;

    async function createWrapper(version) {
        Contena.Store.get('context').app.config.version = version;

        wrapper = mount(await wrapTestComponent('ct-version', { sync: true }), {
            global: {
                stubs: ['ct-color-badge'],
            },
        });
    }
