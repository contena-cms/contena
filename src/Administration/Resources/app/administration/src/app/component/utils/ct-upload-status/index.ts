...
import { type Snackbar, useSnackbar } from '@contena/meteor-component-library';
...
const IgnoredErrors = [
    ResponseErrorCodes.DUPLICATED_FILE_NAME, // Handled by ct-duplicated-media-v2
] as const;

const ErrorMessages = {
    [ResponseErrorCodes.ILLEGAL_FILE_NAME]: 'global.ct-media-upload.notification.illegalFilename.message',
    [ResponseErrorCodes.ILLEGAL_URL]: 'global.ct-media-upload.notification.illegalFileUrl.message',
    [ResponseErrorCodes.ILLEGAL_FILE_TYPE]: 'global.ct-media-upload.notification.illegalFileType.message',
    [ClientErrorCodes.REQUEST_TIMEOUT]: 'global.ct-media-upload.notification.transportError.message',
    [ClientErrorCodes.REQUEST_CANCELED]: 'global.ct-media-upload.notification.requestCanceled.message',
} as const;

const StatusMessages = {
    PAYLOAD_TOO_LARGE: 'global.ct-media-upload.notification.payloadTooLarge.message',
    TRANSPORT_ERROR: 'global.ct-media-upload.notification.transportError.message',
} as const;
...
export default Contena.Component.wrapComponentConfig({
    ...
    mixins: [
        Contena.Mixin.getByName('notification'),
    ],
    data() {
        return {
    ...
    computed: {
        ...
        snackbarMessage(): string {
            ...
            return this.$t('global.ct-media-upload.snackbar.message', {
                count: uploadCount,
            ...
            if (this.uploadComplete) {
                Object.assign(config, {
                    ...
                    errorMessage: this.hasFailedUploads
                        ? this.$t('global.ct-media-upload.snackbar.errorMessage', { count: uploadCount })
                        : undefined,
                    duration: 0,
                });
            }
        ...
        showErrorNotification(payload: UploadFailedPayload) {
            ...
            if (messageSnippets.length === 0 && !hasIgnoredError) {
                ...
                if (typeof message !== 'string' || message.length === 0) {
                    message = this.$t('global.ct-media-upload.notification.failure.message');
                }
