...
import 'src/app/component/grid/ct-pagination';

describe('src/component/grid/ct-pagination', () => {
    let wrapper;

    function getActivePage() {
        return wrapper.find('.ct-pagination__list-button.is-active');
    }

    function getButtonAtPosition(position) {
        const allPageButtons = wrapper.findAll('button.ct-pagination__list-button');
    ...
    function getPositionOfActiveButton() {
        ...
        return allPageButtons.findIndex((currentElement) => currentElement.classes().includes('is-active'));
    }
    ...
    async function createWrapper() {
        return mount(await wrapTestComponent('ct-pagination', { sync: true }), {
            props: {
                total: 275,
                limit: 25,
                page: 1,
    ...
    it('should have the right amount of elements', async () => {
        const pageButtons = wrapper.findAll('button.ct-pagination__list-button');
        expect(pageButtons).toHaveLength(6);

        const separator = wrapper.findAll('.ct-pagination__list-separator');
        expect(separator).toHaveLength(1);

        const activeButton = wrapper.findAll('.ct-pagination__list-button.is-active');
    ...
    it('should have right amount of elements when on third page', async () => {
        ...
        const allPageButtons = wrapper.findAll('.ct-pagination__list-button');
        expect(allPageButtons).toHaveLength(7);

        const separator = wrapper.findAll('.ct-pagination__list-separator');
        expect(separator).toHaveLength(1);

        const activePageButton = wrapper.find('.ct-pagination__list-button.is-active');
    ...
    it('should navigate to another page via page button', async () => {
        ...
        const secondPageButton = wrapper.find('button.ct-pagination__list-button:not(.is-active)');
    ...
    it('should have right amount of elements when setting the prop totalVisible to 3', async () => {
        ...
        expect(wrapper.findAll('.ct-pagination__list-button')).toHaveLength(3);
        expect(wrapper.findAll('.ct-pagination__list-separator')).toHaveLength(1);

        expect(wrapper.find('.ct-pagination__list-button.is-active').exists()).toBe(true);

        const rightArrow = wrapper.find('.icon--regular-chevron-right-xs');
        await rightArrow.trigger('click');

        expect(wrapper.findAll('.ct-pagination__list-button')).toHaveLength(3);
        expect(wrapper.findAll('.ct-pagination__list-separator')).toHaveLength(2);
    });
