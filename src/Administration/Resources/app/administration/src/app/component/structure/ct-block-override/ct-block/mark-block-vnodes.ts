...
/**
 * Marks the root elements a block renders with the block name. Development-only; the caller checks
 * that the block inspector is enabled.
 *
 * Elements and components take the marker as an attribute, which falls through to a component's
 * root. Fragments are walked into, text and comments stay as they are. An existing marker from an
 * enclosing native block is extended instead of replaced.
 *
 * @private
 */
