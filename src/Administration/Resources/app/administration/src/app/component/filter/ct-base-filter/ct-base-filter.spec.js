...
async function createWrapper() {
    return mount(await wrapTestComponent('ct-base-filter', { sync: true }), {
        props: {
...
describe('components/ct-base-filter', () => {
    it('should hide reset button by default', async () => {
        const wrapper = await createWrapper();

        expect(wrapper.find('.ct-base-filter__reset').exists()).toBeFalsy();
    });
    ...
    it('should show reset button when showResetButton is true', async () => {
        ...
        expect(wrapper.find('.ct-base-filter__reset').exists()).toBeTruthy();
