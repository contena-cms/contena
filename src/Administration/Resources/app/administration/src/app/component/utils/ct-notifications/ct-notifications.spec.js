...
jest.mock('src/app/composables/use-notification-translation', () => ({
    useNotificationTranslation: () => ({
        getTranslatedTitle: ({ title }) => (title ? `Translated: ${title}` : ''),
        getTranslatedMessage: ({ message }) => message ?? '',
    }),
}));

describe('src/app/component/utils/ct-notifications', () => {
    beforeEach(() => {
        setActivePinia(createPinia());
    });

    it('should translate title when it is a translation key', async () => {
        const wrapper = mount(await wrapTestComponent('ct-notifications', { sync: true }), {
            global: {
                stubs: {
                    'mt-banner': {
