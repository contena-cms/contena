...
async function createWrapper(customOptions = {}) {
    return mount(await wrapTestComponent('ct-media-url-form', { sync: true }), {
        props: {
            variant: 'inline',
        },
        global: {
...
async function setComponentState(wrapper, state) {
    Object.assign(wrapper.vm, state);
    await wrapper.vm.$nextTick();
}

describe('src/app/component/media/ct-media-url-form', () => {
    it('should render in inline mode by default', async () => {
        const wrapper = await createWrapper();
        await flushPromises();

        expect(wrapper.find('.ct-media-url-form__url-input').exists()).toBeTruthy();
        expect(wrapper.find('.ct-media-url-form__submit-button').exists()).toBeTruthy();
    });
    ...
    it('should render in modal mode when variant is set to modal', async () => {
        ...
        await setComponentState(wrapper, { showModal: true });
        await flushPromises();
        await wrapper.vm.$nextTick();
        ...
        expect(modalContent.find('.ct-media-url-form__url-input').exists()).toBeTruthy();

        const modalFooter = modal.find('.mt-modal__footer');
        expect(modalFooter.exists()).toBeTruthy();
        expect(modalFooter.find('.ct-media-url-form__submit-button').exists()).toBeTruthy();
    });
    ...
    it('should render in inline mode when variant is not modal', async () => {
        ...
        const urlInput = wrapper.find('.ct-media-url-form__url-input');
        const submitButton = wrapper.find('.ct-media-url-form__submit-button');
    ...
    it('should show file extension input when URL has no extension', async () => {
        const wrapper = await createWrapper();
        await setComponentState(wrapper, {
            url: 'https://example.com/image',
        });
        await flushPromises();

        expect(wrapper.find('.ct-media-url-form__extension-input').exists()).toBeTruthy();
    });
    ...
    it('should not show file extension input when URL has extension', async () => {
        ...
        expect(wrapper.find('.ct-media-url-form__extension-input').exists()).toBeFalsy();
    });
    ...
    it('should show error message when URL is invalid', async () => {
        ...
        expect(wrapper.find('.ct-media-url-form__url-input').attributes('error')).toBeDefined();
    });
