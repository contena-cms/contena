...
import { getRecordsFromResult, getStateSnapshot, getTotalFromResult } from '../ct-meteor-entity-data-table.utils';
import type { MeteorEntityTableLoadSuccessPayload } from '../ct-meteor-entity-data-table.types';
...
export function useMeteorEntityTableState(options: UseMeteorEntityTableStateOptions) {
    ...
    const load = async (): Promise<MeteorEntityTableRecord[]> => {
        ...
        try {
            ...
            if (!repository) {
                throw new Error('ct-meteor-entity-data-table requires either a repository or an entity.');
            }
