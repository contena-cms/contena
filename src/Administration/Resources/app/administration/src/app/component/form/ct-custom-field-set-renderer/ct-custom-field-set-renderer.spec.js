...
function createEntityCollection(entities = []) {
    return new Contena.Data.EntityCollection('collection', 'collection', {}, null, entities);
}
...
async function createWrapper(props, options = {}) {
    ...
    return mount(
        await wrapTestComponent('ct-custom-field-set-renderer', {
            sync: true,
        }),
        {
            props,
            global: {
                stubs: {
                    ...
                    'ct-form-field-renderer': await wrapTestComponent('ct-form-field-renderer', {
                        sync: true,
                    }),
                    ...
                    'ct-media-field': await wrapTestComponent('ct-media-field'),
                    'ct-media-media-item': await wrapTestComponent('ct-media-media-item'),
                    'ct-media-base-item': await wrapTestComponent('ct-media-base-item'),
                    'ct-media-preview-v2': await wrapTestComponent('ct-media-preview-v2'),
                    ...
                    'mt-text-editor': {
                        props: ['modelValue'],
                        template:
                            '<input type="text" :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)"/>',
                    },
                    'ct-skeleton': await wrapTestComponent('ct-skeleton'),
                    'ct-skeleton-bar': await wrapTestComponent('ct-skeleton-bar'),
...
async function withTranslatedLanguageContext(
    ...
    const previousLanguage = Contena.Store.get('context').api.language;

    Contena.Context.api.languageId = languageId;
    Contena.Context.api.systemLanguageId = systemLanguageId;
    Contena.Store.get('context').api.language = {
        id: languageId,
        parentId,
    };
    ...
    try {
        ...
        Contena.Store.get('context').api.language = previousLanguage;
        Contena.Context.api.languageId = previousLanguageId;
        Contena.Context.api.systemLanguageId = previousSystemLanguageId;
    }
}

describe('src/app/component/form/ct-custom-field-set-renderer', () => {
    /** @type Wrapper */
    let wrapper;
    ...
    const configuredFields = [
        {
            ...
            domFieldValueSelectorBefore: '.mt-select-selection-list__input',
            domFieldValueBefore: 'First choice',
            domFieldValueSelectorAfter: '.mt-select-selection-list__input',
            domFieldValueAfter: 'Second choice',
            changeValueFunction: async (customField) => {
                // open select field
                await customField.find('.mt-select__selection').trigger('click');
                await flushPromises();

                // check if second option exists
                const secondChoiceOption = customField.find('.mt-select-option--second_choice');
        ...
        },
        {
            ...
            domFieldValueSelectorBefore: '.mt-select-selection-list__item-holder--0',
            domFieldValueBefore: 'First choice',
            domFieldValueSelectorAfter: '.mt-select-selection-list__item-holder--1',
            domFieldValueAfter: 'Second choice',
            changeValueFunction: async (customField) => {
                // open select field
                await customField.find('.mt-select__selection').trigger('click');
        ...
        },
        {
            ...
            domFieldValueSelectorBefore: '.ct-media-base-item__name',
            domFieldValueBefore: 'media_before.jpg',
            domFieldValueSelectorAfter: '.ct-media-base-item__name',
            domFieldValueAfter: 'media_after.jpg',
            changeValueFunction: async (customField) => {
                // change input value
                await customField.find('.ct-media-field__toggle-button').trigger('click');
                await flushPromises();
                await customField
                    .find('.ct-media-field__suggestion-list-entry:first-child .ct-media-base-item')
                    .trigger('click');
            },
        },
        {
        ...
        },
        {
            ...
            domFieldValueSelectorBefore: '.ct-entity-single-select__selection-text',
            domFieldValueBefore: 'Germany',
            domFieldValueSelectorAfter: '.ct-entity-single-select__selection-text',
            domFieldValueAfter: 'Vietnam',
            changeValueFunction: async (customField) => {
                // open select field
                await customField.find('.ct-entity-single-select__selection').trigger('click');
                await flushPromises();

                // check if second option exists
                const secondChoiceOption = customField.find('.ct-select-option--1');
                expect(secondChoiceOption.isVisible()).toBe(true);
    ...
    it('should inherit the value from parent entity', async () => {
        ...
        const customFieldEl = wrapper.find('.ct-inherit-wrapper input[name=customFieldName]');
    ...
    it('should render translated custom field values without an explicit parent entity', async () => {
        ...
        await withTranslatedLanguageContext({}, async () => {
            ...
            const customTextFieldEl = wrapper.find('.ct-form-field-renderer-field__translatedTextField input[type="text"]');
            const customCheckboxFieldEl = wrapper.find(
                '.ct-form-field-renderer-field__translatedCheckboxField input[type="checkbox"]',
            );
    ...
    it('should enable translated inheritance for a non-system root language', async () => {
        ...
        await withTranslatedLanguageContext({ parentId: null }, async () => {
            ...
            const textField = wrapper.find('.ct-form-field-renderer-field__translatedTextField input[type="text"]');
            const inheritanceSwitch = wrapper.find(
                '.ct-form-field-renderer-field__translatedTextField .mt-inheritance-switch',
            );
    ...
    it('should keep translated inheritance per custom field when another translated field has its own value', async () => {
        ...
        await withTranslatedLanguageContext({}, async () => {
            ...
            const checkboxInheritanceSwitch = wrapper.find(
                '.ct-form-field-renderer-field__translatedCheckboxField .mt-inheritance-switch',
            );
            ...
            const restoredTextField = wrapper.find('.ct-form-field-renderer-field__translatedTextField input[type="text"]');
    ...
    it('should render inherited false and zero values in translated custom fields', async () => {
        ...
        await withTranslatedLanguageContext({}, async () => {
            ...
            const numberField = wrapper.find('.ct-form-field-renderer-field__translatedNumberField input[type="text"]');
    ...
    it('should not filter custom field sets when selection not active', async () => {
        ...
        const tabs = wrapper.findAll('.mt-tabs__item');
    ...
    it('should initialize new custom fields on entity change', async () => {
        ...
        props.entity.customFields = { initialized: true };

        wrapper.vm.onChangeCustomFieldSetSelectionActive();

        await flushPromises();

        expect(wrapper.vm.customFields).toEqual({ initialized: true });
    });
    ...
    it('should sort sets by position', async () => {
        ...
        expect(wrapper.vm.visibleCustomFieldSets).toHaveLength(2);
        expect(wrapper.vm.visibleCustomFieldSets[0].name).toBe('set2');
    });
    ...
    it('should show the tabs', async () => {
        ...
        expect(wrapper.findAll('.mt-tabs__item').at(0).text()).toContain('Sports');
        expect(wrapper.findAll('.mt-tabs__item').at(1).text()).toContain('Clothing');
    });
    ...
    it('should contain the right fields for each tab', async () => {
        ...
        // get tab contents
        const tabContentSports = wrapper.find('.ct-custom-field-set-renderer-tab-content__custom_sports');
        const tabContentClothing = wrapper.find('.ct-custom-field-set-renderer-tab-content__custom_clothing');
        ...
        const footballField = tabContentSports.find('.ct-form-field-renderer-input-field__custom_sports_football');
        const scoreField = tabContentSports.find('.ct-form-field-renderer-input-field__custom_sports_score');

        expect(footballField.exists()).toBe(true);
        expect(scoreField.exists()).toBe(true);

        // check if fields get render correctly
        expect(footballField.attributes('componentname')).toBe('ct-field');
        expect(footballField.attributes('type')).toBe('text');

        expect(scoreField.attributes('componentname')).toBe('ct-field');
        expect(scoreField.attributes('type')).toBe('number');

        // get fields for clothing tab
        const soccerField = tabContentClothing.find('.ct-form-field-renderer-input-field__custom_sports_soccer');
        expect(soccerField.exists()).toBe(true);

        // check if fields get render correctly
        expect(soccerField.attributes('componentname')).toBe('ct-field');
        expect(soccerField.attributes('type')).toBe('text');

        // click on clothing tab
        await wrapper.findAll('.mt-tabs__item').at(1).trigger('click');
        await flushPromises();
    ...
    it('should load the current active tab', async () => {
        ...
        const footballField = tabContentSports.find('.ct-form-field-renderer-input-field__custom_sports_football');
        const scoreField = tabContentSports.find('.ct-form-field-renderer-input-field__custom_sports_score');
        let nameField = tabContentClothing.find('.ct-form-field-renderer-input-field__custom_clothing_name');
        ...
        // get fields for clothing tab
        nameField = tabContentClothing.find('.ct-form-field-renderer-input-field__custom_clothing_name');
    ...
    configuredFields.forEach(
        ({
            ...
            it(`should render the custom field and update value: ${testFieldLabel}`, async () => {
                ...
                const customField = wrapper.find(`.ct-form-field-renderer-field__${fieldName}`);
            ...
            it(`should render the custom field with parent value: ${testFieldLabel}`, async () => {
                ...
                const inheritanceSwitch = isMeteorComponent
                    ...
                    : wrapper.find('.ct-inheritance-switch');
                ...
                if (isMeteorComponent) {
                    ...
                    expect(inheritanceSwitch.classes()).toContain('ct-inheritance-switch--is-inherited');
                }
            });
            ...
            it(`should render the custom field with his value when has also parent value: ${testFieldLabel}`, async () => {
                ...
                if (isMeteorComponent) {
                    ...
                    expect(inheritanceSwitch.classes()).toContain('ct-inheritance-switch--is-not-inherited');
                }
            });
    ...
    it.each([
        ...
        async ({ customFields, expected }) => {
            ...
            const inputField = wrapper.find('.ct-form-field-renderer-field__field1 input');
            expect(inputField.exists()).toBe(true);

            expect(inputField.element.value).toBe(expected);
        },
    );
});
