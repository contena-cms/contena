...
import 'src/app/component/form/ct-file-input';
import 'src/app/component/context-menu/ct-context-menu-item';

async function createWrapper(customOptions = {}) {
    return mount(await wrapTestComponent('ct-file-input', { sync: true }), {
        ...customOptions,
    });
}
...
describe('src/app/component/form/ct-file-input', () => {
    let wrapper;

    beforeEach(async () => {
        wrapper = await createWrapper();

        fileInput = wrapper.find('.ct-file-input__file-input');
    ...
    it('upload button should be enabled', async () => {
        const uploadButton = wrapper.find('.ct-file-input__button');
    ...
    it('should show error notification able file type is not suitable', async () => {
        ...
        expect(wrapper.vm.createNotificationError).toHaveBeenCalledWith({
            message: 'global.ct-file-input.notification.invalidFileType.message',
    ...
    it('should show error notification able file size is not suitable', async () => {
        ...
        expect(wrapper.vm.createNotificationError).toHaveBeenCalledWith({
            message: 'global.ct-file-input.notification.invalidFileSize.message',
            title: 'global.default.error',
        });
    });
    ...
    it('should not show header and remove icon', async () => {
        const fileName = wrapper.find('.ct-file-input__file-headline');
        const removeIcon = wrapper.find('.ct-file-input__remove-icon');
    ...
    it('should able to remove file when click on remove icon', async () => {
        ...
        let removeIcon = wrapper.find('.ct-file-input__remove-icon');
        await removeIcon.trigger('click');

        expect(wrapper.vm.selectedFile).toBeNull();

        removeIcon = wrapper.find('.ct-file-input__remove-icon');
        expect(removeIcon.exists()).toBeFalsy();
    });
    ...
    it.each([
        ...
        expect(wrapper.vm.createNotificationError).toHaveBeenCalledWith({
            message: 'global.ct-file-input.notification.invalidFileExtension.message',
            title: 'global.default.error',
        });
        expect(wrapper.vm.selectedFile).toBeNull();
    });
