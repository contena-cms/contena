import { createWrapper, setupDateFilterHooks } from './ct-date-filter.fixtures';

const { Criteria } = Contena.Data;

describe('src/app/component/filter/ct-date-filter - range inputs and rendering', () => {
    setupDateFilterHooks();

    it('should emit `filter-update` event when `From` value exists', async () => {
        const wrapper = await createWrapper();
        const input = wrapper.find('.ct-date-filter__from').find('input');
    ...
    it('should emit `filter-update` event when `To` value exists', async () => {
        const wrapper = await createWrapper();
        const input = wrapper.find('.ct-date-filter__to').find('input');
    ...
    it('should emit `filter-update` event when `From` value `To` value exists', async () => {
        ...
        const fromInput = wrapper.find('.ct-date-filter__from').find('input');
        ...
        const toInput = wrapper.find('.ct-date-filter__to').find('input');
    ...
    it('should emit user timezone aware criteria for date ranges', async () => {
        Contena.Store.get('session').setCurrentUser({ timeZone: 'Europe/Berlin' });
    ...
    it('should return default dateType of ct-datepicker', async () => {
        const wrapper = await createWrapper();
    ...
    it('should render timeframe field', async () => {
        ...
        const timeframe = wrapper.find('.ct-date-filter__timeframe');
