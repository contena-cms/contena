...
const images = [
    {
        src: '/administration/static/img/contena-logo-v4.svg',
    ...
    '/administration/static/fixtures/ct-test-image.webp',
    '/administration/static/img/contena-logo-v4.svg',
];
...
async function createWrapper(additionalProps = {}) {
    return mount(await wrapTestComponent('ct-image-slider', { sync: true }), {
        props: {
            ...{
                canvasWidth: 218,
                canvasHeight: 229,
...
describe('src/app/component/media/ct-image-slider', () => {
    it('should display every image, even in an object, independent if the link is external or not', async () => {
        const wrapper = await createWrapper();
        const containerScrollable = wrapper.find('.ct-image-slider__image-scrollable');
        const actualImages = wrapper.findAll('.ct-image-slider__image-scrollable .ct-image-slider__element-image');
    ...
    it('should display descriptions, if enabled and existing', async () => {
        ...
        const actualDescriptions = wrapper.findAll(
            '.ct-image-slider__image-scrollable .ct-image-slider__element-description',
        );
    ...
    it('should navigate using the buttons', async () => {
        ...
        const buttons = wrapper.findAll('.ct-image-slider__buttons-element');
        const containerScrollable = wrapper.find('.ct-image-slider__image-scrollable');
    ...
    it('should mark aria-hidden correctly when navigating', async () => {
        ...
        let expectedIndex;
        const imageWrappers = wrapper.findAll('.ct-image-slider__element-wrapper');
    ...
    it('should show active border around item after clicking on it', async () => {
        ...
        let expectedIndex = 0;
        const imageContainers = wrapper.findAll('.ct-image-slider__element-container');
