...
describe('app/plugins/deprecated.plugin', () => {
    ...
    beforeEach(() => {
        guard = jest.spyOn(Contena.Feature, 'triggerDeprecationOrThrow').mockImplementation(() => {});
    });
    ...
    it('[prop] should guard if the deprecated prop is explicitly set to its default value', async () => {
        component = createComponent({
            customComponent: {
                props: {
                    example: {
        ...
        expect(guard).toHaveBeenCalledWith('V6_4_0_0', expect.stringContaining('example'));
    });

    it('[prop] should guard when a deprecated prop is supplied in kebab case', async () => {
        component = createComponent({
            customComponent: {
    ...
    it('throws once the major flag of the deprecation is active', async () => {
        guard.mockRestore();
        guard = jest.spyOn(Contena.Feature, 'isActive').mockReturnValue(true);
