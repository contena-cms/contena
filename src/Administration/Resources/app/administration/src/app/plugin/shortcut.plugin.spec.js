...
Contena.Utils.debounce = function debounce() {
...
describe('app/plugins/shortcut.plugin', () => {
    ...
    it('should not call the shortcut method when keyboard shortcuts are disabled', async () => {
        const onSaveMock = jest.fn();
        const originalService = Contena.Service.bind(Contena);
        const serviceSpy = jest.spyOn(Contena, 'Service').mockImplementation((serviceName) => {
    ...
    it('Text editor component: should be blurred on save shortcut to react to content changes', async () => {
        ...
        Contena.Component.register('base-component', {
            ...
            methods: {
                onEditorBlur(event) {
                    this.description = event.currentTarget.innerHTML;
                },
                onSave() {
                    onSaveMock();
        ...
        wrapper = mount(await Contena.Component.build('base-component'), {
            attachTo: element,
            global: {
                plugins: [shortcutPlugin],
            },
        ...
        const contentEditor = wrapper.find('.mt-text-editor__content-editor');
