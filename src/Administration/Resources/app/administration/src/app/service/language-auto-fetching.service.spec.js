...
describe('src/app/service/language-auto-fetching.service.js', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(Contena.Service('repositoryFactory'), 'create').mockImplementation(() => ({
            get: repositoryGetMock,
        }));

        Contena.Store.get('context').api.languageId = 'initial-language-id';
    ...
    it('should load the language on initialization', async () => {
        ...
        expect(Contena.Store.get('context').api.language).toEqual(
    ...
    it('should reload the language when languageId changes', async () => {
        ...
        Contena.Store.get('context').api.languageId = 'new-language-id';
        await flushPromises();
