...
const { warn, error } = Contena.Utils.debug;
const { object } = Contena.Utils;
...
export default class PrivilegesService {
    ...
    private defaultUserPrivileges = [
        ...
        'log_entry:create', // for ct-error-boundary
     ...
     *      'rule:create'
     *    ]
     * _getPrivilegesWithDependencies('tag.creator', false);
     *
     * @example
     * // returns [
     *      'tag:read',
     ...
     *      'tag.creator'
     *    ]
     * _getPrivilegesWithDependencies('tag.creator', true);
     */
    private _getPrivilegesWithDependencies(adminPrivilegeKey: string, shouldAddAdminPrivilege = true): string[] {
        ...
        /**
         * Get all privileges (['media:read', fn(), 'media:update', ...])
         * and dependencies (['media.viewer', ...])
         */
     ...
     *             privileges: [
     *                 'media.read',
     *                 Contena.Service('privileges').getPrivileges('rule.viewer')
     *             ],
     *             dependencies: []
     *         }
     *     }
     ...
     *      ...
     *    ]
     * getPrivilegesForAdminPrivilegeKeys(['tag.viewer', 'media.creator'])
     */
