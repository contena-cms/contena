...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, Contena.Context.api).then((response) => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = Contena.Store.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(Contena.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        return this._userConfigRepository
            .save(filterEntity, Contena.Context.api)
            .then(() => {
    ...
    async _pushFiltersToUrl(replaceRoute = false) {
        ...
        const router = Contena.Application.view.router;
        ...
        if (!Contena.Utils.types.isEmpty(routeParams)) {
