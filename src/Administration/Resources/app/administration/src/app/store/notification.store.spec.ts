...
import { getNotificationsForUser } from './notification.store';
import type { NotificationType } from './notification.store';

describe('notifications.store', () => {
    beforeAll(() => {
        Contena.Store.get('session').currentUser = { id: '1' } as EntitySchema.user;
    });
    ...
    it('should have the initial state', () => {
        const notification = Contena.Store.get('notification');
    ...
    it('adds default actions to notifications restored from local storage', () => {
        ...
        expect(getNotificationsForUser().notification).toEqual(
            expect.objectContaining({
                actions: [],
            }),
        );

        localStorage.removeItem('notifications#1');
    });
