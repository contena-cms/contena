import type ContenaError from 'src/core/data/ContenaError';
 ...
 * */
function setApiErrorNested({ expression, error, store }: { expression: string; error: ContenaError; store: ErrorStore }) {
...
function removeApiError(expression: string, store: ErrorStore) {
    ...
    if (Contena.Utils.types.isEmpty(store)) {
 ...
 */
const errorStore = Contena.Store.register({
    id: 'error',
    ...
    state: () =>
        ({
            ...
            system: Record<string, ContenaError>;
            api: Record<string, ContenaError>;
        },

    actions: {
        addApiError({ expression, error }: { expression: string; error: ContenaError }) {
        ...
        addSystemError({ error, id = utils.createId() }: { error: ContenaError; id?: string }) {
    ...
    getters: {
        getErrorsForEntity(state) {
            return (entityName: string, id: string) => {
                ...
                // @ts-expect-error
                return (entityStorage[id] as ContenaError) ?? null;
            };
        },
