...
export function usePlaceholder(): {
    ...
    function placeholder<EntityName extends keyof EntitySchema.Entities>(
        ...
        if (Contena.Utils.types.isString(entity[field]) && entity[field].length > 0) {
            return entity[field];
        }

        // Return the field from parent translation if set
        const parentLanguageId = Contena.Context.api.language ? Contena.Context.api.language.parentId : null;
