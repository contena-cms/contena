...
describe('src/app/composables/use-create-title', () => {
    it('builds the title most specific part first', async () => {
        const createTitle = await createTitleIn({ $module: { title: 'ct-blog.general.mainMenuItemGeneral' } });

        expect(createTitle('Article')).toBe(
            'Article | ct-blog.general.mainMenuItemGeneral | global.ct-admin-menu.textContenaAdmin',
        );
    });
    ...
    it('omits an absent identifier', async () => {
        ...
        expect(createTitle()).toBe('ct-blog.general.mainMenuItemGeneral | global.ct-admin-menu.textContenaAdmin');
    });

    it('appends additional parameters', async () => {
        const createTitle = await createTitleIn({ $module: { title: 'ct-category.general.mainMenuItemList' } });

        expect(createTitle('Category', 'News')).toBe(
            'News | Category | ct-category.general.mainMenuItemList | global.ct-admin-menu.textContenaAdmin',
        );
    });
