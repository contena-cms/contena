...
/**
 * The media item is passed as a getter so it stays reactive and writes to `isLoading` land on the
 * entity supplied by the host component.
 *
 * @private
 */
...
export default function useVideoCover(options: UseVideoCoverOptions): {
    ...
    function openCoverSelectionModal(): void {
        if (!Contena.Service('acl').can('media.editor')) {
            return;
        }
    ...
    async function persistCoverMedia(coverMediaId: string | null): Promise<void> {
        ...
        try {
            await Contena.Service('mediaService').assignVideoCover(item.id, coverMediaId);

            const snippetKey = coverMediaId
                ? 'global.ct-media-media-item.notification.coverSaveSuccess.message'
                : 'global.ct-media-media-item.notification.coverRemoveSuccess.message';

            createNotificationSuccess({ message: i18n.t(snippetKey) });

            Contena.Utils.EventBus.emit('ct-media-library-item-updated', item.id);
        } catch {
            createNotificationError({
                message: i18n.t('global.ct-media-media-item.notification.coverSaveError.message'),
            });
        } finally {
    ...
    async function onCoverSelectionChange(selection: VideoCoverMedia[]): Promise<void> {
        ...
        if (!media || !isImage(media)) {
            createNotificationError({
                message: i18n.t('global.ct-media-media-item.notification.coverSelectionInvalid.message'),
            });
