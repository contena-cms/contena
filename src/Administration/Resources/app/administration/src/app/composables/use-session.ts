...
async function setAdminLocale(locale: string): Promise<void> {
    ...
    const localeToLanguageService = Contena.Service('localeToLanguageService') as LocaleToLanguageService;
    return localeToLanguageService.localeToLanguage(locale).then((languageIdFromService) => {
        ...
        Contena.Application.getContainer('factory').locale.storeCurrentLocale(locale);
    });
}
...
function setAdminLocaleState({
    ...
    if (!locales.find((l) => l === locale)) {
        Contena.Utils.debug.warn('SessionStore', `Locale ${locale} not registered at store`);
        return;
    }
