...
jest.mock('./use-notification', () => ({
    useNotification: () => ({ createNotificationSuccess, createNotificationError }),
}));

const assignVideoCover = jest.fn();
const emit = jest.fn();

function stubContena(can = true): void {
    window.Contena = {
...
describe('src/app/composables/use-video-cover', () => {
    ...
    it('assigns the selected image as the cover and announces the update', async () => {
        ...
        expect(createNotificationSuccess).toHaveBeenCalledWith({
            message: 'global.ct-media-media-item.notification.coverSaveSuccess.message',
        });
        expect(emit).toHaveBeenCalledWith('ct-media-library-item-updated', 'video-1');
    ...
    it('rejects a selection that is not an image', async () => {
        ...
        expect(createNotificationError).toHaveBeenCalledWith({
            message: 'global.ct-media-media-item.notification.coverSelectionInvalid.message',
        });
    });
    ...
    it('removes the cover by assigning null', async () => {
        ...
        expect(createNotificationSuccess).toHaveBeenCalledWith({
            message: 'global.ct-media-media-item.notification.coverRemoveSuccess.message',
        });
    });
    ...
    it('reports a failed assignment and stops the item loading', async () => {
        ...
        expect(createNotificationError).toHaveBeenCalledWith({
            message: 'global.ct-media-media-item.notification.coverSaveError.message',
        });
        expect(item.isLoading).toBe(false);
    });
