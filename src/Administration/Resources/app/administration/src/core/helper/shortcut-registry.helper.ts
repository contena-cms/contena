...
function systemKey(): string {
    deviceHelper ??= new Contena.Helper.DeviceHelper() as unknown as DeviceHelper;

    return deviceHelper.getSystemKey();
}

function areShortcutsDisabled(): boolean {
    const shortcutService = Contena.Service('shortcutService') as { isShortcutsDisabled?: () => boolean } | undefined;
...
function isNavigationSequence(sequence: string): boolean {
    const registry = Contena.Application.getContainer('factory')?.shortcut?.getShortcutRegistry?.();
...
function handleKeyDown(event: KeyboardEvent): void {
    ...
    if (eventTarget?.closest('.ct-modal, .ct-modal__dialog, .mt-modal')) {
        resetSequence();
