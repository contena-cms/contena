...
describe('src/core/helper/shortcut-registry.helper', () => {
    ...
    it('ignores every keystroke while the shortcut service reports shortcuts as disabled', () => {
        ...
        jest.spyOn(Contena, 'Service').mockReturnValue({ isShortcutsDisabled: () => true } as never);
        add('SYSTEMKEY+S', onSave);
    ...
    it('does not trigger a single key shortcut while a navigation shortcut sequence is typed', () => {
        const onToggle = jest.fn();
        const shortcutFactory = Contena.Application.getContainer('factory').shortcut;

        shortcutFactory.register('GS', '/ct/settings/index');
        add('S', onToggle);
        ...
        shortcutFactory.getShortcutRegistry().delete('GS');
    });

    it('does not trigger a single key shortcut when a modifier key is pressed', () => {
        const onToggle = jest.fn();
    ...
    it('does not trigger shortcuts from inside a Meteor modal', () => {
        const onToggle = jest.fn();
        const modal = document.createElement('div');
        ...
        modal.dispatchEvent(new KeyboardEvent('keydown', { key: 's', bubbles: true }));
    ...
    it('ignores repeated keydown events while a key is held', () => {
        const onToggle = jest.fn();
