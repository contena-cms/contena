...
export default function createRouter(Router, View, moduleFactory, LoginService) {
    ...
    function addGlobalNavigationGuard(router) {
        ...
        router.beforeEach(async (to) => {
            ...
            const isPasswordResetRoute = to.path.startsWith('/login/reset/');
            ...
            if (!loggedIn && (to.name === 'login' || loginAllowlist.includes(to.path) || isPasswordResetRoute)) {
                return true;
            }
            ...
            if (to.meta.privilege && !Contena.Service('acl').can(to.meta.privilege)) {
                return { name: 'ct.privilege.error.index' };
            }
    ...
    function convertRouteComponentToViewComponent(route) {
        const hasOwnProperty = Contena.Utils.object.hasOwnProperty;
    ...
    function getViewComponent(componentName) {
        return Contena.Application.view.getComponentForRoute(componentName);
    }

    function getAssetPath() {
        return Contena.Context.api.assetsPath;
    }
    ...
    function setModuleFavicon(routeDestination, assetsPath) {
        ...
        favRef.href = favicon
            ...
            : `${assetsPath}administration/static/img/favicon/favicon-v4-32x32.png`;
