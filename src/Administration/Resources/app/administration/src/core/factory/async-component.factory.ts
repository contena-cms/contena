...
import { defineComponent, getCurrentInstance, isRef, proxyRefs, shallowRef, watch } from 'vue';
...
const extensionParentSetup = new WeakMap<object, Record<string, unknown>>();

/**
 * Returns the setup bindings produced by the direct parent of an SFC component extension.
 * This is only available synchronously while the extending component's setup function runs.
 *
 * @public
 */
function getExtensionParentSetup(): Record<string, unknown> {
    ...
    return extensionParentSetup.get(instance) ?? {};
}

function mergeExtensionSetup(parentConfig: ComponentConfig, childConfig: ComponentConfig): void {
    const parentSetup = parentConfig.setup;
    const childSetup = childConfig.setup;

    if (typeof parentSetup !== 'function' || typeof childSetup !== 'function') {
        return;
    }

    childConfig.setup = (props, context) => {
        const exposedBindings: Record<string, unknown> = {};
        const mergedContext = {
            ...context,
            expose(bindings?: Record<string, unknown>): void {
                Object.assign(exposedBindings, bindings);
            },
        };

        // Vue's public ComponentOptions setup type intentionally resolves to `any` here.
        const parentSetupResult = parentSetup(props, mergedContext) as unknown;
        const parentBindings =
            parentSetupResult && typeof parentSetupResult === 'object' && !(parentSetupResult instanceof Promise)
                ? (parentSetupResult as Record<string, unknown>)
                : { ...exposedBindings };

        const instance = getCurrentInstance();
        if (instance) {
            extensionParentSetup.set(instance, createExtensionParentBindings(parentBindings));
            exposeSetupBindingsToRenderContext(instance, parentBindings);
        }

        const childSetupResult = childSetup(props, mergedContext) as unknown;

        context.expose(exposedBindings);

        if (typeof childSetupResult === 'function' || childSetupResult instanceof Promise) {
            return childSetupResult;
        }

        const childBindings: Record<string, unknown> =
            childSetupResult && typeof childSetupResult === 'object' ? (childSetupResult as Record<string, unknown>) : {};
        const mergedBindings = {
            ...parentBindings,
            ...childBindings,
        };

        if (typeof parentSetupResult === 'function') {
            syncExtensionBindingsWithParent(parentBindings, childBindings);
        }

        if (instance) {
            exposeSetupBindingsToRenderContext(instance, mergedBindings);
        }

        if (typeof parentSetupResult === 'function') {
            return parentSetupResult;
        }
...
function createExtensionParentBindings(parentBindings: Record<string, unknown>): Record<string, unknown> {
    return Object.fromEntries(
        Object.entries(parentBindings).map(
            ([
                key,
                binding,
            ]) => {
                if (isRef(binding) && typeof binding.value === 'function') {
                    return [
                        key,
                        shallowRef(binding.value),
                    ];
                }
...
function exposeSetupBindingsToRenderContext(instance: object, bindings: Record<string, unknown>): void {
    // Vue passes this internal context as `_ctx` to render functions generated from inline SFC templates.
    const renderContext = (instance as { ctx: Record<string, unknown> }).ctx;
    const renderContextBindings = proxyRefs(bindings) as Record<string, unknown>;

    Object.keys(bindings).forEach((key) => {
        Object.defineProperty(renderContext, key, {
            configurable: true,
            enumerable: true,
            get: () => renderContextBindings[key],
            set: (value: unknown) => {
                renderContextBindings[key] = value;
            },
        });
    });
}

function syncExtensionBindingsWithParent(
    parentBindings: Record<string, unknown>,
    childBindings: Record<string, unknown>,
): void {
    Object.entries(childBindings).forEach(
        ([
            key,
            childBinding,
        ]) => {
            const parentBinding = parentBindings[key];

            if (!isRef(parentBinding) || !isRef(childBinding) || typeof parentBinding.value !== 'function') {
                return;
            }
 ...
 * @private
 */
async function build(componentName: string): Promise<ComponentConfig | boolean> {
    ...
    if (config.extends) {
        ...
        if (typeof config.extends === 'string') {
            const buildedComp = await build(config.extends);
        ...
        if (extendComp) {
            ...
            mergeExtensionSetup(extendComp, config);
     ...
     */
    const inheritsSfcRender = hasInheritedRender(config);
    const inheritsVueTemplate = hasInheritedVueTemplate(config);

    if (typeof config?.render === 'function' || config?._renderedBySfcTemplate || inheritsSfcRender) {
        config._hasSfcTemplateRender = true;
        delete config.template;
        delete config._renderedBySfcTemplate;
        return config;
    }

    if (typeof config?.template !== 'string' && !inheritsVueTemplate) {
        return false;
    }
...
function hasInheritedRender(config: ComponentConfig): boolean {
    let parent = config.extends;

    while (parent && typeof parent !== 'string') {
        if (typeof parent.render === 'function' || parent._hasSfcTemplateRender === true) {
            return true;
        }
...
function hasInheritedVueTemplate(config: ComponentConfig): boolean {
    let parent = config.extends;
