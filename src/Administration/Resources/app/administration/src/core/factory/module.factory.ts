...
interface DefaultSearchConfiguration {
    _searchable?: boolean;
    _score?: number;
    [field: string]: boolean | number | DefaultSearchConfiguration | undefined;
}
...
export interface ModuleManifest {
    ...
    icon?: string;
    favicon?: string;
    defaultSearchConfiguration?: DefaultSearchConfiguration & { _searchable: boolean };
    searchMatcher?: (regex: RegExp, labelType: string, manifest: ModuleManifest) => false | ModuleSearchResult[];
}
...
function getModuleRegistry(): Map<string, ModuleDefinition> {
    modules.forEach((value, key) => {
        if (hasOwnProperty(value.manifest, 'flag') && !Contena.Feature.isActive(value?.manifest?.flag ?? '')) {
            modules.delete(key);
        }
    });
...
function registerModule(moduleId: string, module: ModuleManifest): false | ModuleDefinition {
    ...
    if (hasOwnProperty(module, 'navigation') && module.navigation) {
        ...
        module.navigation = module.navigation.filter((navigationEntry) => {
            ...
            if (module.type === 'plugin' && (navigationEntry.path || navigationEntry.link) && !navigationEntry.privilege) {
                warn(
                    'ModuleFactory',
...
function getModuleRoutes(): CtRouteConfig[] {
    ...
    modules.forEach((module) => {
        module.routes.forEach((route) => {
            if (hasOwnProperty(route, 'flag') && !Contena.Feature.isActive(route.flag ?? '')) {
                return;
            }
...
function addSettingsItemsToStore(moduleId: string, module: ModuleManifest): void {
    if (hasOwnProperty(module, 'flag') && !Contena.Feature.isActive(module.flag ?? '')) {
        return;
    }
    ...
    module.settingsItem.forEach((settingsItem) => {
        if (settingsItem.group && settingsItem.to && (settingsItem.icon || settingsItem.iconComponent)) {
            if (module.type === 'plugin' && !settingsItem.privilege) {
                warn(
                    'ModuleFactory',
...
function addEntryRouteToExtensionRouteStore(config: { extensionName: string; route: string }): void {
    ...
    Contena.Store.get('extensionEntryRoutes').addItem(config);
}
