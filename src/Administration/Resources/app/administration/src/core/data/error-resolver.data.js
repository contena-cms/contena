...
export default class ErrorResolver {
    ...
    resetApiErrors() {
        return Contena.Store.get('error').resetApiErrors();
    }
    ...
    handleDeleteError(errors) {
        errors.forEach(({ error, entityName, id }) => {
            const contenaError = new this.ContenaError(error);
            Contena.Store.get('error').addSystemError({
                error: contenaError,
            });
    ...
    reduceErrorsByWriteIndex(errors) {
        ...
        errors.forEach((current) => {
            ...
            segments.reduce((pointer, segment, index) => {
                ...
                if (index === lastIndex) {
                    pointer[segment] = new this.ContenaError(current);
                } else {
                    pointer[segment] = {};
                }
    ...
    addSystemErrors(systemErrors) {
        systemErrors.forEach((error) => {
            Contena.Store.get('error').addSystemError(error);
        });
    }
    ...
    resolveError(fieldName, error, definition, entity, changeset) {
        ...
        if (!(error instanceof this.ContenaError)) {
            error = new this.ContenaError(error);
        }
    ...
    buildAssociationChangeset(entity, changeset, error, associationName) {
        if (!changeset || !Contena.Utils.object.hasOwnProperty(changeset, associationName)) {
            Contena.Store.get('error').addApiError({
    ...
    resolveJsonFieldError(basePath, error) {
        Object.keys(error).forEach((fieldName) => {
            ...
            if (error[fieldName] instanceof this.ContenaError) {
                Contena.Store.get('error').addApiError({
                    expression: path,
