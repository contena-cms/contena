...
export default class Repository<EntityName extends keyof EntitySchema.Entities> {
    ...
    get schema(): EntityDefinition<any> {
        return Contena.EntityDefinition.get(this.entityName);
    }
     ...
     */
    save(entity: Entity<EntityName>, context = Contena.Context.api): Promise<void | AxiosResponse> {
     ...
     */
    saveAll(entities: EntityCollection<EntityName>, context = Contena.Context.api): Promise<unknown> {
     ...
     * Detects changes of all provided entities and send the changes to the server
     */
    async sync(entities: EntityCollection<EntityName>, context = Contena.Context.api, failOnError = true): Promise<unknown> {
     ...
     * @private
     */
    sendUpserts(changeset: $TSDangerUnknownObject[], failOnError: boolean, context = Contena.Context.api): Promise<void> {
     ...
     */
    syncDeleted(ids: string[], context = Contena.Context.api): Promise<void> {
        const headers = this.buildHeaders(context);
     ...
     */
    sendDeletions(queue: DeletionQueue, context = Contena.Context.api): Promise<AxiosResponse[]> {
        const headers = this.buildHeaders(context);
     ...
     */
    buildHeaders(context = Contena.Context.api): {
        Accept: string;
        Authorization: string;
        'Content-Type': string;
        'ct-api-compatibility': boolean;
        ...
        let headers: { [key: string]: unknown } = {
            ...
            'ct-api-compatibility': compatibility as boolean,
        };

        if (context.languageId) {
            headers = {
                'ct-language-id': context.languageId,
                ...headers,
            };
        }
        ...
        if (context.versionId) {
            headers = {
                'ct-version-id': context.versionId,
                ...headers,
            };
        }
        ...
        if (context.inheritance) {
            headers = {
                'ct-inheritance': context.inheritance,
                ...headers,
            };
        }
    ...
    private runCachedRead<T>(cacheOptions: RepositoryCacheOptions | undefined, fn: () => Promise<T>): Promise<T> {
        ...
        return Contena.Service('cacheService').query({
