...
Contena.Component, Contena.Service(), Contena.Store
...
- **service.factory**: DI container (`Contena.Service().register()`)
- **state.factory**: Pinia stores (`Contena.Store.register()`)
...
- **criteria**: Query builder from `@contena/meteor-admin-sdk`
...
❌ Bypassing Contena.X global (breaks decoration)
