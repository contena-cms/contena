...
import { handleFactory, send } from '@contena/meteor-admin-sdk/es/channel';
import SerializerFactory from '@contena/meteor-admin-sdk/es/_internals/serializer';
...
describe('core/service/extension-api-data.service.ts', () => {
    it('should keep functions on entity', async () => {
        const entity = new Entity(Contena.Utils.createId(), 'jest', {
            name: 'jest',
        });
    ...
    it('should update entity', async () => {
        const entity = new Entity(Contena.Utils.createId(), 'jestupdate', {
            name: 'beforeupdate',
        });
        ...
        jest.spyOn(window, 'addEventListener').mockImplementationOnce((event, handler) => {
            const e = new Entity(Contena.Utils.createId(), 'jestupdate', {
                name: 'beforeupdate',
            });
            ...
            const data = {
                ...
                _callbackId: Contena.Utils.createId(),
            };
    ...
    it('should update collection', async () => {
        ...
        collection.add(
            new Entity(Contena.Utils.createId(), 'jest', {
                id: 1,
                name: 'jest1',
            }),
        );
            ...
            mediaCollection: new EntityCollection(
                ...
                [
                    new Entity('image1', 'media', {
                        url: 'https://contena.com/image1.jpg',
                        tags: new EntityCollection('image1/tags', 'tag', {}, null, [
                            ...
                            new Entity('tag3', 'tag', {
                                name: 'Contena Community',
                            }),
                        ]),
                    }),
                    new Entity('image2', 'media', {
                        url: 'https://contena.com/image2.jpg',
    ...
    it.each([
        ...
        if (env === 'prod') {
            ...
            Contena.Utils.debug.warn = mock;
        } else {
            envBefore = process.env;
            process.env = {};
            Contena.Utils.debug.error = mock;
        }

        Contena.Store.get('extensions').addExtension({
            name: 'JestApp',
