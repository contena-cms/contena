 ...
 */
import { updateSubscriber, register, handleGet } from '@contena/meteor-admin-sdk/es/data';
import get from 'lodash-es/get';
import debounce from 'lodash-es/debounce';
import cloneDeepWith from 'lodash-es/cloneDeepWith';
import { selectData } from '@contena/meteor-admin-sdk/es/_internals/data/selectData';
import MissingPrivilegesError from '@contena/meteor-admin-sdk/es/_internals/privileges/missing-privileges-error';
...
handleGet((data, additionalOptions) => {
    ...
    if (registeredDataSet.deprecated) {
        const extension = Object.values(Contena.Store.get('extensions').extensionsState).find((ext) =>
        ...
        if (process.env.NODE_ENV !== 'production') {
            Contena.Utils.debug.error(...debugArgs);
        } else {
            Contena.Utils.debug.warn(...debugArgs);
        }
    }
    ...
    updateSubscriber(id, (value) => {
        ...
        function setObject(transferObject: transferObject, prefix: string[] = []): void {
            ...
            Object.keys(transferObject).forEach((property) => {
                ...
                if (
                    // @ts-expect-error
                    Contena.Utils.hasOwnProperty(transferObject[property], 'getDraft', this) &&
                    // @ts-expect-error
                    typeof transferObject[property].getDraft === 'function'
                ) {
                    setObject(
                        {
                            [property]: Contena.Utils.object.cloneDeep(transferObject[property]),
                        },
                        segments,
                    );
...
function instanceScope(instance: scopeInterface, path: string): PublishScope {
    return {
        ...
        write: (segments, value) => {
            ...
            Contena.Utils.object.get(instance, parsedPath.pathToLastSegment)[parsedPath.lastSegment] = value;
        },
