 ...
 * @ct-package fundamentals@framework
 * @private
 */
export default class UserConfigService extends ApiService {
    ...
    search(keys: string[] | null = null, forceReload = false) {
        const currentUser = Contena.Store.get('session').currentUser as SessionUser | undefined;
        const currentUserId = currentUser?.id ?? 'anonymous';
        const cacheService = Contena.Service('cacheService') as CacheServiceContract;
        ...
        return cacheService
            ...
            .catch((error) => {
                Contena.Utils.debug.error('UserConfigService', error);
            });
    }
    ...
    upsert(upsertData: Record<string, unknown>): Promise<void> {
        ...
        return this.httpClient.patch<void>(this.getApiBasePath(), upsertData, { headers }).then((response) => {
            (Contena.Service('cacheService') as CacheServiceContract).invalidateCaches({
                cacheKey: [
                    'user-config',
                    currentUserId,
                ],
