...
export function date(val: string, options: DateFilterOptions = {}): string {
    ...
    const lastKnownLang = Contena.Application.getContainer('factory').locale.getLastKnownLocale();
    const userTimeZone = Contena?.Store?.get('session')?.currentUser?.timeZone ?? 'Asia/Shanghai';
