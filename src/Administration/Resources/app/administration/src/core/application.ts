...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * Contena.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * Contena.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Contena.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * Contena.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Contena.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * Contena.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * Contena.Application.addServiceProvider('mediaService', (container) => {
     *    return new MediaApiService(container.mediaService);
     * });
     */
    ...
    registerApiContext(context: ContextStore['api']): ApplicationBootstrapper {
        Contena.Context.api = Contena.Classes._private.ApiContextFactory(context);

        return this;
    }

    /**
     * Registers the Administration application context.
     */
    registerAppContext(context: ContextStore['app']): ApplicationBootstrapper {
        Contena.Context.app = Contena.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * Contena.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Contena.Application.addServiceProviderMiddleware('mediaService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = Contena.Context.app.features;
     ...
     *
     * @example
     * Contena.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * Contena.Application.addServiceProviderDecorator('mediaService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    start(config = {}): Promise<void | ApplicationBootstrapper> {
        return this.registerConfig(config).initializeFeatureFlags().startBootProcess();
    }
    ...
    bootLogin(): Promise<void | ApplicationBootstrapper> {
        ...
        sessionStorage.setItem('ct-login-should-reload', 'true');
    ...
    bootFullApplication(): Promise<void | ApplicationBootstrapper> {
        ...
        return this.initializeInitializers(initPreContainer, '-pre')
            ...
            .then(() => Promise.all(Contena.Plugin.getBootPromises()))
            .then(() => {
                if (!this.view) {
                    return Promise.reject();
                }
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (Contena.Context.app.environment === 'testing') {
    ...
    private async initializeLoginInitializer(): Promise<unknown[]> {
        ...
        await Contena.Application.getContainer('init-pre').apiServices;
    ...
    private async loadPlugins(): Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode) {
            const response = await fetch('./ct-plugin-dev.json');
            plugins = (await response.json()) as bundlesPluginResponse;

            // Added via webpack.config.js@193 || plugins.vite.ts@123
            if (Contena.Utils.object.hasOwnProperty(plugins, 'metadata')) {
                delete plugins.metadata;
            }
        } else {
            plugins = Contena.Context.app.config.bundles as bundlesPluginResponse;
        }

        // prioritize main ct-commercial plugin because other plugins depend on the license handling
        if (plugins.CtCommercial) {
            await this.injectPlugin(plugins.CtCommercial);
        }

        const injectAllPlugins = Object.entries(plugins)
            .filter(([pluginName]) => {
                // Filter the ct-commercial bundle because it was loaded beforehand
