...
import type { MessageDataType, HandleMethod, BaseMessageOptions } from '@contena/meteor-admin-sdk/es/channel';
import { handle as sdkHandle } from '@contena/meteor-admin-sdk/es/channel';
import type { ContenaMessageTypes } from '@contena/meteor-admin-sdk/es/message-types';
...
export default {
    ...
    handle: <MESSAGE_TYPE extends keyof ContenaMessageTypes>(
        type: MESSAGE_TYPE,
        ...
        const aclHook = (
            ...
            return new Promise((resolve, reject) => {
                ...
                const missingPrivileges = data.privileges!.filter((p) => !Contena.Service('acl').can(p));
                if (missingPrivileges.length > 0) {
                    reject(new MissingPrivilegesError(type, missingPrivileges));
                } else {
                    const result = method(data, additionalInformation);

                    if (isPromise<ContenaMessageTypes[MESSAGE_TYPE]['responseType']>(result)) {
