...
[Meteor Admin SDK](https://github.com/contena/meteor). Instead of calling `Contena.Telemetry.track()`
...
|---|---|----------------------------------------------------------------------------|
| **Method** | `Contena.Telemetry.track(eventData)` | `telemetry.dispatch({ event, data })` from `@contena/meteor-admin-sdk` |
...
| **Debug mode** | `Contena.Telemetry.debug = true` in browser console | Not available – inspect `postMessage` traffic in DevTools instead          |

## Firing events

```ts
import { telemetry } from '@contena/meteor-admin-sdk';
...
A minimal composable wrapper keeps call sites clean (pattern from CtAnalytics
...
Usage in a component (CtAnalytics `client/src/components/date-filter/date-filter.vue`):
...
```ts
import { consent } from '@contena/meteor-admin-sdk';
...
- `Contena.Telemetry.debug = true` has no effect from inside an iframe. Use the browser's `postMessage`
