...
export class Telemetry {
    ...
    private initializePageChanges(): void {
        void Contena.Application.viewInitialized.then(() => {
            // @ts-expect-error router is available after viewInitialized is fulfilled
            const router = Contena.Application.view.router as Router;
    ...
    private dispatchEvent<N extends EventTypes>(eventType: N, eventData: EventPayload<N>): void {
        Contena.Utils.EventBus.emit('telemetry', new TelemetryEvent<N>(eventType, eventData));
    }
    ...
    private initializeDebugListener(): void {
        ...
        watch(this.#debug, (newValue) => {
            if (newValue) {
                Contena.Utils.EventBus.on('telemetry', debugListener);
            } else {
                Contena.Utils.EventBus.off('telemetry', debugListener);
            }
        });
    }
}
