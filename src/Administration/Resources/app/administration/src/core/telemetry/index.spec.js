...
describe('src/core/telemetry/index.js', () => {
    ...
    beforeEach(() => {
        ...
        Contena.Service = jest.fn((serviceName) => {
    ...
    describe('user changes', () => {
        ...
        it('dispatches identify events', async () => {
            ...
            Contena.Utils.EventBus.on('telemetry', handler);

            const currentUser = {
                id: '8b8ebef4-7fa3-4844-ab7e-120463ea558b',
                admin: true,
            };
            Contena.Store.get('session').currentUser = currentUser;
            ...
            Contena.Utils.EventBus.off('telemetry', handler);
        });
    });
    ...
    describe('page changes', () => {
        it('emits page change event after a router push', async () => {
            ...
            Contena.Application.view.router = router;
            await router.push({ name: 'home' });

            Contena.Application.viewInitialized = Promise.resolve();
    ...
    describe('debug', () => {
        it('registers a listener if debug is turned on', async () => {
            ...
            const onSpy = jest.spyOn(Contena.Utils.EventBus, 'on');
            const offSpy = jest.spyOn(Contena.Utils.EventBus, 'off');
