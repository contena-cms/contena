/* eslint-disable ct-test-rules/await-async-functions */
...
async function importComponent(componentName) {
    ...
    if (componentConfig.r === true) {
        // eslint-disable-next-line ct-core-rules/enforce-async-component-registers
        Contena.Component.register(componentName, component);
    }

    // The component extends another component check the extended component is registered
    if (componentConfig.en) {
        if (!Contena.Component.getComponentRegistry().has(componentConfig.en)) {
    ...
    if (componentConfig.e) {
        Contena.Component.extend(componentName, componentConfig.en, component);
    }
...
export default async function wrapTestComponent(componentName, config = {}) {
    ...
    if (syncComponents.includes(componentName) || config?.sync === true) {
        return new Promise((resolve) => {
            Contena.Component.build(componentName).then((res) => {
