...
function isComponentCall(call: CallExpression<ts.CallExpression>, functionString: string): boolean {
    ...
    return [
        `Contena.Component.${functionString}`,
...
function buildAliasPathForArrowFunctionImport(arrowFunction: ArrowFunction, sourceFile: SourceFile): string {
    ...
    // Contena.Component.register('ct-xyz', () => import('src/app/xyz'));
...
for (const sourceFile of sourceFiles) {
    // collect all "Contena.Component.register" or "Contena.Component.extend" calls inside the file
...
const filestring = `export default ${JSON.stringify(componentImportMap)};`
