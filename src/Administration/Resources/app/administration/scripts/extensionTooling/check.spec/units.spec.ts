...
describe('scripts/extensionTooling/check units', () => {
    ...
    beforeEach(() => {
        projectRoot = createTempProject('ct-tooling-check-');
    });
    ...
    /**
     * The reported failure: an installation reached through /tmp/project whose real location
     * is /private/tmp/project. The root then also occurs in the MIDDLE of the path
    ...
    describe('--update-baseline on an extension that cannot hold one', () => {
        const options = { projectRoot: '/project', administrationRoot: '/project/admin', updateBaseline: true };
        const failing = (basePath: string, vendor = false) =>
            extension(project('Frontend', { basePath, vendor }), { eslint: run('failed', { findings: 3 }) });

        it('keeps failing the run instead of absorbing the findings', () => {
            expect(computeExitCode([failing('src/Frontend')], options, false)).toBe(1);
        ...
        it('says why nothing was recorded', () => {
            const recorded = recordProjectBaseline(failing('src/Frontend'), '/project');
        ...
        it('names the vendor reason separately, since vendor findings are non-fatal anyway', () => {
            const recorded = recordProjectBaseline(failing('vendor/acme/x', true), '/project');
