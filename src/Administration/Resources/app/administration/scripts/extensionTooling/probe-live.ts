...
export const ESLINT_NOT_COMPOSED_DETAIL = 'own ESLint config does not compose the Contena preset — see setup guidance.';
...
/**
 * Live probe: a custom tsconfig composes the Contena preset when its
...
export async function probeTsMode(
    ...
    return verdict.composes
        ? { path: relativePath, composes: false, detail: 'the tsconfig does not reach the Contena type surface.' }
        : verdict;
}

/**
 * Live probe: a custom ESLint config composes the Contena preset when the
