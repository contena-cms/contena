...
describe('extension tooling shared type surface (e2e)', () => {
    it(
        ...
        async () => {
            const projectRoot = createTempProject('ct-tooling-surface-clean-');
    ...
    it(
        ...
        async () => {
            const projectRoot = createTempProject('ct-tooling-surface-broken-');
            const administrationRoot = createVendorAdmin(projectRoot, { entitySchema: 'real' });
            ...
            try {
                ...
                expect(fs.existsSync(path.join(projectRoot, 'custom/plugins/Plug/.contena-admin-baseline.json'))).toBe(
                    false,
                );
            } finally {
                cleanupTempProject(projectRoot);
