...
describe('scripts/extensionTooling e2e', () => {
    ...
    beforeAll(() => {
        projectRoot = createTempProject('ct-tooling-e2e-');
        ...
        writeFile(path.join(shimAdminFolder, 'src/main.ts'), [
            "export const shimmedRepository = Contena.Service('repositoryFactory').create('blog');",
        ]);
        writeFile(path.join(shimAdminFolder, 'tsconfig.json'), [
            '{',
            '    "extends": "./.contena/tsconfig.json",',
        ...
        for (const bundleName of [
            ...
            writeFile(
                ...
                ['export const suiteCriteria = new Contena.Data.Criteria(1, 25);'],
            );
        }
        ...
        writePluginsConfig(projectRoot, [
            ...
            },
            {
                ...
                basePath: 'vendor/contena/administration',
                administrationPath: 'Resources/app/administration/src',
            },
        ]);
    ...
    it('connects zero-config, shim-config, multi-bundle, and vendor layouts', () => {
        ...
        // tsconfig/eslint that compose the generated .contena/ bridge.
        ...
        expect(
            fs.existsSync(
                path.join(projectRoot, 'custom/plugins/ShimConfig/src/Resources/app/administration/.contena/tsconfig.json'),
            ),
        ).toBe(true);
    ...
    it(
        ...
        async () => {
            const cardPath = path.join(
                projectRoot,
                'custom/plugins/ZeroConfig/src/Resources/app/administration/src/component/ct-typed-card.vue',
            );

            writeFile(cardPath, [
                '<template>',
                '    <button @click="run">{{ shownOnly }}</button>',
                '</template>',
                '',
    ...
    it(
        ...
        async () => {
            fs.appendFileSync(zeroConfigMainPath, "const invalidBlogId: number = 'invalid';\nvoid invalidBlogId;\n");
    ...
    it(
        ...
        async () => {
            fs.appendFileSync(
                zeroConfigMainPath,
                "export const nopeRepository = Contena.Service('repositoryFactory').create('nope_entity');\n",
            );
