...
const BRIDGE_ESLINT_LINES = [
    ...
    'export default [ ...contena /* , your rules */ ];',
];
...
export function describeToolGuidance(
    ...
    const why = config.detail ?? 'the config does not compose the Contena preset.';
...
export function describeNextStep(
    ...
    if (state === 'needs-bridge') {
        return [
            "It isn't checked with the Contena preset yet. Bridges are generated automatically —",
            `    run \`${commands.setup}\``,
