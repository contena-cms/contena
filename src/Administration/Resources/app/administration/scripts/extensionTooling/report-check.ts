...
function skipReason(tool: string): string {
    return tool === 'TypeScript'
        ? 'own tsconfig does not reach the Contena type surface'
        : 'own config does not compose the Contena factory';
}
...
export function renderCheckReport(result: CheckExtensionsResult, options: RenderOptions = {}): string {
    ...
    if (options.fix && withFindings > 0) {
        ...
        lines.push(
            '',
            colors.dim(
                '  --fix applied the auto-fixable findings (incl. Contena ct-* → mt-* deprecation codemods). ' +
