 ...
 *
 * Automatic bridging: the self-ignoring .contena/ bridge generated for every
...
describe('scripts/extensionTooling/setup automatic bridging', () => {
    ...
    beforeEach(() => {
        ({ projectRoot, administrationRoot } = createSetupProject('ct-tooling-setup-bridge-'));
    ...
    it('bridges every discovered extension automatically — vendor included', () => {
        ...
        const shimDir = path.join(projectRoot, 'custom/plugins/ZeroConfig/src/Resources/app/administration/.contena');
        ...
        expect(fs.readFileSync(path.join(shimDir, 'eslint.mjs'), 'utf8')).toContain('contenaAdminExtension');
        ...
        for (const bundle of [
            ...
            expect(
                fs.existsSync(
                    path.join(projectRoot, `custom/plugins/Suite/src/${bundle}/Resources/app/administration/.contena`),
                ),
            ).toBe(true);
        }
        ...
        expect(
            fs.existsSync(path.join(projectRoot, 'vendor/acme/custom-admin/src/Resources/app/administration/.contena')),
        ).toBe(true);
        expect(result.writes.some((write) => write.file.includes('.contena'))).toBe(true);
    });
    ...
    it('scaffolds committable plugin configs that extend the bridge and are never overwritten', () => {
        ...
        expect(pluginTsconfig).toContain('"extends": "./.contena/tsconfig.json"');
        expect(pluginTsconfig).not.toContain(GENERATED_MARKER);
        expect(pluginEslint).toContain("import contena from './.contena/eslint.mjs'");
    ...
    it('stays silent about an existing plugin config that already composes the bridge', () => {
        ...
        writeFile(path.join(adminFolder, 'eslint.config.mjs'), [
            `import contena from '${BRIDGE_ESLINT_SPECIFIER}';`,
    ...
    it('merges plugin aliases and preset host paths into the bridge tsconfig', () => {
        ...
        const shimTsconfig = fs.readFileSync(
            path.join(projectRoot, 'custom/plugins/ZeroConfig/src/Resources/app/administration/.contena/tsconfig.json'),
            'utf8',
        );
