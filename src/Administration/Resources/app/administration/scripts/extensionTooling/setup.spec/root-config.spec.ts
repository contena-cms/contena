...
describe('scripts/extensionTooling/setup root-config bridge mode', () => {
    ...
    beforeEach(() => {
        projectRoot = createTempProject('ct-tooling-rootcfg-');
    ...
    it('bridges once beside an explicit --root-config and scaffolds one covering config', () => {
        ...
        expect(fs.existsSync(path.join(projectRoot, 'custom/plugins/Mono/.contena/tsconfig.json'))).toBe(true);
        expect(
            fs.existsSync(
        ...
        expect(scaffold).toContain('"extends": "./.contena/tsconfig.json"');
    ...
    it('shares one bridge for the package config that governs several roots', () => {
        ...
        expect(fs.existsSync(path.join(projectRoot, 'custom/plugins/Shared/.contena/tsconfig.json'))).toBe(true);
    ...
    it('bridges each governing config of a package with several shared configs', () => {
        ...
        expect(fs.existsSync(path.join(projectRoot, 'custom/plugins/Ambiguous/src/GroupOne/.contena'))).toBe(true);
        expect(fs.existsSync(path.join(projectRoot, 'custom/plugins/Ambiguous/src/GroupTwo/.contena'))).toBe(true);
