...
describe('renderCheckReport findings baseline', () => {
    ...
    it('lists the baselines written under --update-baseline', () => {
        const output = report([extension(project('MyPlugin'))], {
            baselineUpdates: ['custom/plugins/MyPlugin/.contena-admin-baseline.json — 43 recorded, 2 pruned'],
        });

        expect(output).toContain('Baseline updated');
        expect(output).toContain('custom/plugins/MyPlugin/.contena-admin-baseline.json — 43 recorded, 2 pruned');
    });
