 ...
 * administration:*` in a Composer/Flex installation (where those composer scripts do
...
const FLEX_ADMIN_ROOT = '/project/vendor/contena/administration/Resources/app/administration';
const MONOREPO_ADMIN_ROOT = '/project/src/Administration/Resources/app/administration';
const flexCommands = resolveToolingCommands('/project', FLEX_ADMIN_ROOT);

describe('extension tooling command layout', () => {
    it('uses the composer scripts for a platform (monorepo) checkout', () => {
        expect(resolveToolingCommands('/project', MONOREPO_ADMIN_ROOT)).toEqual(DEFAULT_TOOLING_COMMANDS);
