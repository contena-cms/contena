...
const FLEX_COMMANDS = resolveToolingCommands(
    '/project',
    '/project/vendor/contena/administration/Resources/app/administration',
);

describe('scripts/extensionTooling/report renderSetupReport', () => {
    const managed = project('FroshTools');
    const needsBridge = project('CtPayPal', {
        tsconfig: owned('custom/plugins/CtPayPal/src/Resources/app/administration/tsconfig.json', 'why it drifts'),
        eslintConfig: owned('custom/plugins/CtPayPal/src/Resources/app/administration/eslint.config.mjs', 'and why'),
    });
    ...
    it('classifies every bucket and gives each uncovered one its action', () => {
        ...
        expect(output).toContain('"extends": "./.contena/tsconfig.json"');
        expect(output).toContain("import contena from './.contena/eslint.mjs'");
    ...
    it('gives each unwired config the fix for its own defect', () => {
        ...
        expect(bareBlock).toContain('add "extends": "./.contena/tsconfig.json"');
        expect(bareBlock).toContain("import contena from './.contena/eslint.mjs'");
    });
    ...
    it('marks the extensions this run freshly bridged', () => {
        const output = setupReport(
            setupResult([bridged], {
                ...
                writes: [
                    {
                        file: 'custom/plugins/Bridged/src/Resources/app/administration/.contena/tsconfig.json',
                        state: 'created',
                    },
                ],
            }),
    ...
    it('labels changed files by ownership and summarizes the bridge files', () => {
        const result = setupResult([project('Mono')], {
            changed: true,
            writes: [
                { file: 'custom/plugins/Mono/.contena/tsconfig.json', state: 'created' },
                { file: 'custom/plugins/Mono/.contena/eslint.mjs', state: 'created' },
        ...
        expect(output).toContain('2 git-ignored .contena/ bridge file(s) — never commit');
        expect(output).not.toContain('custom/plugins/Mono/.contena/tsconfig.json');
    });
    ...
    it('lists platform bundles in their own bucket, excluded from the count', () => {
        const frontend = project('Frontend', {
            ...
            eslintConfig: owned('src/Frontend/Resources/app/administration/eslint.config.mjs', 'drift'),
        });
        const output = setupReport(
            setupResult([
                managed,
        ...
        expect(output).toContain('platform  Frontend');
        // Platform bundles never get an action: the core toolchain checks them.
        expect(output).not.toContain('Frontend: drift');

        const emptyWithPlatform = setupReport(setupResult([frontend]));
