...
function bridgeReadmeContent(context: GeneratorContext): string {
    return [
        ...
        'and the installed Contena Administration: its `tsconfig.json` and `eslint.mjs`',
        ...
        '  commit it.',
        `- After a Contena update (or moving the project), re-run \`${context.commands.setup}\``,
        '  to regenerate it.',
        '',
    ].join('\n');
}
...
/**
 * Writes the git-ignored bridge files into `<bridgeParent>/.contena/`. The
    ...
    record(
        ...
        writeManagedFile(
            ...
            [
                ...
                `import { contenaAdminExtension } from ${JSON.stringify(asRelativeSpecifier(shimEslintPath, factoryPath))};`,
                '',
                ...
                '',
                'export function contenaAdminExtensionConfig(options = {}) {',
                '    return contenaAdminExtension({ tsconfigRootDir: adminFolder, ...options });',
                '}',
                '',
                'export default contenaAdminExtensionConfig();',
                '',
            ].join('\n'),
            context.dryRun,
        ),
