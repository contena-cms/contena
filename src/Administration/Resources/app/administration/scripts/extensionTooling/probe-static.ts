...
export function tsconfigVerdict(absolutePath: string, relativePath: string): OwnedConfig {
    ...
    if (!reachesPreset(absolutePath, config)) {
        return {
            ...
            composes: false,
            detail:
                'the extends chain does not reach the Contena preset or a generated ' +
...
export function eslintConfigVerdict(absolutePath: string, relativePath: string): OwnedConfig {
    ...
    return {
        path: relativePath,
        composes: false,
        detail: 'the config does not compose the Contena factory, so the preset rules never apply.',
