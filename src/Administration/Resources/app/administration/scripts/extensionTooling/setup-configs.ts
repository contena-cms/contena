...
export function createRootEslintConfig(context: GeneratorContext, projects: ExtensionToolingProject[]): ManagedFileState {
    ...
    const content = [
        `// ${GENERATED_MARKER}`,
        `import { contenaAdminExtension } from ${JSON.stringify(factorySpecifier)};`,
        '',
    ...
    if (state === 'conflict') {
        context.instructions.push(
            [
                ...
                `    import { contenaAdminExtension } from ${JSON.stringify(factorySpecifier)};`,
                '    export default [',
                '        ...contenaAdminExtension({ tsconfigRootDir: import.meta.dirname, extensionRoots: [/* … */] }),',
                '        // your own config',
                '    ];',
            ].join('\n'),
        );
...
export function scaffoldExtensionConfigs(
    ...
    const tsconfigContent =
        `// ${configKind} for ${name}. Extends the generated Contena bridge in ${SHIM_DIR_NAME}/\n` +
    ...
    const eslintContent = [
        `// ${configKind} for ${name}. Composes the generated Contena bridge in ${SHIM_DIR_NAME}/`,
    ...
    if (tsconfigResult === 'skipped') {
        ...
        if (!verdict.composes) {
            warn(
                context,
                `${tsconfigPath} already exists and was not touched — ${verdict.detail ?? 'it does not compose the Contena preset.'} ` +
    ...
    if (eslintResult === 'skipped') {
        ...
        if (!verdict.composes) {
            warn(
                context,
                `${eslintPath} already exists and was not touched — ${verdict.detail ?? 'it does not compose the Contena factory.'} ` +
