...
describe('scripts/extensionTooling/probe-static', () => {
    ...
    beforeEach(() => {
        projectRoot = createTempProject('ct-tooling-probe-');
    });
    ...
    describe('tsconfigVerdict', () => {
        ...
        it('rejects a config whose chain never reaches the preset', () => {
            ...
            expect(verdict.detail).toContain('does not reach the Contena preset');
        ...
        it('rejects a config that inherits the bridge "files" without declaring an own "include"', () => {
            writeFile(path.join(projectRoot, 'admin/.contena/tsconfig.json'), [
                '{ "files": ["../../extension-tooling/admin-types.d.ts"] }',
            ]);
            writeFile(path.join(projectRoot, 'admin/tsconfig.json'), ['{ "extends": "./.contena/tsconfig.json" }']);
        ...
        it('accepts an "include" inherited from the plugin\'s own base config', () => {
            writeFile(path.join(projectRoot, 'admin/.contena/tsconfig.json'), ['{ "files": ["x.d.ts"] }']);
            writeFile(path.join(projectRoot, 'admin/base.json'), [
                '{ "extends": "./.contena/tsconfig.json", "include": ["src/**/*"] }',
            ]);
    ...
    describe('eslintConfigVerdict', () => {
        it('accepts bridge and factory imports, rejects an unrelated config', () => {
            const cases = {
                'bridge.mjs': ["import contena from './.contena/eslint.mjs';"],
                'factory.mjs': ["import { contenaAdminExtension } from '../extension-tooling/eslint.mjs';"],
            ...
            expect(own.detail).toContain('does not compose the Contena factory');
        });
    });
});
