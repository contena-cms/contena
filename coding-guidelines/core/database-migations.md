## Introduction
Changing the database structure of Contena is an important and sensitive topic, because it can effect the installation of customers and their data in many ways.
...
Migrations in Contena are grouped by major versions.
...
As every other change, also your database changes should always be [backward compatible](https://developer.contena.cn/docs/resources/guidelines/code/backward-compatibility.html) for minor and patch releases and support blue-green deployment.
...
- `\Contena\Core\Framework\Migration\MigrationStep::dropTableIfExists`
- `\Contena\Core\Framework\Migration\MigrationStep::dropColumnIfExists`
- `\Contena\Core\Framework\Migration\AddColumnTrait::columnExists`
...
When naming database tables, it's essential to steer clear of prefixes like 'ct_' that were historically used for plugins.
...
Don'ts:
On the other hand, avoid using prefixes such as 'ct_' as seen in past plugins.
For example, refraining from naming a table 'ct_order_history' ensures better scalability, clarity, and uniformity within the database schema.
