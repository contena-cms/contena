...
```html
<ct-field type="text" v-if="feature.isActive('v6.5.0.0')"></ct-field>
```

### Using flags in config.xml

When you want to toggle config input fields in config.xml like [basicInformatation.xml](https://gitlab.contena.cn/contena/6/product/platform/-/blob/trunk/src/Core/System/Resources/config/basicInformation.xml), you can add a `flag` element like this:
...
...
    public function boot(): void
    {
        Feature::setRegisteredFeatures(
            ...
            $this->container->getParameter('kernel.cache_dir') . '/contena_features.php'
        );
...
```
